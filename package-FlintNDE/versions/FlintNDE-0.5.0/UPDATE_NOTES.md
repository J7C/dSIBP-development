...
- 独立包与 MadStree v0.13 的 `Vendor/FlintNDE` 保持同一份实现和 0.4.0 版本身份。
...
- 完整 Python 回归 170/170；Wolfram `Needs["FlintNDE`"]` 端到端 25/25，其中新增测试覆盖
  浅层目录、259/260 字符边界及 Python 前置输入写入失败。
- 独立包与 MadStree v0.13 Vendor 的 42 个非缓存交付文件逐文件 SHA-256 一致。
