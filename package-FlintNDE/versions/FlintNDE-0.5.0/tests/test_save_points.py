...
class SavePointTests(unittest.TestCase):
    ...
    def test_raw_path_save_tags_write_primary_results_in_cwd(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                os.chdir(previous)
            root = Path(directory)
            point_files = save_point_files(root)
            self.assertEqual(len(point_files), 2)
            summary = read_save_summary(root)
            ...
            self.assertEqual(result["primary_snapshots"][-1][0, 0], acb(7))

    def test_saved_value_is_primary_when_orders_are_distinguishable(self) -> None:
        """保存点必须写低阶生产值，而不是高阶参考值。"""

        system = AnalyticMatrixSystem(
            lambda _point: acb_mat([[1]]),
            1,
            singularities=(acb(10),),
            name="primary-save-system",
        )
        with tempfile.TemporaryDirectory() as directory:
            with self.assertWarnsRegex(UserWarning, "saved values must be produced"):
                result = transport_path_refined(
                    system,
                    column_vector([1]),
                    [acb(0), (acb("1/2"), "save")],
            ...
            record = read_first_save_record(Path(directory))
        primary = result["primary_snapshots"][-1][0, 0]
        reference = result["reference_snapshots"][-1][0, 0]
        saved = acb(record["result"][0]["real"], record["result"][0]["imag"])
        self.assertGreater(float(abs(primary - reference).mid()), 1.0e-5)
        self.assertTrue(result["save_points_forced_certified"])
        self.assertLess(float(abs(saved - primary).mid()), 1.0e-35)
        self.assertGreater(float(abs(saved - reference).mid()), 1.0e-5)
    ...
    def test_completed_point_survives_later_transport_failure_without_summary(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            root = Path(directory)
            self.assertEqual(len(save_point_files(root)), 1)
    ...
    def test_formal_exponential_start_can_be_saved_but_not_continued(self) -> None:
        ...
        boundary = exponential_boundary([{"phi": [{"power": -1, "coefficient": 1}], "a": 0, "b": 0, "C": [0, 1]}])
        with tempfile.TemporaryDirectory() as directory:
            transport_path(
    ...
    def test_exponential_boundary_rejects_wrong_phi_signature(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "phi does not match exact sector"):
