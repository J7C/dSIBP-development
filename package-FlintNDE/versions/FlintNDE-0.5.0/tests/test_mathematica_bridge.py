...
def test_polynomial_simple_poles_are_discovered_and_executed_without_replanning() -> None:
    ...
    with patch.object(bridge, "plan_transport_path", fail_if_replanned):
...
def test_higher_execution_precision_requires_replanning() -> None:
    ...
    with _ASSERTIONS.assertRaisesRegex(ValueError, "replan at the requested working precision"):
        bridge.run_request(
...
def test_exact_intermediate_singular_target_round_trips_through_bridge() -> None:
    """两阶段 bridge 保留精确奇点用户序号、Infinity 文本和后续普通值。"""

    system = _rational_system(["-1"], ["-1", "1"])
    planned = bridge.run_request(
        _plan_request(
            system,
            ["1/2", "1", "3/2", "2"],
            singularity_mode="singularity_jump",
    ...
    assert planned["status"] == "complete"
    assert planned["plan"]["path"]["report"]["singular_target_count"] == 1

    executed = bridge.run_request(_execute_request(system, planned))
    assert executed["status"] == "complete"
    assert len(executed["singularTargets"]) == 1
    singular = executed["singularTargets"][0]
    assert singular["userPointIndex"] == 1
    assert singular["classification"] == "true_pole"
    assert singular["values"] == [{"text": "Infinity"}]
    with localcontext() as context:
        context.prec = 50
        final_value = Decimal(executed["primaryFinalVector"][0]["real"])
        assert abs(final_value + Decimal(1)) < Decimal("1e-25")


def test_wolfram_precision_markers_are_removed_before_exact_parsing() -> None:
    ...
    for literal, expected in cases:
        with _ASSERTIONS.subTest(literal=literal):
...
def test_partial_fraction_schema_rejects_unexpected_field() -> None:
    ...
    with _ASSERTIONS.assertRaisesRegex(ValueError, "current schema"):
...
def test_execute_rejects_internal_raw_plan() -> None:
    ...
    with _ASSERTIONS.assertRaisesRegex(ValueError, "plannedResult"):
...
class MathematicaBridgeTest(unittest.TestCase):
    ...
    test_polynomial_simple_poles = staticmethod(
        test_polynomial_simple_poles_are_discovered_and_executed_without_replanning
    )
    test_polynomial_only = staticmethod(test_polynomial_only_rational_input_uses_formula_route)
    test_higher_precision = staticmethod(test_higher_execution_precision_requires_replanning)
    test_high_order_pole = staticmethod(test_high_order_pole_uses_general_rational_route)
    test_default_avoid = staticmethod(test_default_avoid_refuses_and_explicit_jump_plans)
    test_exact_singular_target = staticmethod(
        test_exact_intermediate_singular_target_round_trips_through_bridge
    )
    test_wolfram_precision = staticmethod(
        test_wolfram_precision_markers_are_removed_before_exact_parsing
    )
