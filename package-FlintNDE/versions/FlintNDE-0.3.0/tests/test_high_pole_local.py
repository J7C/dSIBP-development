...
class HighPoleLocalTest(unittest.TestCase):
    ...
    def test_irregular_start_rejects_boundary_without_phi(self) -> None:
        """指数型奇点不得从 ``C`` 静默猜测指数签名。"""

        system = RationalMatrixSystem(
            ((rational_function(1, [0, 0, 1]),),),
            name="scalar-irregular-start-missing-phi",
        )
        basis = build_local_solution_basis(system, 0, 24)
        with self.assertRaisesRegex(TypeError, "ExponentialBoundary.*phi"):
            basis.resolve_boundary(
                frobenius_boundary([{"a": 0, "b": 0, "C": [1]}])
            )

    def test_irregular_start_requires_and_validates_explicit_phi(self) -> None:
        """指数型奇点起点必须给出 ``{phi,a,b,C}``，并由 exact sector 验证。"""
        ...
        result = transport_path(
            system,
            exponential_boundary([{"phi": [{"power": -1, "coefficient": -1}], "a": 0, "b": 0, "C": [1]}]),
            path,
            order=32,
            sample_count=96,
        )
        ...
        self.assertEqual(
            result[1][0]["boundary"]["term_resolutions"][0]["verified_sector"],
            1,
        )
        self.assertLess(
