...
\begin{verbatim}
python -m pip install path/to/package-FlintNDE/versions/FlintNDE-0.5.0
\end{verbatim}
...
The first \texttt{import flintnde} in each Python process also prints one citation notice to
stderr: ``FlintNDE package paper, arXiv identifier pending.''  It does not list dSIBP,
MadStree, AMFlow, or any other paper.  A file-protocol adapter may set
\texttt{FLINTNDE\_SUPPRESS\_CITATION\_NOTICE=1} before import and preserve its own citation
list in response metadata; the notice must never be mixed into machine-readable stdout.

The source tree also provides a standard Wolfram Language package.  Add the 0.5.0 version
...
In explicit singularity-jump mode, an exact intermediate singular user point is not an
ordinary transport node.  One local power--log basis owns that target and every user point
inside its convergence disk on both sides; the disk radius is the distance to the nearest
other singularity.  Transport resumes from the outgoing ordinary match point.  A terminal
singular target uses \texttt{plan\_singular\_target\_match}: if the current state lies outside
the conservative local disk, the planner inserts a hidden ordinary match point along the
incoming direction.  The lower-order primary chain supplies the reported value, while the
higher-order reference chain checks component classifications and finite-value accuracy.
Non-collinear turning at an exact singular point remains fail closed because the continuation
branch is not fixed by that input.

This target logic is coordinate agnostic. Candidate points are discovered from the complete exact
rational matrix, and a target may lie anywhere in $\mathbb Q(i)$; neither the variable name nor the
numeric value zero has a distinguished role. The regression suite shifts a target to $2+\mathrm{i}$ while
retaining another finite singularity at $5+\mathrm{i}$ and obtains one removable and one true-pole component
from the same two-dimensional system. This guards the general entry against zero-point, single-letter,
and topology-specific classifications.
...
primary and reference chains are evaluated independently.  User-visible endpoint values,
dense samples, saved points, and regulator-fit samples are taken from the lower-order primary
chain.  The higher-order reference chain is retained only for the final relative-difference
diagnostic; that diagnostic is not confused with the radius carried by an individual
...
refined transport writes the primary chain only; the reference chain remains an accuracy
diagnostic and never replaces a user result.
...
An explicit \texttt{sample\_points} sequence is an ordered production candidate pool.  It may
contain more values than the current square interpolation needs: the first round consumes only
the required prefix, and a failed validation round consumes later candidates without recomputing
previous solves.  The optional \texttt{initial\_internal\_maximum\_power} fixes the highest power
included in that first square system.  Once an explicit pool is exhausted, reconstruction fails
to generate a regulator value outside the user-approved range, but returns the current best
coefficients with \texttt{precision\_target\_met=False} and reason
\texttt{candidate\_pool\_exhausted}.  To bound the complete run, including checks, the user must
also provide an explicit disjoint \texttt{validation\_points} sequence.

Alternatively, \texttt{sample\_angle\_range=(theta\_min,theta\_max)} selects an open angular
interval in radians for automatic complex samples.  The implementation chooses at most three
uniformly spaced interior rays and cycles the precision-driven radii over them.  No boundary ray
is used, and there is no user radius cap: the magnitude remains
$\epsilon_j=\epsilon_0(1+j\delta)$ with $\epsilon_0$ fixed by the same AMFlow-inspired accuracy
formula.  Automatic validation samples preserve the corresponding angle and scale only the radius.
...
and the rationalized one-percent grid.  An explicit ordered sequence is a candidate pool:
each round consumes only the prefix needed by its current internal highest power, reuses earlier
values, and never generates a point after the pool is exhausted. \\
\texttt{sample\_angle\_range=}
\newline\texttt{"automatic"} & Optional open angular interval $(\theta_{\min},\theta_{\max})$
in radians for automatic complex samples.  Up to three uniformly spaced interior rays are used;
the radius remains automatic.  This option is mutually exclusive with explicit
\texttt{sample\_points}; automatic preserves the positive-real-axis grid. \\
\texttt{sample\_count="automatic"} & Automatic production interpolation count.  With explicit
\texttt{sample\_points}, an explicit count must equal the total candidate-pool length; it is not
the number consumed in the first round. \\
...
\newline\texttt{"automatic"} & Optional explicit independent grid.  These values never enter
the fit and must be disjoint from the whole production candidate pool.  Supply both explicit
production and validation grids to restrict every regulator solve to a user-approved range. \\
...
\texttt{fit\_extra\_order=2} & Minimum automatic first-round buffer above the requested highest
power.  It retains its previous meaning when the initial internal power is automatic. \\
\texttt{initial\_internal\_}
\newline\texttt{maximum\_power="automatic"} & Optional absolute first-round internal highest
power $q\ge m$.  With certified leading power $n_{\min}$, the first round requires exactly
$q-n_{\min}+1$ production candidates; insufficient capacity fails before any NDE solve. \\
\texttt{fit\_order\_increment=2} & Additional candidates consumed after each failed validation
round.  Previously computed production and validation values remain cached. \\
\texttt{fit\_max\_rounds=3} & Maximum number of fit/validation rounds.  If a square fit exists but
the target residual is still missed, the current coefficients are returned without precision
certification; diagnostics distinguish candidate exhaustion, the sample cap, and the round cap. \\
