...
\begin{verbatim}
python -m pip install path/to/package-FlintNDE/versions/FlintNDE-v0.1.0.dev0
\end{verbatim}
...
Singular start & The input is an exact $\{a,b,C\}$ power--log record or an explicit
$\{\phi,a,b,C\}$ exponential record, not a finite value at the singularity.  The package validates the indicial, exponential, and log branch, evaluates it
...
certified exponential solution, legacy $\{a,b,C\}$ input infers the sector from $C$;
explicit $\phi$ is checked against that same exact sector. \\
...
Laurent matrices and requires exact equality.  Legacy $\{a,b,C\}$ exponential input remains
accepted by inferring the sector from $C$, while saved output always records $\phi$ explicitly.
...
automatically for legacy input; explicit reusable input supplies $\phi$ and the program checks
it against the inferred sector.  One term must not mix distinct
