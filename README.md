...
- 若 workflow 使用 `LinearSystemMode -> "numeric"`，`numericRules` 必须覆盖所有外部不变量 `kk[i,j]` 和 time-IBP 顶点外部能量；缺失时会在 seed 生成前返回 `numericRulesMissingExternalInvariants` 或 `numericRulesMissingVertexEnergies`。线性系统生成后还会检查系数是否已全数值化，若仍残留 `dim`、`nu`、外部能量等参数则返回 `nonNumericCoefficients` 并列出 `coefficientVariables`。
