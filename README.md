...
- dS 中要区分“进入内线动量的外部三动量向量”和“只进入顶点时间相位的无质量外腿能量模”。`externalMomenta` 只列前者，即会在内线动量中以 `l + sum k` 形式和圈动量纠缠的独立外部向量；顶点 e 指数中的外腿能量由 `vertexEnergies` 给出：若该能量由 `externalMomenta` 张成且用户希望复用关系，应写成外部不变量变量名的函数，例如 `Sqrt[s11]`；若它只是独立的绝对值参数，则内部/文档统一建议记为 `ke[i]`。特别注意 `|ke1+ke2|`、`|ke1|`、`|ke2|` 是三个独立参数时必须分别命名，例如把 `|ke1+ke2|` 另记为 `ke[3]`，不要自动写成 `ke[1]+ke[2]`。外腿能量参数之间不需要也不应自动建立互相点积；只有用户显式把某个顶点能量写成外部不变量表达式时，才与圈外动量空间复用同一个变量。
