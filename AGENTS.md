...
- 三个子项目的工作树各只保留一个当前最新版；升级后旧源码、旧任务书、旧报告和旧结果
  从 Git 历史追溯，不在现行目录内继续归档。当前版本下互不替代的正式 validation case
  可以并列保留，但临时测试、cache、runtime 和已被后续同类证据取代的报告必须清理。
- 本仓库缺省不做向后兼容。当前工作版本中的接口被替代时，物理删除旧函数名、参数别名、
  wrapper、fallback、旧 schema 读取分支、兼容测试和现行文档中的旧调用法；冻结历史版本
  只存在于 Git 历史，不得被当前入口加载或转发。只有用户明确要求迁移期时才可另行设计兼容层。
