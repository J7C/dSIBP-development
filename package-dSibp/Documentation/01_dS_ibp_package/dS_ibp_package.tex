...
\title{dS IBP Package 020.0 用户手册}
\author[a]{Su Yingze}
\affiliation[a]{Institute not specified}
\emailAdd{yingze.su@outlook.com}
\abstract{本手册说明 dS IBP package 的费曼规则、massive/massless 指标约定、time 与 loop-momentum IBP、独立变量微分方程 seed、EOM/缩并 canonical、拓扑输入、公开 API、Kira 接口和 tree vertex family。当前发布为 020.0：full 模式使用 \texttt{J[aList,linePacks,ispList]}，time-only 模式使用 \texttt{J[sectorKey,timeShifts,stateBits]}，初始化后 schema 固定且不能混用。用户显式分别声明任意命名的 \texttt{loopExternal\allowbreak Momenta} 与 \texttt{independentExternal\allowbreak Momenta}，程序验证结构圈数、routing、声明完备性和动力学坐标，不自动猜角色。package 只生成、序列化和验证关系，不运行 reduction。}
...
本 package 生成 dS 圈图的 canonical IBP seed，并把关系转换为后端中立的 \texttt{linearData}。当前稳定发布为 020.0，支持 topology-driven 的任意圈数、任意拓扑以及 massive/massless 混合内线；标准入口和冻结单文件兼容入口分别为
\begin{verbatim}
package-dSibp/versions/020_dSIBP/
package-dSibp/independent-benchmark/package/package_020.0.wl
\end{verbatim}
工作树只保留 018、019、020：018 与 019 是冻结历史版本，020 是当前主线；更早版本从 Git 历史追溯。020 继承标准 context、初始化 metadata、完整 time/loop-momentum 生成元、massive EOM、共同-theta odd-subset contact、contact 可达 sector、跨 sector coincident canonical、Kira serializer/importer 以及 DE/scaling 检查。full 模式保留双端点三槽 line pack；time-only 在 020 改为显式 sector key、compact 时间幂和 building-block 状态位列表。

仓库内标准加载与独立 benchmark 单文件加载是两个等价入口，应在新 kernel 中任选其一：
\begin{verbatim}
packageDir = FileNameJoin[{Directory[],"package-dSibp","versions","020_dSIBP"}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];

(* 独立 benchmark 的自包含单文件入口： *)
Get["package-dSibp/independent-benchmark/package/package_020.0.wl"];
\end{verbatim}
...
\section{020 标准 package 与初始化}

一个 020 example 的开头分为四个独立单元：package 加载、详细输入、缺省 option、初始化。所有路径相对于 example 的 \texttt{main.wl}：
\begin{verbatim}
exampleDir = DirectoryName[$InputFileName];
packageDir = FileNameJoin[{
  exampleDir,"..","..","..","..","versions","020_dSIBP"
}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];
...
\texttt{DSSeeds} 始终完整枚举物理独立的离散基底：massive line 为端点 $n_i=0,1$ 四态，masslessFull quotient 为 shared bit 的二态；raw 四态 relation 仍可由 metadata 与导数检查审计。\texttt{quickCheck}、\texttt{fullDiscrete} 与 \texttt{bounded} 只提供不同的缺省连续 \texttt{seedRanges}，不改变离散基底，也不触发抽样。timeOnly 与 full 共用 Head \texttt{J}，但使用式~\eqref{timeOnlyPublicJ} 与式~\eqref{threeSlotJ} 两个不兼容 schema；不存在需要用户填写的 \texttt{TreeState} 或其它私有分派参数。
