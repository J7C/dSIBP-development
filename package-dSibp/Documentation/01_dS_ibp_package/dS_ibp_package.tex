...
\title{dS IBP Package 018.1 用户手册}
\author[a]{Su Yingze}
\affiliation[a]{Institute not specified}
\emailAdd{yingze.su@outlook.com}
\abstract{本手册说明 dS IBP package 的费曼规则、统一积分表示、massive/massless 指标约定、time 与 loop-momentum IBP、独立变量微分方程 seed、EOM/缩并 canonical、拓扑输入、公开 API、Kira 接口和 tree vertex family。当前发布为 018.1，属于 018 主线：所有公开积分统一写成 \texttt{J[aList,linePacks,ispList]}；用户显式分别声明任意命名的 \texttt{loopExternal\allowbreak Momenta} 与 \texttt{independentExternal\allowbreak Momenta}，程序验证结构圈数、routing、声明完备性和动力学坐标，不自动猜角色。前者使用完整 \texttt{ssij} 根号 Gram 坐标，后者只为用户声明的实际无圈模长生成 \texttt{sE1,sE2,...}。package 只生成、序列化和验证关系，不运行 reduction。}
...
本 package 生成 dS 圈图的 canonical IBP seed，并把关系转换为后端中立的 \texttt{linearData}。当前稳定发布为 018.1，模块源码位于 018 主线，支持 topology-driven 的任意圈数、任意拓扑以及 massive/massless 混合内线；标准入口和冻结单文件兼容入口分别为
\begin{verbatim}
package-dSibp/versions/018_dSIBP/
package-dSibp/independent-benchmark/package/package_018.1.wl
\end{verbatim}
工作树只保留 016、017、018：016 与 017 是冻结历史版本，018 是当前主线。010--015 已于 2026-07-30 从工作树删除，需要时从 Git 历史追溯。018 继承标准 context、初始化 metadata、完整 time/loop-momentum 生成元、massive EOM、共同-theta odd-subset contact、contact 可达 sector、跨 sector coincident canonical、Kira serializer/importer 以及 DE/scaling 检查，并把 massive/massless 统一为双端点三槽 line pack，加入完整结构化 sector normalization、compact subsector 求导、h/H parity transport 及唯一公开 tree 表示。

仓库内标准加载与独立 benchmark 单文件加载是两个等价入口，应在新 kernel 中任选其一：
\begin{verbatim}
packageDir = FileNameJoin[{Directory[],"package-dSibp","versions","018_dSIBP"}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];

(* 独立 benchmark 的自包含单文件入口： *)
Get["package-dSibp/independent-benchmark/package/package_018.1.wl"];
\end{verbatim}
...
\texttt{DSSeeds} 始终完整枚举物理独立的离散基底：massive 为适用的 $n_i=0,1$ 四态，masslessFull 为 $n_2\to0$ 的 $00/10$ 两态；raw 四态 relation 仍可由 metadata 与导数检查审计。\texttt{quickCheck}、\texttt{fullDiscrete} 与 \texttt{bounded} 只提供不同的缺省连续 \texttt{seedRanges}，不改变离散基底，也不触发抽样。\texttt{timeOnly} 与 full mode 使用同一个三参数 \texttt{J} schema；不存在需要用户填写的 \texttt{TreeState} 或其它私有分派参数。
