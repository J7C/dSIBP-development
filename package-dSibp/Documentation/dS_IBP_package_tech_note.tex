...
本 package 对纯 massless 与 massive/massless 混合积分族都采用逐传播子的双 theta 合并路线。对一条内部派生为 masslessFull 的线，公开 \texttt{lines} 项中的 \texttt{"endpoints"->\{u,v\}} 是有序数据。令 $\Delta=\tau_u-\tau_v$，并对 $++/--$ 分别取 $\sigma=+1/-1$，先定义二维物理基
\begin{align}
 M_0 &=
...
每条 massive 完整线 $e$ 都带编译后的 \texttt{functionSystem}。公开输入缺省使用 h preset；需要其它系统时，唯一可选字段 \texttt{"functionSystem"} 接受 \texttt{"h"}、\texttt{"H"} 或给出 $P,Q,T,W$ 的完整 Association。022 不读取旧 \texttt{bbType/eomCoefficients}。
...
实际生成顺序固定为：按式~\eqref{seedEnvelopeInverse017} 构造候选整数点，\emph{在生成元作用前}按当前 sector 的 parity 筛点，再把连续指标值代入 general seed，应用用户已经单向定向的 sector symmetry，并对每个线性项执行 \texttt{Together/Cancel}。\texttt{DSSeeds} 始终只生成符号 general templates；精确数值系数规则由 \texttt{DSLinear[...,CoefficientRules->rules]} 持有。formal DE 工作流必须先按符号 topology 构造参量微分算符和 target closure，再在 post-derivative 阶段代入固定非奇异精确有理点。逐关系的直接复核应收集全部不同的 \texttt{J}，提取各 \texttt{Coefficient} 和常数项，先精确重构原关系，再确认系数域满足当前 linear/Kira 合同；不能把多积分关系除以任意一个 \texttt{J}。生成后的 parity certificate 只检查关系，不删除关系中的积分，也不把不满足 parity 的积分替换为零。\texttt{candidatePointCount}、\texttt{parityAcceptedPointCount}、逐组 shift/range 和空域状态均写入返回数据。用户包络唯一决定规模；package 不设置 seed 数、离散态数或方程数上限。
...
\subsection{Massive 函数系统}

\begin{itemize}
  \item 缺省 h preset：归一化 Hankel，$P=(2\nu+1)/x$、$Q=1$。
  \item \texttt{"functionSystem"->"H"}：裸 Hankel 矩阵 EOM，含 $\nu^2/x^2$ 二次 pole；shrink power 为 $-1$。
  \item \texttt{"functionSystem"->Association}：显式给出并编译 $P,Q,T,W$；不保留旧字段适配。
\end{itemize}
...
\end{equation}
这里 $A_{v,s}=a_{v,s}+a_{0,v,s}$；full line 使用 $B_{e,s}=b_{e,s}+b_{0,e,s}$，shrunk line 使用 $B_{e,s}=b_{S,e,s}+b_{S0,e,s}$；$r_{j,s}$ 是 ISP 分子幂次，因为每个 ISP 是二次齐次的 loop scalar product，所以贡献 $2r_{j,s}$。$L_{\rm root}$ 来自 root topology；contact sector 不重新降圈。time-only 模式收到公开 $J[\texttt{sectorKey},\texttt{timeShifts},\texttt{stateBits}]$ 后，必须先通过唯一的 public-to-internal adapter 恢复带 sector line metadata 的内部三槽表示，再读取次数、ISP 幂和 prefactor；不得把公开三槽按 full-loop line pack 解释。转换失败或输入不符合当前公开 schema 时，\texttt{DSScaleCheck} fail closed。最后一项必须从结构化 \texttt{sectorPrefactorData} 物化完整 $N_s$ 后求 Euler 对数导数，不能只看 \texttt{J} 的 pack。若 $N_s$ 或 master 的显式线性组合关于所声明的 Euler 变量不齐次，该路线返回无效 scaling specification，而不是选择一个次数。\texttt{PureMassiveBubble} 仍保留为 reference 专用关系，不负责含 ISP 的一般 topology。
...
\begin{equation}
 ...
 =\prod_e k_e^{-\left(B_{e,s}-B_{e,r}\right)}.
...
\textbf{当前实现边界（022）}：本节公式在 dSIBP 中服务 massive-only time-only family。公开入口 \texttt{DSTreeSeeds}、\texttt{repIterative}、\texttt{DSTreeNaiveIBP}、\texttt{DSTreeNaiveDE} 与 \texttt{DSTreeDLogDE} 均返回当前 \texttt{J[sectorKey,timeShifts,stateBits]} 表示；naive IBP-DE 与直接 dlog DE 使用同序 normalized masters 交叉检查。含 massless full line 的 quotient-basis 公式尚未独立重推，因此公式型迭代与 DE 接口返回 \texttt{PendingRederivation}，但 general \texttt{DSTreeSeeds} 仍可用于 IBP seed 对照。根目录的独立程序包 \texttt{package-MadStree/} 另行实现直接公式、边界和 FlintNDE 输运；dSIBP 不调用它，MadStree 的结果也不构成 dSIBP 自身的 package oracle。
...
\end{verbatim}
把每个顶点的 $a_e$ 迭代到指定终点；\texttt{Automatic} 表示全零终点。显式终点的长度必须等于顶点数，且每段步数必须是可判定整数。多 sector 情形按 contact-reachable sector DAG 从低到高处理，避免把含 source 的非齐次关系当作齐次 replacement repeated。017 的公开输入始终是三参数 $J$；massive-only family 先按 sector tag 映射到 Private vertex basis，公式内核完成单步关系后再映回同一公开表示。这个要求保证用户 line \texttt{momentum} 决定的传播子模长不会被 fixed-line 内部 notation \texttt{sEi} 替换。含 massless full line 时 quotient-basis 公式尚未重推，接口返回 \texttt{PendingRederivation}，不使用旧单槽公式猜测结果。
...
外腿相位能量导数仍可由 loop 代表元的相位项投影。传播子的动量模长则不能使用完整 loop 动量导数，因为后者作用于积分变量。由式~\eqref{dkh0}--\eqref{dkh1}，以 tree 的 $\tau^a$ convention 对 bare sector integral $I_s$ 写成
\begin{align}
 \partial_k I_s[a,n=0]&=-I_s[a+1,n=1],\label{eq:tree-naive-dk0}\\
 \partial_k I_s[a,n=1]&=I_s[a+1,n=0]-\frac{2\nu+1}{k}I_s[a,n=1].\label{eq:tree-naive-dk1}
\end{align}
...
\end{equation}
所以 Wronskian/zero-point prefactor 的导数不可遗漏，也不得在 $J_s$ 外再次乘 $N_s$。最终 dlog master 为 $F_s=S_sJ_s$，其乘积法则是
\begin{equation}
 \partial_xF_s=(\partial_xS_s)J_s+S_s(\partial_x\log N_s)J_s+S_sN_s\partial_xI_s.
\end{equation}
其中 $S_s$ 是有理 selector；连续幂只存在于 $N_s$ 的 master 定义中，其导数以 $c\nu/k$ 形式进入 DE。\texttt{DSTreeNaiveDE} 最后只使用上述 raw derivative 与 naive 线性规则，输出的逐变量矩阵应与 $\partial_x\Omega$ 在完全相同的 ordered normalized basis 下逐项相等。
...
每个 sector 的 normalized binary integral 按 $\bm J_s=N_s\bm I_s$ 定义；$N_s$ 来自传播子收缩并保存在 sector metadata 中。为了把所有进入同一 lower sector 的 contact 矩阵化为有理常 selector，dlog basis 另选 $\bm F_s=S_s\bm J_s$。$S_s$ 是可追溯的有理 basis coefficient，不是第二份物理 sector prefactor。于是
\begin{align}
 \Omega_{ss}^{\rm global}&=\Omega_s+\log(S_sN_s)\,\mathbb I_s,\\
 \Omega_{st}^{\rm global}&=-i\sum_{v\in s}
 \operatorname{Embed}_v\!\left(T_v^{-1}\widetilde\Omega_{0;v}T_v\right)
 \left(S_sR_{st}^{(v)}S_t^{-1}\right),\qquad t\prec s,
 \label{eq:tree-sector-offdiagonal}
\end{align}
其中 $R_{st}^{(v)}$ 由当前 loop \texttt{dtau} 的 tagged $R^{(1)}$ raw source 得到，并已按统一 $J$ 表示包含 $N_s/N_t$；只有 $S_s/S_t$ 在 tree dlog 层追加。该写法逐项实现 Eq.~(3.68)，且共同-theta多传播子 source 会自然给直接跨多层 sector 的块。$\log(S_sN_s)$ 作为整体 letter 保存；不把一般复幂拆开，也不使用 \texttt{PowerExpand}。

\texttt{DSTreeDLogDE[context]} 返回同序的 \texttt{masters}、\texttt{omega}、\texttt{letters/letterMatrices}，并附带每个 master 的 \texttt{masterNormalizationRecords}、contact 使用的 \texttt{dlogSelectorCoefficients}、\texttt{selectorAudits}、\texttt{contactMaps} 与 \texttt{omegaBlocks}。记录明确区分有理 contact selector、物理 sector prefactor 及二者的 complete normalization；naive/direct 两条路线均从同一记录取其对数导数。缺省 \texttt{standard} 路线信任同一 producer 的统一公开表示 stamp，不再全对象扫描 legacy/private token，也不构造 \texttt{dlogResidual}；显式 \texttt{AuditLevel->"full"} 才运行公开表示、\texttt{LinearSolve} residual 与 dlog 严格重构证书。矩阵按 contact DAG 排序；从 lower sector 回到 parent 的块及同层 sibling 块必须严格为零。
...
\end{tabular}
\end{center}
fixed/non-loop line 没有 $b/b_S$ 指标；上表对应的 zero point 改由 sector prefactor 保存。\texttt{shrinkBShift} 与 \texttt{shrinkZeroPointShift} 是由函数系统和 Wronskian 编译出的内部物理数据，公开 topology 不能覆盖。额外 normalization 通过 master/basis 选择处理，不建立第二套 shrink 输入入口。
