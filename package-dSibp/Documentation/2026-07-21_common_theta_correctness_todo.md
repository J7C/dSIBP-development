...
- [x] 把 full/shrunk pack pattern 作为 shrink set 的隐式、可逆编码：由该 pattern 派生并核验 `sectorKey`；不同 shrink set 即使 compact `aList` 相同也不得混合。
- [x] fixed/non-loop line 的 `b/bS` 不进入三槽 pack；每个 sector 按 root fixed-line 顺序保存结构化 `sectorPrefactorData`。
- [x] `sectorPrefactorData` 分别保存稳定 `parameterKeys`、当前输出坐标的 `parameterList`、`powerList/powerParts` 和常数 prefactor，不缓存已经乘开的 `parameter^power`。
- [x] 参数重定义后逐 fixed line 更新 `parameterList`；prefactor materializer、求导、scaling、serializer 和 time-only 转换只消费同一 metadata。
- [x] 对 normalized `J_s=N_s I_s`，验证 `ds` 含 `D[Log[N_s]] J_s`，跨 sector contact/derivative 含 `N_s/N_t`；atomic massless fixed 幂次不得内外重复。
- [x] fixed-line shrink 只由 source/target zero point 的选择决定吸收量；逐 transition 序列化 $B_s$、$B_t$、override 来源、compiled $(s,z)$ 和剩余 coefficient exponent $s+z-(B_t-B_s)$，所有 reduction/DE 路径共用。不得新增与 zero point 平行的整数吸收选项。
