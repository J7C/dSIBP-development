...
验证目录：`independent-validation/MadStree-v0.4-validation-01-massless-redundant-vs-quotient/`。

状态：`passed (24/24, fresh v0.4 rerun 2026-07-30)`。完整符号 intertwining、25/15 两套生产边界、批量输运、全部相关 massless sector 与物理 $q^2$ 关系均已执行。
...
执行结果：root line 顺序 `{em,ez}` 的实际 key 为 `{"11","01","10","00"}`；25/15 两套生产边界均由 v0.4 直接生成。目标投影差为约 29 位精度的零，全部 massless 关系最大残差为约 37 位精度的零，总 wall time `27.195 s`。详见 v0.4 T1 自动报告与 summary。

## T2 2411.03088 单顶点三 massive 的 `k0 -> infinity` 解

验证目录：`independent-validation/MadStree-v0.4-validation-02-paper2411-three-massive-vertex/`。

状态：`passed (12/12, fresh v0.4 rerun 2026-07-30)`。生产 Frobenius producer、独立论文级数、测试专用定义积分和 FlintNDE 输运已分路线执行。
...
执行结果：leading coefficients exact 相等；定义积分 oracle 最大相对差 `2.27e-45`，目标点输运与 28 阶独立级数最大相对差 `9.46e-23`，总 wall time `128.645 s`。详见 v0.4 T2 自动报告与 summary。

## T3 2411.03088 两顶点单条 `G++` 的五主积分解

验证目录：`independent-validation/MadStree-v0.4-validation-03-paper2411-two-vertex-gpp/`。

状态：`passed (18/18, fresh v0.4 rerun 2026-07-30)`。独立论文奇点系统与 MadStree 生产边界分别经 FlintNDE 输运，完整五维向量及 contact 分量均通过。
...
执行结果：完整 multivariate/singular connection、五个 leading vectors 和 coefficients residual 均 exact 为零；完整目标向量与 contact 分量均为约 48 位精度的零，总 wall time `9.246280 s`。详见 v0.4 T3 自动报告与 summary。

## T4 单顶点公式、低阶约化与 H/h 双向变换

验证目录：`independent-validation/MadStree-v0.4-validation-04-vertex-reduce-hh/`。

状态：`passed (15/15, fresh v0.4 rerun 2026-07-30)`。独立 oracle 只使用 2401 的局部二态方程、二进制张量顺序和一阶 time IBP，不读取开发测试中的 expected。
...
执行结果：单 sector top key 为 `"11"`；两个 convention 的 `4x4` 矩阵、低阶约化、master 排列及 H/h 双向变换均 exact 通过，总 wall time `1.555188 s`。详见 v0.4 T4 自动报告与 summary。

## T5 simultaneous contact、time-only 圈图与 normal-crossing

验证目录：`independent-validation/MadStree-v0.4-validation-05-simultaneous-cycle-chart/`。

状态：`passed (17/17, fresh v0.4 rerun 2026-07-30)`。独立 expected 来自共同-theta 恒等式、图 partition、root 顺序位串和严格 rank blow-up，不读取开发测试的计数或矩阵。
...
执行结果：triangle root 顺序 `{l12,l23,l31}` 的 sector keys 为 `{"111","011","101","110","001","010","100"}`；七 sector、九 transition、六张 strict chart 和 shifted-child dlog closure 全部通过，总 wall time `1.881676 s`。详见 v0.4 T5 自动报告与 summary。

## T6 MadStree--FlintNDE 保存点、调用目录与能力边界

验证目录：`independent-validation/MadStree-v0.4-validation-06-flintnde-adapter-capability/`。

状态：`not rerun for v0.4`。本任务验证 Wolfram 公开入口实际调用 FlintNDE，不用 Python 直调代替 MadStree adapter；它不依赖 sector key，本轮沿用功能边界但不建立 v0.4 验证目录或通过报告。
...
执行结果：本轮未执行。v0.3 基线的 `16/16`、最大相对差约 `1.09e-33` 与 `4.122 s` 仍见冻结报告，不写成 v0.4 fresh 结果。
