# MadStree v0.5 独立验证任务书

更新日期：2026-07-30。

本任务书单列于验证目录之外。每项验证的程序、结果和自动生成报告分别放入对应的带版本号目录；报告应复述本任务书中与该项有关的目标、输入、方法和通过条件，因此验证目录内不再复制任务书。前置能力未完成时保持 `pending`，不得把结构检查写成数值通过。本版本只因 time-only sector key 变化重跑 T1--T5；T6 的 FlintNDE 保存与能力边界不依赖该 key，本轮不重跑。
...
验证目录：`independent-validation/MadStree-v0.5-validation-01-massless-redundant-vs-quotient/`。

状态：`passed (24/24, fresh v0.5 rerun 2026-07-30)`。完整符号 intertwining、25/15 两套生产边界、批量输运、全部相关 massless sector 与物理 $q^2$ 关系均已执行。
...
- quotient 四个 sector 的 master 数依次为 `{8,2,4,1}`，总数 `15`；冗余四态的对应维数为 `{16,4,4,1}`，总数 `25`。root line 顺序固定为 `{em,ez}`，sector 顺序按定长位串 `{"11","01","10","00"}` 固定，报告保存 schema 与实际 key 以复核。
...
执行结果：root line 顺序 `{em,ez}` 的实际 key 为 `{"11","01","10","00"}`；25/15 两套生产边界均由 v0.5 直接生成。目标投影差为约 29 位精度的零，全部 massless 关系最大残差为约 37 位精度的零，总 wall time `31.142 s`。详见 v0.5 T1 自动报告与 summary。

## T2 2411.03088 单顶点三 massive 的 `k0 -> infinity` 解

验证目录：`independent-validation/MadStree-v0.5-validation-02-paper2411-three-massive-vertex/`。

状态：`passed (12/12, fresh v0.5 rerun 2026-07-30)`。生产 Frobenius producer、独立论文级数、测试专用定义积分和 FlintNDE 输运已分路线执行。
...
执行结果：leading coefficients exact 相等；定义积分 oracle 最大相对差 `2.27e-45`，目标点输运与 28 阶独立级数最大相对差 `9.46e-23`，总 wall time `161.428 s`。详见 v0.5 T2 自动报告与 summary。

## T3 2411.03088 两顶点单条 `G++` 的五主积分解

验证目录：`independent-validation/MadStree-v0.5-validation-03-paper2411-two-vertex-gpp/`。

状态：`passed (18/18, fresh v0.5 rerun 2026-07-30)`。独立论文奇点系统与 MadStree 生产边界分别经 FlintNDE 输运，完整五维向量及 contact 分量均通过。
...
执行结果：完整 multivariate/singular connection、五个 leading vectors 和 coefficients residual 均 exact 为零；完整目标向量与 contact 分量均为约 48 位精度的零，总 wall time `8.250435 s`。详见 v0.5 T3 自动报告与 summary。

## T4 单顶点公式、低阶约化与 H/h 双向变换

验证目录：`independent-validation/MadStree-v0.5-validation-04-vertex-reduce-hh/`。

状态：`passed (15/15, fresh v0.5 rerun 2026-07-30)`。独立 oracle 只使用 2401 的局部二态方程、二进制张量顺序和一阶 time IBP，不读取开发测试中的 expected。
...
- 只检查线性组合
  `2 MSIntegral[topKey,{1},{0,0}]-3 MSIntegral[topKey,{-1},{1,0}]`，其中单 sector 的 `topKey` 从 context 读取为全 `1` 位串。
...
执行结果：单 sector top key 为 `"11"`；两个 convention 的 `4x4` 矩阵、低阶约化、master 排列及 H/h 双向变换均 exact 通过，总 wall time `1.516580 s`。详见 v0.5 T4 自动报告与 summary。

## T5 simultaneous contact、time-only 圈图与 normal-crossing

验证目录：`independent-validation/MadStree-v0.5-validation-05-simultaneous-cycle-chart/`。

状态：`passed (17/17, fresh v0.5 rerun 2026-07-30)`。独立 expected 来自共同-theta 恒等式、图 partition、root 顺序位串和严格 rank blow-up，不读取开发测试的计数或矩阵。
...
执行结果：triangle root 顺序 `{l12,l23,l31}` 的 sector keys 为 `{"111","011","101","110","001","010","100"}`；七 sector、九 transition、六张 strict chart 和 shifted-child dlog closure 全部通过，总 wall time `1.858434 s`。详见 v0.5 T5 自动报告与 summary。

## T6 MadStree--FlintNDE 保存点、调用目录与能力边界

验证目录：`independent-validation/MadStree-v0.5-validation-06-flintnde-adapter-capability/`。

状态：`passed (16/16, fresh v0.5 rerun 2026-07-30)`。Wolfram 公开入口实际调用 v0.5 内置 FlintNDE；普通点、正则奇点保存、调用目录归属和超能力奇点 fail closed 均已执行。
...
执行结果：普通点三个保存值相对差最大约 `1.09e-33`；正则奇点保存 Frobenius 边界并继续到普通点，高阶不支持极点在 Python 前置能力检查后拒绝；总 wall time `3.113660 s`。详见 v0.5 T6 自动报告与 summary。
