...
FlintNDE 的位置只由 `$MSFlintNDERelativePath` 定义，v0.7 缺省为版本目录内的 `Vendor/FlintNDE`；`MSSetFlintNDERelativePath` 可显式覆盖。普通与奇点 JSON schema 分别为 `madstree_flintnde_transport_v1`、`madstree_flintnde_singular_transport_v1`。临时 JSON 写入调用脚本目录的 `results_temp/`；成功后删除。Python 使用默认 `__pycache__/` 并由 Git 忽略，版本源码目录不接收用户结果。`MSRuntimeDirectory` 的绝对路径直接展开，相对路径相对调用脚本目录解析；实现只使用单参数 `ExpandFileName`，兼容当前 Mathematica 内核。
...
```text
package-MadStree/versions/MadStree-v0.7/
  VERSION.md
  README.md
