...
- 在 v0.6 基础上修改建立（版本沿革 v0.5 -> v0.6 -> v0.7；v0.6 由冻结 v0.5 复制）。
- v0.6 源码、手册、examples 与验证结果作为基线保持不变；v0.7 仅在其上调整版本号、补充英文/引用提醒/示例与文档同步。

## 新增功能

- 版本根新增 `MadStree.m`；把 `versions/MadStree-v0.7/` 加入 `$Path` 后可直接调用 `Needs["MadStree`"]`。
...
- 新增 `MSBatchEvaluateTree`，多点批量求值入口：可复用同一自动边界或有限锚点（`AnchorPoint`/`AnchorValues`），支持 `Parallel` 并发执行；返回与 `MSFlintNDETransport` 相同的结果列表，可直接交给 `MSExportEvaluationData` 导出。
- 新增 `MSExportEvaluationData`，把多点数值求值结果（点坐标 + 各 master 实虚部 + status/relativeDifferenceInf/targetRelativeErrorMet）导出为 CSV 与 JSON，供外部绘图工具使用；`MSOutputDirectory` 缺省写入调用目录的 `results/madstree_evaluation/run-UUID/`。
...
- `MSFlintNDETransport` 的 singular-then-ordinary 组合后端现把 `relativeDifferenceInf`、`relativeDifferenceMidpoint`、`targetRelativeError`、`primarySeconds`、`referenceSeconds` 一并提升到 `flintNDE` 顶层，与纯 ordinary 边界输出同构；此前只有 `targetRelativeErrorMet` 在顶层，导致示例 01 在 `singularFrobenius` 边界下平铺访问 `flintNDE["relativeDifferenceInf"]` 返回 `Missing`。`singularLaunch`/`ordinaryTransport` 子结构不变，既有嵌套访问兼容。

## 迁移

- v0.6 的显式加载路径可改为把 v0.7 版本根加入 `$Path`；无需再把 `Kernel/` 直接加入 `$Path`。
...
- v0.7 在 v0.6 基础上建立（v0.6 由冻结 v0.5 复制）；v0.5 的全部 11 个开发 tests 和三个 examples 已串行 fresh 通过（历史证据）。T1--T6 独立验证分别通过 `24/24`、`12/12`、`18/18`、`15/15`、`17/17`、`16/16`；正式报告和机器 summary 均保存在版本化独立验证目录（v0.5）。T1 的 25 态与 15 态边界由 v0.5 分别直接生成；T6 由 Wolfram 公开入口调用内置 FlintNDE。
