...
from flintnde.transport import evaluate_vector_series, evaluate_vector_series_many
...
class FastMultipointEvaluationTest(unittest.TestCase):
    """检查节点 Taylor 多项式的批量求值与逐点 Horner 一致。"""

    @classmethod
    def setUpClass(cls) -> None:
        configure_working_precision(80, 32)

    def test_large_bucket_uses_fast_multipoint_evaluation(self) -> None:
        """大桶使用 FLINT 子积树/余数树并满足工作精度误差门槛。"""

        coefficients = [
            acb_mat([[acb(index + 1)], [acb(2 * index - 1)]])
            for index in range(20)
        ]
        points = [acb(index, index % 3) / acb(100) for index in range(25)]
        fast_values, algorithm = evaluate_vector_series_many(coefficients, points)
        self.assertEqual(algorithm, "fast")
        maximum_difference = max(
            float(abs(fast_values[index][row, 0] - horner[row, 0]).mid())
            for index, point in enumerate(points)
            for horner in [evaluate_vector_series(coefficients, point)]
            for row in range(2)
        )
        self.assertLess(maximum_difference, 1.0e-70)

    def test_small_bucket_uses_iterative_evaluation(self) -> None:
        """小桶避免子积树固定开销并与 Horner 的球结果相交。"""

        coefficients = [acb_mat([[acb(1)], [acb(2)]]), acb_mat([[acb(3)], [acb(4)]])]
        points = [acb("1/10"), acb("1/5")]
        values, algorithm = evaluate_vector_series_many(coefficients, points)
        self.assertEqual(algorithm, "iter")
        for index, point in enumerate(points):
            expected = evaluate_vector_series(coefficients, point)
            for row in range(2):
                self.assertTrue(abs(values[index][row, 0] - expected[row, 0]).contains(0))
...
class DenseOutputTest(unittest.TestCase):
    ...
    def test_refined_dense_results_use_primary_order(self) -> None:
        """双链可区分时，embedded/certified 都必须返回低阶主链值。"""

        system = AnalyticMatrixSystem(
            lambda _point: acb_mat([[1]]),
            1,
            singularities=(acb(10),),
            name="primary-dense-output",
        )
        path = [acb(0), acb("1/2")]
        sample_points = [acb("1/4")]
        primary = transport_path(
            system, column_vector([1]), path, order=2, sample_points=sample_points
        )[3]["sample_results"][0]["value"]
        reference = transport_path(
            system, column_vector([1]), path, order=8, sample_points=sample_points
        )[3]["sample_results"][0]["value"]
        self.assertGreater(
            float(relative_difference_inf(primary, reference).mid()), 1.0e-6
        )
        for mode in ("embedded", "certified"):
            result = transport_path_refined(
                system,
                column_vector([1]),
                path,
            ...
            returned = result["sample_results"][0]["value"]
            self.assertLess(
                float(relative_difference_inf(returned, primary).mid()), 1.0e-45
            )
            self.assertGreater(
                float(relative_difference_inf(returned, reference).mid()), 1.0e-6
            )
