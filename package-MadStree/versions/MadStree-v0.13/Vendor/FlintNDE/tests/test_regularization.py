...
from flintnde.regularization import (
    _parallel_input_records,
    _restore_parallel_inputs,
    _solve_sample,
)
...
class SeriesReconstructionTest(unittest.TestCase):
    ...
    def test_sample_solver_returns_primary_endpoint(self) -> None:
        """正规化拟合消费低阶生产值，高阶端点只参与 NDE 误差核验。"""

        fake_result = {
            "primary_snapshots": [acb_mat([[1]]), acb_mat([[2]])],
            "reference_snapshots": [acb_mat([[1]]), acb_mat([[3]])],
            "primary_segments": [],
            "reference_segments": [],
            "relative_difference_inf": arb(0),
            "primary_seconds": 0.1,
            "reference_seconds": 0.2,
        }
        with patch(
            "flintnde.regularization.transport_path_refined",
            return_value=fake_result,
        ):
            endpoint, report = _solve_sample(
                DEmatrix=zero_system(1),
                boundary=[acb(1)],
                path=[acb(0), acb(1)],
                sample_argument=fmpq(1, 10),
                sample_point=acb("1/10"),
                transport_order=4,
                transport_extra_order=2,
                transport_sample_count=None,
                transport_extra_sample_count=None,
                radius_fraction=0.6,
                nde_tolerance=arb("1e-10"),
            )
        self.assertEqual(endpoint[0, 0], acb(2))
        self.assertEqual(report["endpoint"][0], acb(2).str(50))
