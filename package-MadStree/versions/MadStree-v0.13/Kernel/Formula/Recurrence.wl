...
(***
File: Recurrence.wl
Purpose: Constructs the Eq. (3.65) contact maps and directly performs stepwise and full iterative reduction using the (-tau)^A formula.
Key boundary: contact always carries the target sector and the exact aShift; do not drop the shift first and then guess the subsector master.
***)
  ...
  Switch[line["type"],
    ...
    "masslessFull",
      If[
        ...
        2 I line["fullContourSign"] complementCoefficient,
...
(* 返回 contact 所属连通分量唯一的 SK 轮廓权重；外腿指数符号不得进入此处。 *)
msContactComponentContourSign[
  sourceSector_Association,
  componentPosition_Integer,
  context_?MSContextQ
] := Module[{rootVertices, signs},
  rootVertices = sourceSector["vertexComponents"][[componentPosition]];
  signs = DeleteDuplicates@Lookup[
    Select[context["vertices"], MemberQ[rootVertices, #["id"]] &],
    "contourSign",
    {}
  ];
  If[
    ...
    First[signs],
    Failure[
      "MixedVertexTypeContactComponent",
      <|"sectorKey" -> sourceSector["sectorKey"],
        "componentPosition" -> componentPosition, "contourSigns" -> signs|>
    ]
  ]
];


(* 统一 Eq. (3.68) 与 recurrence 的 contact phase：纯 massive 读取轮廓支，含 massless 时读取 quotient/Wronskian 相位。 *)
msContactPhase[
  sourceSector_Association,
  contactMap_Association,
  componentPosition_Integer,
  context_?MSContextQ
] := Module[{componentContourSign, masslessCount},
  componentContourSign = msContactComponentContourSign[
    sourceSector,
    componentPosition,
    context
  ];
  If[Head[componentContourSign] === Failure, Return[componentContourSign]];
  masslessCount = Count[contactMap["lineTypes"], "masslessFull"];
  If[masslessCount === 0, componentContourSign, (-1)^masslessCount]
];
...
msRemainingVector[
  ...
  componentPosition_Integer,
  context_?MSContextQ
] := Module[{result, maps, map, matrix, targetSector, targetShift, targetVector, contactPhase},
  ...
  Do[
    ...
    contactPhase = msContactPhase[sector, map, componentPosition, context];
    If[Head[contactPhase] === Failure, Return[contactPhase]];
    (* M0/M1 递推式已有自身的 contact 符号；这里的 -phase 只把 raw R 矩阵对齐到论文 dlog contact convention。 *)
    result += -contactPhase matrix.targetVector,
    {contactMap, maps}
  ];
  result
];
