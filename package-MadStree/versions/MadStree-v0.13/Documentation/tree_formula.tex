...
缺省调用仍不含任何 regulator 数值点。若用户需要限制全部取值范围，可用
\texttt{EpSamplePoints -> \{...\}} 提供有序生产候选池，并以
\texttt{EpValidationPoints -> \{...\}} 提供与整个生产池分离的独立验证点。候选池可以冗余；
\texttt{EpInitialInternalMaximumPower -> $q$} 指定首轮内部最高幂后，首轮只消费拟合
