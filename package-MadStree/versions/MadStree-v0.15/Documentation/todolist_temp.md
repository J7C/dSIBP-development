...
- [x] N1. 建立 v0.15 并同步版本目录、loader、索引、任务书和更新说明。
...
- [x] P1. 明确单点和多点共用 `MSEvaluatePath[context,pointSequence]`；v0.15 的单点是同一
...
以下是 v0.13 已完成并由 v0.15 继承的拓扑合同：tensor formula、sector DAG、dlog DE、
...
- [x] M7. 验收后删除 v0.12 目录和旧版本验证资产；清空中间产物后只从 v0.15 路径 fresh 重跑。
...
three-vertex `8/8`。该修复使 022-v0.15 的 15x15 比较暴露 MadStree 自身 recurrence 与 dlog
...
- [x] M12. 修复 vertex-family `exponentType` 复用 `msVertexTypeSign` 的语义串线。v0.15 的
...
M7 完成记录（2026-08-19）：v0.15 的开发回归、六个 examples、三项当前独立验证及 Vendor
FlintNDE `170/170` 均已从新路径 fresh 通过；随后物理删除未跟踪旧版本 `MadStree-v0.12`
（98 文件，2,528,713 bytes），并删除 v0.15 的 `results_temp`、隔离失败运行树、
`test/results_test`、example runtime 与 Python cache。当前 `versions/` 只保留 v0.15；正式独立
...
6. 先运行 topology/schema/相位最小回归，再逐个 fresh 运行六个 examples、正规化和三个当前
   独立验证。v0.15 验收完成后才删除 v0.12，且现行 loader 只指向 v0.15。
# 解析公式先落盘补充门禁
