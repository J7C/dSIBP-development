...
\begin{abstract}
本手册对应 \texttt{package-MadStree} v0.15；time-only sector 继续由 root propagator 顺序的定长字符串标识。
...
\end{align}
$\cN_s$ 是 sector normalization，$\Phi_s$ 是已初始化的 massive-contact phase，$\Xi_{s,\bm a}$ 收集 shared-massless chamber sign 与 RedundantH normalization，$E_{sC,\bm a}$ 是该 component 全部 Hankel endpoint coefficients 的乘积。所有量都从当前 sector metadata 取得，不能从 root sector 猜 child 的幂次或二态因子。
