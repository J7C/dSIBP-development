...
- 独立包与 MadStree v0.15 的 `Vendor/FlintNDE` 保持同一份实现和 0.5.0 版本身份。
...
以下能力说明为从 0.3.0 继承且在 0.5.0 继续保留的基线。
...
- 独立包与 MadStree v0.15 Vendor 的 42 个非缓存交付文件逐文件 SHA-256 一致。
...
- 当前共享 regulator、transport、singularity 与根 `__all__` 不变；同一改动已逐字节同步到
  MadStree v0.15 Vendor。独立包和 Vendor Python 各 `170/170`，Wolfram 端到端 `25/25`，
