...
当前版本：`v0.15`。本目录从 v0.13 升级；接口变化见 `UPDATE_NOTES.md`。
...
FlintNDE 0.5.0 数值后端随 v0.15 放在版本目录内，缺省位置只在一个相对路径变量中定义：
...
七个 examples 已在删除既有 `results/` 与 `results_temp/` 后从 v0.15 路径全部 fresh 运行并退出 `0`；
...
v0.15 当前 fresh 回归包括 core `61/61`、公式 artifact `24/24`、点序列/规划 `27/27`、二维
900 点复平面分组 `8/8`、
单顶点 NDE `13/13`、runtime/export `10/10`、辅助外腿 `6/6` 及 Examples 01--07 `7/7`。v0.15 独立验证任务书
已同步新接口，但本轮未执行独立验证；旧版本报告和结果已删除，不作为 v0.15 证据。
