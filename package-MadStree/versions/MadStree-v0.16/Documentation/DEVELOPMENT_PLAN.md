...
本版只修复 massive contact/pinch sector 的物理边界 coefficient，并把三顶点 V5.5 八个 SK
分支升级为正式独立检验。`J_s=calN_s I_s`、sector normalization、master 顺序、contact
recurrence、dlog DE 和 FlintNDE 0.5.0 本体不因该物理修复改变。

v0.16 是唯一现行版本，不读取或转发 v0.15 路径、版本字符串、函数、参数、schema 或结果。
验收后 v0.15 只从 Git 历史恢复。

### 0.2 参考资料

- MadStree 公开定义：`MSIntegralDefinition`、`MSMasterIntegrals`、`MSDLogDE`、
  `MSBoundaryData`。
- 论文定义积分：2411.03088 Eq. (4.2)；Eq. (4.11) 只作为相位差诊断。
- 外部三顶点实现：`reference/ref_code_3vertexanalyticcheck/001_dsde3vertex_v5.5.wl`
  及其 Python companion，正式 reference 原件只读并记录 SHA-256。
- arXiv:2309.10849v2 Eqs. (103)--(110)、(141)--(143)；只认证原文实际
  给出的 SK/time-order 分支，不外推为四分支 oracle。

### 0.3 数据流

```text
MadStree v0.16 topology
  -> ordered masters + normalization
  -> exact dlog DE
  -> corrected Frobenius boundary
  -> frozen MadStree payload ----\
                                +-> same FlintNDE -> low-order result
V5.5 isolated producer          |                  + high-order refinement
  -> ordered masters + normalization
  -> exact dlog DE
  -> per-SK E2-infinity boundary
  -> frozen V5.5 payload -------/

arXiv:2309.10849v2 Eq. (103) -> V5.5 eight-branch total gate
```

V5.5 与 MadStree 都直接计算八个 SK 分支；八支各自完成 master/normalization、完整 DE、全部
边界 branch 和普通点结果比较。原始 V5.5 八支总和先通过 Eq. (103)，才作为 MadStree 的逐支
reference；额外乘 `Exp[Pi Im[nu]]` 的路线只作必须失败的反事实。
普通点只作最后旁证，不能反推 basis、normalization、Wick 方向或额外相位。

### 0.4 检查点与输出

1. 两顶点 single pinch 与三顶点 double pinch 的边界 coefficient 做 exact 回归；故意恢复
   `I^n` 时必须失败。
2. Validation-04 fresh 重跑，确认两顶点论文 DE、五支边界和三路线普通点仍在预设误差预算内。
3. Validation-07 每支保存完整 DE 差计数、边界逐字段 residual、冻结 payload digest、实际
   FlintNDE 路径/节点、低高阶差、wall time 和逐分量联合误差预算。
4. 正式轻量证据只进 validation `results/`；V5.5 隔离副本、backend JSON、cache 和日志只进
   `results_temp/`，不反向成为生产输入。
5. 结合实际 loader/caller、Wolfram 定义和 Python AST 审计旧入口与死代码；随后完成
   UTF-8、Wolfram 分节、PDF、生成物和 `git diff --check` 门禁。
...
- 内部派生类型 `masslessFull` 不是外腿指数。缺省 `masslessRepresentation="Quotient"` 把两个端点四态 quotient 成整条边共享的二维 slot；显式 `"RedundantH"` 则保留两个 h endpoint 的四态表示。该选择只在初始化时给出并固定在 context。
...
- `massiveFull` 有两个独立 endpoint h slots 且可 contact；`masslessFull` 缺省为整条边共享的二维 quotient slot，也可选两个 `masslessEndpointH` slots 的冗余四态；
...
```text
Exp[I vertexSign[v] k0User[v] tau[v]],
vertexSign[v] in {+1,-1}.
```
...
```text
K[v] = I vertexSign[v] k0User[v] > 0,
k0User[v] = -I vertexSign[v] K[v].
```
...
该证明只讨论“所有 `K -> infinity` 同时发生”造成的退化。完全不含 `K` 的奇点已经显式列在现有 DE letters 中，不是边界 producer 的待解决障碍：缺省 `Automatic` 忠实处理用户显式给出的有限奇点，由 FlintNDE 局部幂对数基返回有限值或文本 `Infinity`，并在同一直线分支上覆盖收敛域内双侧用户点。程序不替用户猜测奇点处非共线转向、一般 blow-up 或额外 gauge transformation 引入的新分母。
...
若目标点落在 `M0=0`，普通递推和普通点 Taylor 输运停止；先在 generic `k0` 完成解析约化，再把已闭合 DE 交给 FlintNDE 局部奇点路线。只有局部基通过能力门禁且分支由同一直线来向/出射方向确定时，才返回受控有限值或 `Infinity`；否则 fail closed。
...
4. 从 chart 内的小有限点出发，先到 preferred point（若提供），再按用户 `pointSequence` 分段；普通段由 FlintNDE 规划，显式奇点点由局部基处理；
5. 若某些顶点能量是辅助变量，优先逐个移除，并在每一步重做全 sector 普通点检查；只有检查通过才允许以普通点 DE 到达零；
6. 一旦零点是有限奇点，不把它交给普通点 Taylor 路线；`Automatic` 使用已认证局部基分类并输出。奇点处非共线转向、连续奇点或超出当前局部能力时返回对应 fail-closed 状态。
...
- `MSEvaluatePath[context,pointSequence,ParameterRules->{...}]`：`pointSequence` 首行为有序可跑动
  坐标符号，后续为等宽值行；固定参数只在 `ParameterRules` 给一次。单点只有一行值，多点追加
  值行；识别最大连续复仿射单变量段、逐段拉回，并在一次请求中委托 FlintNDE 规划与输运；
- `MSReconstructEpSeries[context,ep,pointSequence,MaximumEpPower->m]`：复用同一坐标表与
  `ParameterRules` 合同，用户只给所需最高幂；程序
...
任何经过中途节点的多点输运都称为折跃；只有穿过奇点并使用局部基连接两侧匹配点时才称为奇点折跃。`SingularityMode -> "Automatic"` 是 MadStree 缺省：显式奇点用户点不被静默绕开，同一局部基覆盖收敛域内双侧点并从出射普通点继续；末端奇点必要时使用由目标收敛半径决定的隐藏匹配点。`"Avoid"` 显式拒绝，`"SingularityJump"` 显式选择穿越分支并提示确认多值性。独立 FlintNDE 的 Python/Wolfram 低层接口缺省仍为 `avoid`/`"Avoid"`。`MessageLanguage` 缺省 `"EN"`，可选 `"CN"`；两个选项都严格拒绝其它值和宽松大小写。

普通值行缺省保存，临时途经点写作 `{{values...},"tmp"}`。执行结果携带坐标、数值、状态和
用户序号；普通保存点由 `MSExportEvaluationData` 导出。表头和 `ParameterRules` 必须互斥并
覆盖解析 DE 与边界所需全部符号，否则在 Python/NDE 启动前返回缺失或非数值符号。

解析 `MSDLogDE[context]` 是正式用户结果，`MSWriteFormulaArtifacts` 缺省把同序 masters、
recurrence metadata、完整解析 dlog DE 与 manifest 写入
`results/madstree_formula/run-<UUID>/`。两个公开数值入口必须在启动 Python/FlintNDE 前调用
同一内部门禁：为同一 context/调用根写出或复用完整的四文件资产，失败即停止，并在返回值
`"formulaArtifacts"` 中给出正式路径。数值参数只生成函数内局部副本，不得覆盖解析对象或
`dlog_de.wl`，也不得改变后端请求 schema。普通数值导出缺省写入 `results/madstree_evaluation/run-<UUID>/`；
`results_temp/` 和 `test/results_test/` 分别只保存运行中间物和开发测试产物。
...
`TransportOrder` 主链是节点、dense 点、段末值和正规化拟合的生产结果；更高的
`ReferenceTransportOrder` 链只用于相对差精度门禁。正则奇点边界初始化及相邻复仿射段均分别
...
```text
package-MadStree/versions/MadStree-v0.16/
  VERSION.md
  README.md
...
- [x] 集中配置版本内 `Vendor/FlintNDE` 0.5.0 相对路径，并完成 exact $\mathbb Q(i)(s)$ 序列化、普通点检查、adaptive path 和 refinement 输运。
...
结构检查之外，当前版本独立数值验收统一登记在
`../../../independent-validation-task/MadStree-v0.16-independent-validation-task.md`。validation-01
比较 900 个 exact 用户点的自动规划/fast multipoint 与严格用户节点路线；validation-02 检查
...
- 辅助 `k0=0` 若为当前已认证有限奇点，可由局部基分类并返回有限值或 `Infinity`；一般 blow-up、连续奇点和非共线奇点转向仍不自动选择；
