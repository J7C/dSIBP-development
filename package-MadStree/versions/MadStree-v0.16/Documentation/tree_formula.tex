...
\title{MadStree v0.16: tree formula}
...
\begin{abstract}
本手册对应 \texttt{package-MadStree} v0.16；time-only sector 继续由 root propagator 顺序的定长字符串标识。
...
\section{v0.16 拓扑输入约定}

每个顶点必须用 \texttt{vertexType -> "+"|"-"} 指定 Schwinger--Keldysh 轮廓支。
传播子不再重复输入 \texttt{skType}、\texttt{sigma} 或端点符号；公开
\texttt{type} 只接受 \texttt{"massive"} 或 \texttt{"massless"}。例如三顶点链写成
\begin{verbatim}
<|
 "vertices" -> {
...
|>
\end{verbatim}
程序从有序端点的 \texttt{vertexType} 自动派生 Full/Cross/External、
\texttt{++/--/+-/-+}、Full 整体符号、端点符号和缺省 Hankel branches。公式中的
\texttt{masslessFull}、$\sigma_e$ 等名称是归一化后的内部物理量，不是重复的用户字段。
顶点 ID 可为整数。传播子公开接口不定义或读取 ID，而始终按 \texttt{lines} 输入位置内部编号；只有
显式 \texttt{thetaBundles} 需要引用传播子时才使用这一位置编号。

字段 \texttt{externalLegEnergy} 表示附着在顶点 $v$ 的外腿无 theta 指数参数，不是
“顶点自身具有的能量”。\texttt{vertexType=+} 给 $\exp(-\ii k_v\tau_v)$，
\texttt{vertexType=-} 给 $\exp(+\ii k_v\tau_v)$。SK contour sign 与该指数符号在内部
分开保存，但都由同一个 \texttt{vertexType} 唯一派生。没有物理外腿时可省略
\texttt{externalLegEnergy} 或显式给 0；拓扑内部均归一化为 0，边界 producer 则使用私有
辅助阻尼能量建立无穷远边界，再把该辅助坐标输运到用户给定的零点。

\subsection{加载与引用提醒}

标准 Wolfram 入口在全部模块成功加载后，每个 kernel 只显示一次引用提醒；Notebook 中的
arXiv 号是可点击链接，headless kernel 保留完整 URL。Python 文件协议不向 JSON stdout
混入文本提醒，而在响应 metadata 保存同一 MadStree 引用清单；直接运行适配器 CLI 时显示
完整文本。使用 MadStree 开展研究时，建议引用：
\begin{enumerate}
...
\end{equation}
其中 $q_b$ 保留用户输入的名字和定义。顶点所附无 theta 指数参数、massless cross/external 分支都可以是这种 block；同一个顶点可以关联多个 $b$. 把 $E_b$ 视为一维空间 $\cE_b\simeq\mathbb C$ 上的向量，其 time generator 是
\begin{equation}
 Q_b=[\ii\chi_bq_b],
 \qquad I_{\cE_b}=[1].
...
\end{equation}
一维 blocks 可以在 $M_0$ 中聚合成 $p_{0,v}$，但不能从 registry 删除，因为用户仍按各自的传播子模长和顶点所附指数参数输入数值点与微分变量。massless full edge 的同一个编号 $e$ 同时出现在两个顶点的 list 中；这正是“共享而非复制”的数据表达。
...
固定 sector $s$，令 $n_s^{\mathrm{slot}}$ 是该 sector 的二态因子数，$c_s$ 是 contact 后的 time components 数。沿用 2401 的二进制表示，把 state 写成
\begin{equation}
...
\end{equation}
第一位是最高有效位；Kronecker 积中左侧二态因子慢变、右侧快变。$\operatorname{Del}_i\bm a$
删除第 $i$ 位，$\operatorname{Ins}_i(\widehat{\bm c},x)$ 在该位置补入 $x\in\{0,1\}$；
这些操作与参考代码的二进制顺序完全一致。
...
\end{equation}
不能发生 contact shrink 的传播子恒有 $q_j=1$，top sector 为全 $1$ 字符串。该 key 的类型是字符串，不是二进制整数；前导零不可删除。例如 $E=(e_1,e_2,e_3,e_4)$ 且 $L_s^{\rm ctr}=\{e_1,e_2,e_4\}$ 时，key 是 \texttt{"0010"}。完整积分头始终把该 key 放在第一参数。因此两个 subsectors 即使 $c_s,n_s^{\mathrm{slot}},\bm n,\bm a$ 全部相同，也仍是不同的
...
前文使用论文的 $\tau^{\nu_0}$ convention。package 中参与递推、DE 与数值输运的积分对象是 normalized object
\begin{equation}
 J_s=\cN_sI_s,
\end{equation}
其中同一组现有指标下的裸积分紧凑定义为
...
\end{equation}
$s=\texttt{sectorKey}$，$\bm n=\texttt{aShifts}$，$\bm a=\texttt{stateBits}$；
$\mathcal H_{s,\bm a}$ 是前文已定义的 active line 二态 building blocks 的有序乘积。
massive endpoint 与 \texttt{masslessEndpointH} 读取各自状态位，massless shared quotient
读取整条边共享的一位；Full 的 $++/--$ time ordering 保留在相应 building block 中，已收缩线不再出现。
程序不逐个 master 展开这一乘积。
...
\end{verbatim}
它返回同指标的裸积分标签、精确 $\cN_s$ 及惰性等式；
\texttt{MSMasterIntegrals[context]} 的每条同序记录直接附带这些字段。$\cN_s=1$ 时显示
$J_s(\bm n;\bm a)=I_s(\bm n;\bm a)$。
...
\texttt{externalLegEnergy/}\allowbreak\texttt{timePower/}\allowbreak\texttt{hBlocks/}\allowbreak\texttt{exponentialBlocks}。每个纯指数 block 是一维因子，只平移
effective external-leg energy；每个 h block 是二维 massive endpoint slot。该 context 可直接调用
...
|>];
\end{verbatim}
\texttt{vertexType} 决定整个顶点的 SK 轮廓支，\texttt{exponentType} 只决定相应额外指数
block 在 effective external-leg energy 中的字面正负号。单顶点 family 没有传播子 incidence，
所以其初始化 schema 与树图的 \texttt{vertices/lines} 有意不同；context 建立后，两者共用同一
master、递推、DE、边界、数值选项和返回结构。
...
上述定理也给出了自动化范围。完全不含任何 $K_v$ 的奇点已经显式列在 DE letters 中，不是这个边界 producer 的待解决障碍。普通折线可用 \texttt{"Avoid"} 拒绝穿越；\texttt{"Automatic"} 把用户显式给出的有限奇点交给后端局部幂对数基，分类有限极限、pole 与 log 发散，并用同一局部基覆盖收敛域内双侧用户点。更一般的 blow-up、奇点处非共线转向及额外 gauge transformation 引入的新分母仍不自动继承上述证明。
...
若目标点本身落在 $M_0=0$，则必须先在 generic 顶点所附指数参数下约化再取受控极限，或由用户基于已输出 DE 另行处理该奇点，不能把零点交给普通点数值输运。

因此推荐流程是：先按式 \eqref{eq:preferred-rank-from-point} 选择首选 chart，从式 \eqref{eq:nested-blowup-coordinates} 的边界得到同序向量，在 chart 内取一个小而有限的普通点，再由用户按所需同伦类给出到 preferred 普通点和最终目标点的多变量折线。\texttt{"Avoid"} 报告穿过 letter hypersurface 的问题点对；\texttt{"Automatic"} 对显式奇点用户点使用局部基分类并输出，且在同一直线分支上从出射普通匹配点继续；\texttt{"SingularityJump"} 显式选择穿越分支并要求确认多值性。辅助顶点能量可逐个取零；若零点是有限奇点，则返回有限值或 \texttt{Infinity}。若奇点位于复仿射段转向处，则输入没有唯一指定延拓分支，程序仍 fail closed。

\subsection{当前 2411 无穷远边界与 FlintNDE 输运}

生产入口不在有限 Euclidean 普通点直接计算定义积分。\texttt{MSBoundaryData} 以文献 \cite{Chen:2024jms} 的 $K_v\to\infty$ 数据为唯一边界来源。若某个顶点没有物理外腿，程序在同一解析 dlog 系统中内部引入私有辅助外腿能量 $k_{0,v}^{\rm aux}$，先从对应 $K_v^{\rm aux}\to\infty$ 的阻尼极限构造边界数据，再由 FlintNDE 沿该辅助坐标输运到物理目标 $k_{0,v}^{\rm aux}=0$；用户无需在 \texttt{pointSequence} 或 \texttt{ParameterRules} 中输入该私有坐标。包括单顶点 massive-external family 在内，所有已闭合 tree/time-only context 都统一消费 sector DAG、component、slot registry、normalization、state order 与 strict rank，并走“边界领头项 + FlintNDE Frobenius 递推”路线。式 (3.44)--(3.46) 的单顶点显式多变量级数只作为测试 oracle，不进入生产分派。
...
\end{align}
$\cN_s$ 是 sector normalization，$\Xi_{s,\bm a}$ 收集 shared-massless chamber sign 与 RedundantH normalization，$E_{sC,\bm a}$ 是该 component 全部 Hankel endpoint coefficients 的乘积。每条被收缩的 massive Full line $e$ 给 $\cN_s$ 贡献
\begin{equation}
 -\frac{4\ii}{\pi}\eta_e k_e^{-2\nu_e^{\rm formula}-1},
 \qquad
 \eta_e=\begin{cases}+1,&++\\-1,&--\end{cases},
 \label{eq:massive-full-pinch-normalization}
\end{equation}
其中 $\eta_e$ 只定义物理 child master 的 normalization；它不再乘入 normalized-master dlog contact block 或 recurrence event。massive contact 的其余常系数已完整包含在各 component 的定义积分系数中，不能再按收缩深度额外乘相位。所有量都从当前 sector metadata 取得，不能从 root sector 猜 child 的幂次或二态因子。
...
对两顶点等时间幂 $\nu_0$ 的负 prefactor convention，2411 式 (4.2) 在论文自身的 endpoint basis 中定义
\begin{equation}
 I_R^{\rm paper}=-\frac{4\ii}{\pi}e^{\pi\operatorname{Im}\nu_1}k_s^{-2\nu_1-1}
 \int_{-\infty}^{0}\dd\tau\,(-\tau)^{2\nu_0-2\nu_1}
 e^{\ii(k_{12}+k_{34})\tau}.
 \label{eq:paper-two-vertex-child-normalization}
\end{equation}
这个式子不能不经换基直接抄成 MadStree 的 sector normalization。对物理阶 $\nu_1=\ii\mu$，论文的共轭阶端点与 MadStree 两端共同阶之间满足
\begin{equation}
 e^{-\pi\mu}H^{(2)}_{-\ii\mu}(z)=H^{(2)}_{\ii\mu}(z).
 \label{eq:paper-endpoint-to-common-hankel-order}
\end{equation}
因此式 \eqref{eq:paper-two-vertex-child-normalization} 的显式 Boltzmann 因子已经进入 endpoint basis 变换；该例是 $G_{++}$，所以式 \eqref{eq:massive-full-pinch-normalization} 取 $\eta_e=+1$，在 MadStree 的共同 Hankel 阶 basis 中 child normalization 是
\begin{equation}
 \cN_R^{\rm MS}=-\frac{4\ii}{\pi}k_s^{-2\nu_1-1},
 \label{eq:madstree-common-order-child-normalization}
\end{equation}
不能再乘 $e^{\pi\operatorname{Im}\nu_1}$。同一第五分支的 leading vector，以及在两种 basis 中对应的物理系数分别为
\begin{align}
 v_5&=\left(0,\frac{1}{2\nu_1-\nu_0},\frac{1}{\nu_0+1},0,1\right)^T,
 \notag\\
 C_5^{\rm paper}&=-\frac{4\ii}{\pi}e^{\pi\operatorname{Im}\nu_1}
 k_s^{-2\nu_1-1}e^{-\ii\pi(2\nu_0-2\nu_1+1)/2}
 \Gamma(2\nu_0-2\nu_1+1),
 \notag\\
 C_5^{\rm MS}&=-\frac{4\ii}{\pi}
 k_s^{-2\nu_1-1}e^{-\ii\pi(2\nu_0-2\nu_1+1)/2}
 \Gamma(2\nu_0-2\nu_1+1).
 \label{eq:paper-two-vertex-contact-boundary}
\end{align}
这里使用 $\int_{-\infty}^{0}\dd\tau\,(-\tau)^p e^{\ii K\tau}=(-\ii)^{p+1}\Gamma(p+1)K^{-p-1}$ 的 $i\epsilon$ 分支。论文印刷式 (4.11) 的相位 $e^{\ii\pi(\nu_1-\nu_0)}$ 相对式 (4.2) 的这个直接积分多一个 $\ii$；MadStree 按公开定义 $J_s=\cN_s I_s$ 采用直接积分，并在和论文比较前显式化到同一 endpoint basis。正 prefactor context 只把这些公式中的论文参数统一换成 \texttt{formulaNu}$=-|\nu|$。上述五维结果现在只是式 \eqref{eq:generic-sector-boundary-component}--\eqref{eq:generic-sector-indicial-vector} 的一个检查例，不是生产分派；一般不等 time powers 直接读取各 component 自身的 $A_{sC}$。实正物理动量的 massive pinch normalization 固定为式 \eqref{eq:madstree-common-order-child-normalization}，不能把形式符号 $(-k_s)^{-2\nu_1-1}$ 交给 Mathematica 主支。
...
第二个参数始终叫 \texttt{pointSequence}。其首行是有序可跑动坐标符号，后续是等宽坐标
值行；单点只有一行值，多点只追加值行。固定、不可偏导参数只在
\texttt{ParameterRules} 中一次给出。表头与固定参数必须互斥，且二者必须覆盖解析 DE 与边界
需要的全部符号；缺失或替换后仍非数值时，在建立 runtime 与启动 Python/FlintNDE 前返回
列出问题符号的 Failure。函数、选项、后端 schema、\texttt{pointResults} 和返回结构不因点数改变。
其中 \texttt{TransportOrder} 给出生产主链阶数，节点、dense 点、段末值和正规化拟合均取这条
低阶链；更高的 \texttt{ReferenceTransportOrder} 只用于相对差精度核验。正则奇点初始化与
后续各复仿射段分别传播两条链的状态，参考链末点不替换用户结果，也不重启下一段主链。
不同段不共享局部系数，因而没有构造多变量高维 Taylor 球。后端位置由相对项目根目录的单一配置
变量控制。\texttt{MSRuntimeDirectory} 表示临时运行根本身；\texttt{Automatic} 只在调用脚本旁
建立一次 \texttt{results\_temp/}，内部短目录为 \texttt{nde/} 和 \texttt{cache/}，不写 package
源码目录。Windows 下在创建目录和启动 Python 前检查完整 cache、input 与 log 路径；超过 259
字符时单独返回 \texttt{RuntimePathTooLong}。输入写入、缺少 \texttt{python-flint}、启动失败、
无输出和损坏输出使用不同 Failure 标签。普通点 CSV/JSON 导出只有在全部请求格式写出成功时才
返回 \texttt{"written"}。

解析 \texttt{MSDLogDE[context]} 本身是正式用户结果。纯符号工作流可显式调用
\texttt{MSWriteFormula\allowbreak Artifacts}；\texttt{MSEvaluatePath} 与
\texttt{MSReconstructEp\allowbreak Series} 则在启动任何 Python/FlintNDE 数值任务前自动写出或复用
同一 context/调用根的正式解析资产。写出失败时数值 NDE 不启动，成功返回值的
\texttt{"formula\allowbreak Artifacts"} 给出全部实际路径。缺省在调用脚本旁写出
\path{results/madstree_formula/run-<UUID>/masters.wl}、
\texttt{recurrence\_metadata.wl}、\texttt{dlog\_de.wl} 和 \texttt{manifest.wl}。
其中 \texttt{dlog\_de.wl} 始终是未代数值的完整解析 DE；数值阶段只在求值函数内部派生一份
参数化副本，不覆盖解析对象或正式文件。同一会话中重复求值复用仍完整存在的四个文件；若文件
被删除则下次调用重建，任一文件未写成时不返回成功。普通点导出缺省写入
\path{results/madstree_evaluation/run-<UUID>/evaluation_data.csv/json}。
\texttt{results/} 只保存用户正式结果，\texttt{results\_temp/} 只保存请求、日志与 cache，
\texttt{test/results\_test/} 只保存开发测试产物。

任何经过中途节点的多点输运都称为折跃；只有穿过奇点并用局部基连接两侧匹配点时才称为
奇点折跃。缺省 \texttt{SingularityMode -> "Automatic"} 忠实处理用户显式给出的奇点并
提示局部支风险：精确奇点不作为普通节点，同一局部基按目标奇点到最近其它奇点的距离覆盖
收敛域内双侧用户点，可去奇点返回数值，真实 pole 或零阶 log 发散分量返回文本
\texttt{Infinity}，并在 \texttt{singularityClassifications} 单列证据；随后从出射普通点继续。
若奇点是最终用户点且当前状态在局部圆外，FlintNDE 沿来向插入一个域内隐藏普通匹配点。
低阶 \texttt{TransportOrder} 链给出结果，高阶 \texttt{ReferenceTransportOrder} 链只核验
分类一致性和有限值误差。\texttt{"Avoid"} 显式拒绝穿越奇点；\texttt{"SingularityJump"}
显式选择奇点折跃分支，并提示用户确认等价绕行类的多值性。奇点处非共线转向、连续奇点和
关闭规划后的中间奇点仍 fail closed。\texttt{MessageLanguage -> "EN"} 缺省给出英文提示，设为 \texttt{"CN"} 时给出
中文提示；两个值严格区分大小写。每段有限奇点由后端从拉回后的单变量系统内部识别，用户无需
输入奇点列表。
...
并复用所有既有生产点和独立验证点。最多三轮仍未通过时保留当前最佳系数，但返回
\texttt{computed\_with\_warning}、\texttt{precisionTargetMet -> False} 和结构化原因；程序不降低容差、不改用最小二乘，
也不从头重算缓存。不同 $\epsilon$ 任务由 \texttt{ParallelTaskCount -> 12} 的缺省外层进程池
调度；实际并发取任务数与上限的较小者。

缺省调用仍不含任何 regulator 数值点。若用户需要限制全部取值范围，
\texttt{EpSamplePoints} 提供有序生产候选池，\texttt{EpValidationPoints} 提供独立验证点；
验证点须与整个生产池分离。
候选池可以冗余。

\texttt{EpInitialInternalMaximumPower} 指定首轮内部最高幂 $q$；首轮只消费拟合
$n_{\min},\ldots,q$ 所需的 $q-n_{\min}+1$ 个候选点，失败扩阶才顺序消费后续点。候选耗尽时
不生成池外点，当前系数随 \texttt{candidate\_pool\_exhausted} 警告返回。

用户也可设置 \texttt{EpSampleAngleRange -> \{thetaMin,thetaMax\}}，以弧度给出复角开区间。
程序在区间内部均匀选择最多三条射线，不取边界；各点模长仍由上述精度公式决定，没有模长
上限参数。自动验证点保持对应角度，只缩小模长。为保持本包 exact dlog pullback，后端把
高精度生成的实部、虚部分别转成 exact rational，形成 $\mathbb Q(i)$ 中的样本后再交回 MadStree。
该选项与显式 \texttt{EpSamplePoints} 互斥，缺省 \texttt{Automatic} 不改变原正实轴 schema。
相关选项缺省均为 \texttt{Automatic}，因此原缺省调用和 adapter JSON 字段集合不变。

普通坐标值行缺省保存；临时途经点写作
\texttt{\{\{value1,value2,...\},}\allowbreak\texttt{"tmp"\}}。
普通保存点在 \texttt{MSEvaluatePath} 返回值中携带坐标、完整主积分向量、状态和用户序号；
使用下列调用导出 CSV/JSON：
\begin{verbatim}
MSExportEvaluationData[
  result,
  MSOutputDirectory -> outputDirectory
]
\end{verbatim}
临时点不进入普通点导出。
...
\subsection{单顶点函数族：定义、约化与统一点序列}

单顶点函数族使用 \texttt{MSInitVertexFamily}，不需要构造一个只有单端点线的伪拓扑。包内
\texttt{Examples/02\_single\_vertex\_family.wl} 同时给出紧凑模型和等价显式模型，随后读取
masters、构造 dlog DE 并约化一个 shifted integral。数值部分只使用一个入口：
\begin{verbatim}
...
multi  = MSEvaluatePath[ctx,multipointSequence,
  ParameterRules->parameterRules];
\end{verbatim}
例子逐分量检查共同首点，说明单点只是同一 \texttt{pointSequence} 的长度 1 情形。

\subsection{两顶点 massless Full：多 sector 与多点求值}

\texttt{Examples/01\_massless\_full\_edge.wl} 从两个 \texttt{+} 顶点和一条公开
\texttt{"massless"} 线开始；程序自动派生 Full/quotient、top/child sectors、主积分、contact
map 与 dlog DE。例子把 \texttt{\{k1,k2\}} 作为表头，后续行只写坐标值，共同的
\texttt{q/a1/a2} 只在 \texttt{ParameterRules} 中给一次；随后开启 FlintNDE 节点规划和 fast
multipoint，并用 \texttt{MSExportEvaluationData} 导出保存点。临时途经点写成
\texttt{\{\{value1,value2\},"tmp"\}}，不另设路径对象。

\subsection{massive 三顶点：零中间外腿能量不是自动奇点}

\texttt{Examples/05\_massive\_three\_vertex\_tree.wl} 初始化同一个三顶点、两条 massive 线的
context，固定 $k_3=5\ii$、$q_{12}=1$、$q_{23}=2$、$\nu_{12}=3/4$、$\nu_{23}=1/3$ 与
$a_1=a_2=a_3=1$。原点列
\begin{verbatim}
...
\end{verbatim}
由第二次独立的 \texttt{MSEvaluatePath} 规划、求值和导出，两组只复用 context。

奇点判据是把每个点代入完整 top/contact-sector dlog letters，而不是检查某个坐标是否为零。
本例第二组四点没有任何 letter 为零，因此它们都是普通点，25 个主积分分量全部保存，
\texttt{singularityClassifications} 为空；低阶主链与高阶参考链的最大逐点相对差约为
$2.31\times10^{-31}$。若别的函数族在零坐标或任意非零复坐标真正命中 exact letter，仍由同一
FlintNDE 局部幂对数入口区分可去、pole 与 log 发散，不按变量名或图类型分派。

\subsection{三顶点正规化：自动 Laurent 支撑与增量拟合}

\texttt{Examples/06\_massless\_three\_vertex\_ep\_regularization.wl} 取
$a_1=a_2=a_3=1+\epsilon$，用户只指定 \texttt{MaximumEpPower->0}。程序先由符号边界和 DE
认证最低幂，再自动选择生产/独立验证点；验证精度不足时只增加内部拟合阶数和新增点，并复用
已计算值。坐标表只含不依赖 $\epsilon$ 的 $k_i$ 值，$a_i\to1+\epsilon$ 只写在
\texttt{ParameterRules}；该例的认证最低幂是 0，故返回有限项而不虚构 pole。

\subsection{无物理外腿：私有辅助能量输运到零}

\texttt{Examples/07\_zero\_external\_leg\_energy.wl} 展示无物理外腿顶点的两种等价输入：
省略 \texttt{externalLegEnergy} 或显式输入 0。例子只在 \texttt{pointSequence} 中给物理
坐标，检查私有辅助能量不暴露给用户、解析 dlog 保持一致，并实际从阻尼边界输运到零点。
