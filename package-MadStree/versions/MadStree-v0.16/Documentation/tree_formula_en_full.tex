...
\begin{abstract}
This handbook corresponds to \texttt{package-MadStree} v0.16; time-only sectors continue to be identified by fixed-length strings in the order of the root propagators.
...
\section{v0.16 topology input convention}
...
\end{align}
$\cN_s$ is the sector normalization, $\Xi_{s,\bm a}$ collects the shared-massless chamber sign and the RedundantH normalization, and $E_{sC,\bm a}$ is the product of all Hankel endpoint coefficients of the component. Each contracted massive Full line $e$ contributes
\begin{equation}
...
\end{equation}
to $\cN_s$. The factor $\eta_e$ defines only the normalization of the physical child master; it is not multiplied again into the normalized-master dlog contact block or recurrence event. The remaining constant coefficient of a massive contact is already complete in the defining-integral coefficient of each component, and no extra phase may be multiplied according to contact depth. All quantities are taken from the current sector metadata; one must not guess the child powers or two-state factors from the root sector.
...
For the two-vertex case with equal time powers $\nu_0$ in the negative-prefactor convention, Eq.~(4.2) of 2411 defines, in the paper endpoint basis,
\begin{equation}
...
\end{equation}
This expression cannot be copied directly into the MadStree sector normalization without a basis conversion. For the physical order $\nu_1=\ii\mu$, the conjugate-order paper endpoint and the common-order MadStree endpoints obey
\begin{equation}
 e^{-\pi\mu}H^{(2)}_{-\ii\mu}(z)=H^{(2)}_{\ii\mu}(z).
 \label{eq:paper-endpoint-to-common-hankel-order}
\end{equation}
The explicit Boltzmann factor in Eq.~\eqref{eq:paper-two-vertex-child-normalization} is therefore absorbed by the endpoint-basis conversion. This example is $G_{++}$, so Eq.~\eqref{eq:massive-full-pinch-normalization} has $\eta_e=+1$ and the child normalization in the common-Hankel-order MadStree basis is
\begin{equation}
 \cN_R^{\rm MS}=-\frac{4\ii}{\pi}k_s^{-2\nu_1-1},
 \label{eq:madstree-common-order-child-normalization}
\end{equation}
and must not be multiplied by $e^{\pi\operatorname{Im}\nu_1}$ again. The same fifth branch has the leading vector and the basis-specific physical coefficients
\begin{align}
 v_5&=\left(0,\frac{1}{2\nu_1-\nu_0},\frac{1}{\nu_0+1},0,1\right)^T,
 \notag\\
...
\end{align}
This uses the $i\epsilon$ branch $\int_{-\infty}^{0}\dd\tau\,(-\tau)^p e^{\ii K\tau}=(-\ii)^{p+1}\Gamma(p+1)K^{-p-1}$. The phase $e^{\ii\pi(\nu_1-\nu_0)}$ printed in Eq.~(4.11) is larger by one factor of $\ii$ than this direct evaluation of the defining Eq.~(4.2). MadStree follows its public definition $J_s=\cN_s I_s$, uses the direct integral, and explicitly maps to the same endpoint basis before comparison with the paper. The positive-prefactor context only replaces the paper parameters in these formulas uniformly by \texttt{formulaNu}$=-|\nu|$. The five-dimensional result above is now merely a check example of Eqs.~\eqref{eq:generic-sector-boundary-component}--\eqref{eq:generic-sector-indicial-vector}, not a production dispatch; general unequal time powers read the $A_{sC}$ of each component directly. The massive pinch normalization for real positive physical momenta is fixed by Eq.~\eqref{eq:madstree-common-order-child-normalization}; the formal symbol $(-k_s)^{-2\nu_1-1}$ must not be handed to the Mathematica principal branch.
