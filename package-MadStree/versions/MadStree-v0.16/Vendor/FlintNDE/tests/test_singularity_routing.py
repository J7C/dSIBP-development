...
class SingularityRoutingTest(unittest.TestCase):
    ...
    def test_large_translation_detects_internal_singularity(self) -> None:
        """高精度大平移路径仍必须识别宽度为 2 的内部奇点。"""

        base = 10**40
        system = RationalMatrixSystem(
            ((rational_function(1, [-base, 1]),),),
            name="large-translation-internal-pole",
        )
        with self.assertRaises(AdaptivePathSingularityError):
            build_adaptive_path(
                system,
                NamedPoint("left", base - 1),
                NamedPoint("right", base + 1),
                path_tolerance=1.0e-60,
            )

    def test_message_language_is_case_sensitive(self) -> None:
        """通用 routing 入口不得自动把小写语言值转换为现行值。"""
        ...
        with self.assertRaisesRegex(ValueError, "EN.*CN"):
            build_adaptive_path_plan(
                system,
                NamedPoint("left", 0),
                NamedPoint("right", "1/2"),
                message_language="en",
            )

    def test_default_avoid_mode_reports_general_rational_singularity(self) -> None:
        system = RationalMatrixSystem(
            ((rational_function(1, [0, 1]),),),
            name="default-avoid-general-rational",
        )
        with self.assertRaises(AdaptivePathSingularityError) as context:
            build_adaptive_path(
                system,
                NamedPoint("left", -1),
                NamedPoint("right", 1),
        ...
        self.assertEqual(
            context.exception.report["schema"], "flintnde_singular_path_report_v1"
        )
        self.assertEqual(context.exception.report["singularity_mode"], "avoid")
        self.assertEqual(len(context.exception.report["segments"]), 1)
        ...
        self.assertEqual(plan.singularity_mode, "avoid")
        self.assertEqual(plan.message_language, "CN")
        self.assertTrue(any("该计划不可继续执行" in message for message in plan.messages))



    def test_adaptive_path_round_trip_preserves_general_bridge(self) -> None:
        system = RationalMatrixSystem(
            ((rational_function([-2, 2], [0, -2, 1]),),),
            name="adaptive-round-trip-general-rational",
        )
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
        ...
        serialized = adaptive_path_to_json(path, digits=70)
        self.assertEqual(serialized["planning_precision_digits"], 70)
        restored = adaptive_path_from_json(system, serialized)
        original = transport_path(
            system, column_vector([3]), path, order=32, sample_count=96
        )
        round_tripped = transport_path(
            system, column_vector([3]), restored, order=32, sample_count=96
        )
        self.assertEqual(
            restored.planning_action, "raw_points_automatic_plan"
        )
        self.assertLess(
            float(abs(original[0][-1][0, 0] - round_tripped[0][-1][0, 0]).mid()),
            1.0e-25,
        )
    ...
    def test_internal_singularity_warns_and_uses_frobenius_bridge(self) -> None:
        ...
        self.assertTrue(
            any("multivalued branch" in str(item.message) for item in caught)
        )
        ratios = [
    ...
    def test_frobenius_batch_shares_system_and_preserves_each_column(self) -> None:
        ...
        final = result["primary_snapshots"][-1]
