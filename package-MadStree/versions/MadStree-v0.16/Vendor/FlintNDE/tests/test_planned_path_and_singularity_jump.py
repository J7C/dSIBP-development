"""检查 0.5.0 的自动规划、直接用户节点、奇点折跃与步内覆盖求值。"""
...
class SingularityJumpPlanningTest(unittest.TestCase):
    ...
    def test_exact_singular_user_point_is_classified_and_continuation_runs(self) -> None:
        """精确奇点是局部目标而非普通节点；同一局部基继续产生后续值。"""

        system = PartialFractionSystem(
            constant=acb_mat([[acb(0)]]),
            residues=(acb_mat([[acb(-1)]]), acb_mat([[acb(0)]])),
            poles=(acb(1), acb(5)),
            name="exact-singular-user-target",
        )
        points = [
            acb("2/5"), acb("7/10"), acb("17/20"), acb(1),
            acb("23/20"), acb("13/10"), acb(2),
        ]
        plan = plan_transport_path(
            system,
            acb(0),
        ...
        self.assertEqual(plan.report["singular_target_count"], 1)
        self.assertEqual(plan.report["singular_target_user_indices"], [3])
        self.assertFalse(any(abs(node - acb(1)).contains(0) for node in plan.nodes))
        incoming = [
            item for item in plan.sample_assignments
            if item["source"] == "covered_by_singularity_jump_incoming"
        ]
        outgoing = [
            item for item in plan.sample_assignments
            if item["source"] == "covered_by_singularity_jump"
        ]
        self.assertGreaterEqual(len(incoming), 1)
        self.assertGreaterEqual(len(outgoing), 1)

        plan = planned_path_from_json(
            planned_path_to_json(plan, digits=70),
            system=system,
        )
        ...
        singular = result["singular_target_results"]
        self.assertEqual(len(singular), 1)
        self.assertEqual(singular[0]["user_point_index"], 3)
        self.assertEqual(singular[0]["classification"], "true_pole")
        self.assertEqual(singular[0]["values"], ("Infinity",))
        self.assertEqual(result["singular_target_relative_difference_midpoint"], 0.0)
        self.assertLess(
            float(abs(result["primary_snapshots"][-1][0, 0] + acb(1)).mid()),
            1.0e-28,
        )

    def test_noncollinear_turn_at_exact_singularity_fails_closed(self) -> None:
        """经过奇点转向不唯一指定解析延拓分支，规划器不得自行猜测。"""

        system = _fractional_diagonal_system()
        with self.assertRaisesRegex(SingularityJumpError, "continuation branch"):
            plan_transport_path(
                system,
                acb(0),
                [acb(1), acb(1, 1)],
    ...
    def test_pole_beyond_terminal_match_point_is_not_a_near_miss_jump(self) -> None:
        """终点外侧的目标极点只限制收敛步长，不应伪造无空间的折跃。"""

        system = PartialFractionSystem(
            constant=acb_mat([[0]]),
            residues=(acb_mat([[-1]]), acb_mat([[1]])),
            poles=(acb(1), acb("11/10")),
            name="terminal-singular-match-approach",
        )
        plan = plan_transport_path(
            system,
            acb(0),
        ...
        self.assertEqual(plan.report["singularity_jump_count"], 0)
        self.assertTrue(abs(plan.nodes[-1] - acb("39/40")).contains(0))
    ...
    def test_serialized_plan_round_trip_executes_without_replanning(self) -> None:
        ...
        self.assertLess(
            float(abs(result["primary_snapshots"][-1][0, 0] - acb(0, -1)).mid()),
            1.0e-28,
        )
...
class PlannedCertificationTest(unittest.TestCase):
    ...
    def test_separate_initial_states_propagate_independent_chains(self) -> None:
        """跨调用续算时，参考链不得从主链状态重新起步。"""

        system = _fractional_diagonal_system()
        plan = direct_user_point_path(system, acb(0), [acb("1/4")])
        primary_initial = column_vector([1, 1])
        reference_initial = column_vector([2, 2])
        with self.assertWarnsRegex(UserWarning, "independent initial states"):
            result = transport_planned_path_refined(
                system,
                primary_initial,
                plan,
                reference_initial_vector=reference_initial,
                primary_order=8,
                reference_order=12,
                certification_mode="embedded",
            )
        expected_primary = transport_planned_path(
            system, primary_initial, plan, order=8
        )[0][-1]
        expected_reference = transport_planned_path(
            system, reference_initial, plan, order=12
        )[0][-1]
        self.assertEqual(result["certification_mode"], "certified")
        self.assertTrue(result["separate_initials_forced_certified"])
        self.assertLess(
            float(relative_difference_inf(result["primary_snapshots"][-1], expected_primary).mid()),
            1.0e-45,
        )
        self.assertLess(
            float(relative_difference_inf(result["reference_snapshots"][-1], expected_reference).mid()),
            1.0e-45,
        )
        self.assertGreater(result["relative_difference_midpoint"], 0.4)
