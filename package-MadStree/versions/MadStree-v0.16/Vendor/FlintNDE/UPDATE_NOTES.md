...
本版从 0.4.0 建立。新增 `evaluate_singular_target`，从认证局部幂对数基判断用户命中
奇点的有限、真实 pole 或 log 发散状态。`test_singular_targets` 专项现为 7/7，包含目标位于
任意非零 exact $\mathbb Q(i)$ 点且其它有限奇点共存的二维逐分量分类；MadStree Vendor 同项
也为 7/7。0.4.0 的普通点、正规输运、正规化和现有奇点折跃行为保持回归通过。

## 基线与定位

- 从冻结的 0.4.0 建立；0.4.0 不再回写。
- 0.5.0 继续定位为与物理模型无关的单变量一阶矩阵微分方程求解器。它不读取 MadStree
  的 dlog letters、主积分顺序、normalization 或多变量图信息。
- 独立包与 MadStree v0.16 的 `Vendor/FlintNDE` 保持同一份实现和 0.5.0 版本身份。
...
6. `reconstruct_series_solution` 的显式 `sample_points` 现可作为冗余有序候选池；新增
   ...
   验证失败后复用旧值并增量取点，候选池耗尽时绝不生成用户范围外取值，并返回带
   `precision_target_met=False` 的当前最佳拟合而不是丢弃系数。
   新参数缺省为 `"automatic"`，未显式配置时输入格式、自动采样公式和结果保持不变。
7. 新增 `sample_angle_range=(theta_min,theta_max)`。该弧度区间按开集解释，程序只在内部
   均匀选择最多三条复射线；模长和验证缩放仍沿用原 AMFlow 式精度策略，不增加模长上限。
   缺省 `"automatic"` 不增加输入字段并严格保留正实轴网格。

以下能力说明为从 0.4.0 继承且在 0.5.0 继续保留的基线。
...
6. **奇点双侧 bucket 与精确用户索引**：精确中间奇点不进入普通节点链；同一局部基按
   奇点到最近其它奇点的距离覆盖收敛域内入射/出射用户点，保留原始 user index，并从
   出射普通点继续。计划 JSON round-trip 后不重新规划。
7. **末端奇点隐藏匹配点**：`plan_singular_target_match` 在当前点位于目标局部圆外时沿来向
   插入域内普通匹配点。主链低阶值作为输出，参考链高阶值只核验逐分量分类、发散状态和
   有限值误差；关闭规划而必须插点时明确失败。
8. **端点外近掠修复**：极点投影落在线段端点时不再被误判为可放置双侧匹配点的 near miss；
   该极点仍参与普通步长的最近奇点收敛半径。
9. **双语提示**：Python `message_language="EN"|"CN"` 与 Wolfram
   `MessageLanguage -> "EN"|"CN"`；缺省英文。规划结果明确说明当前模式，
   执行结果明确说明没有重新规划。
10. **标准 Wolfram 程序包入口**：版本根的 `FlintNDE.m`、`Kernel/init.m` 与
...
11. **浅层 Wolfram 运行目录**：`"WorkDirectory"` 表示运行根本身；`Automatic` 只建立一次
...
12. **无 shell 的 Wolfram launcher**：物理删除 `Run` 命令字符串、引号转义和日志重定向
...
- `primary_order` 现在是节点、dense sample、保存点和正规化拟合的唯一生产阶数；更高的
  `reference_order` 只提供相对差与精度门禁。跨调用续算支持分别传递两条链的初值，避免在
  段界用参考链覆盖主链；带保存点的 embedded 请求自动升级为独立双链以保证保存的是主链值。
- 工作位数统一按
...
- 每个 Python 进程首次 `import flintnde` 时在 stderr 显示一次
  `FlintNDE package paper, arXiv identifier pending` 引用提醒；不列出 dSIBP、MadStree、
  AMFlow 或其它论文。文件协议适配器可通过 `FLINTNDE_SUPPRESS_CITATION_NOTICE=1`
  显式静默，并自行在响应 metadata 保存所属程序包引用。
...
- 独立包与 MadStree v0.16 Vendor 的 42 个运行时代码文件逐文件 SHA-256 一致；两份说明
  文档只因目录身份和相对路径不同而不进入逐字节同步门禁。
- 0.5.0 独立检验：257 点、64 阶 fast/Horner 最大差 `1.96586e-62`；900 点 planned/direct
...
- 七个仓库 examples 均在删除旧 `results/`/`results_temp/` 后从 0.5.0 路径 fresh 执行；正式 PDF 经 XeLaTeX/BibTeX 构建，
  日志无未定义引用，抽查页目检通过。
...
- 当前共享 regulator、transport、singularity 与根 `__all__` 不变；同一改动已逐字节同步到
  MadStree v0.16 Vendor。独立包和 Vendor Python 各 `181/181`，Wolfram 端到端 `25/25`，
  MadStree Backend `17/17`。
