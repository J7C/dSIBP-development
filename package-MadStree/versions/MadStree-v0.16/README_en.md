...
Current version: `v0.16`. This directory is upgraded from v0.15; interface changes are listed in `UPDATE_NOTES_en.md` (Chinese: `UPDATE_NOTES.md`).
...
After deleting the existing `results/` and `results_temp/` trees, all seven examples passed fresh v0.16 runs with `Example PASSED` and exit code `0`. Example 06 passed `16/16` checks with three production points and two independent validation points; the default request of 12 workers was automatically capped at the five actual ep tasks.
...
The current fresh v0.16 regression includes core `61/61`, formula artifacts `24/24`,
...
Python adapter `17/17`, and the vendored FlintNDE `181/181` passed. Independent Validation-04
and Validation-07 passed fresh at `26/26` and `21/21`, respectively.
