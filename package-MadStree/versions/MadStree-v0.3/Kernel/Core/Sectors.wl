...
msSectorKey[{}] := "top";
msSectorKey[ids_List] := "contracted:" <> StringRiffle[ToString[#, InputForm] & /@ ids, ","];
  ...
  While[queue =!= {},
    ...
    Do[
      ...
      Do[
        ...
        sourceKey = msSectorKey[state];
        targetKey = msSectorKey[target];
        AppendTo[
          transitions,
          <|
...
msComponentEnergy[component_List, vertices_List, lines_List] := Module[{energy, endpointPosition},
  energy = Total[Lookup[Select[vertices, MemberQ[component, #["id"]] &], "energy"]];
  Do[
