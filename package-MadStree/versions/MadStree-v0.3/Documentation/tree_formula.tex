...
\end{equation}
这个正号不是 convention-free 的独立规则，而是式 \eqref{eq:package-raising-contact-sign} 与本文 $\widetilde\Omega_0=-\ii\log\kappa$ 定义的共同结果。单条 massless full edge 的定义积分有限差分给出直接检查：使用旧负号时 residual 恰为旧 contact contribution 的 $-2$ 倍；改成式 \eqref{eq:global-offdiagonal-omega} 后只剩有限差分误差。
...
生产入口不在有限 Euclidean 普通点直接计算定义积分。\texttt{MSBoundaryData} 以文献 \cite{Chen:2024jms} 的 $K_v\to\infty$ 数据为唯一边界来源：单顶点 massive-external family 使用式 (3.44)--(3.46) 的显式多变量级数；两顶点单 massive $G^{++}$ 使用 $x=k_{34}/k_{12}$、$y=1/k_{34}$ 的 blow-up，并把一参数曲线 $x=x_A t,y=y_A t$ 上的 exact $\mathbb Q(\ii)(t)$ 系统和五个 indicial branches 交给 FlintNDE。有限定义积分只允许存在于独立验证目录，不是生产 fallback。
...
\end{align}
正 prefactor context 只把这些公式中的论文参数统一换成 \texttt{formulaNu}$=-|\nu|$。当前两顶点 producer 只对两端相等的 time power 认证；不等时返回 \texttt{TwoVertexUnequalTimePowersUnsupported}，不猜测相位推广。实正物理动量的 massive pinch normalization 固定为式 \eqref{eq:paper-two-vertex-child-normalization} 的 $-4\ii k_s^{-2\nu_1-1}/\pi$，不能把形式符号 $(-k_s)^{-2\nu_1-1}$ 交给 Mathematica 主支。
...
\end{center}
奇点与普通段同时存在时可用键 \texttt{"singular"}、\texttt{"ordinary"} 分开列出。接口不接受点名或第三个字段。后端每到一个标记点便立即在调用目录的 \texttt{results/flintnde\_save\_points/run-UUID/} 写独立 JSON，全链成功后再生成总表 \texttt{madstree\_flintnde\_save\_points.json}。普通点记录保存坐标和完整主积分向量；正则奇点起点保存可复用的 Frobenius $\{a,b,C\}$，不能伪造一个“奇点处函数值”。若后续段失败，已经完成的逐点文件保留，但不写完整汇总。

FlintNDE 在启动前分类局部奇点并执行 capability preflight；当前两顶点曲线起点被认证为 regular singular。若输入需要超出后端已声明能力的高阶或不规则奇点处理，适配器直接 fail closed，不把普通 Taylor 输运伪装成奇点解。

原被积函数的 $e^{-K_vt_v}$ 阻尼在 $K_v\to\infty$ 的原坐标表述中看起来具有指数渐近；这不等于正规化主积分的 dlog 系统在 blow-up 坐标 $x_j=0$ 必有本性奇点。对当前认证的单顶点与两顶点 $G^{++}$ families，抽出已知物理权重并作 nested blow-up 后，交给 FlintNDE 的 exact 系统分别是普通匹配问题或 Fuchsian regular-singular 问题。若实际拉回系统仍有不可约二阶以上 pole、需要 Stokes/ramification 或不属于当前 exact field，capability preflight 会返回实际分类并停止；package 不把它自动解释成已解决的边界条件。

当前已经端到端检查单顶点三 massive 的 2411 边界、三顶点 massive/massless 的 quotient/RedundantH 公式表示与 compatible-boundary 输运，以及两顶点 $G^{++}$ 五维系统。后者的完整 connection、奇点拉回、五个 leading vectors、五个系数和目标五维向量均与独立论文路线一致。结构检查还覆盖共同-theta simultaneous events、massless/massive coincident quotient，以及 triangle time-only 圈图 strict charts；FlintNDE 适配器另检查普通/正则奇点保存点和超能力 pole 的 fail-closed。v0.3 独立任务 T1--T6 的 fresh 计数依次为 $20/20,12/12,18/18,15/15,17/17,16/16$。任意 topology 与一般参数区域仍需按专项验证范围解释，不能由这些代表性结果外推为全面认证。
