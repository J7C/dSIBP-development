...
\end{equation}
这里 $s$ 不是由局部 $\bm n,\bm a$ 反推的标签。初始化先把已缩并 line ids 按 root line position 排序，得到 canonical set $L_s^{\rm ctr}$，再由它唯一生成
\texttt{sectorKey}; 完整积分头始终把该 key 放在第一参数。因此两个 subsectors 即使 $c_s,N_s,\bm n,\bm a$ 全部相同，也仍是不同的
...
\end{equation}
这里 $E_{st}$ 是该 simultaneous contact event 实际缩并的边集，
\begin{equation}
 \chi(E_{st})=(-1)^{N_0(E_{st})},
 \qquad
 N_0(E_{st})=\#\{e\in E_{st}:e\text{ is masslessFull}\}.
 \label{eq:massless-contact-phase}
\end{equation}
pure-massive 时 $\chi=1$，式 \eqref{eq:global-offdiagonal-omega} 的正号是式 \eqref{eq:package-raising-contact-sign} 与本文 $\widetilde\Omega_0=-\ii\log\kappa$ 定义的共同结果。massless 指数二态由 2401 式 (3.13)--(3.14) 的两个 theta chambers 得到额外负号；它必须按 event 的 selected lines 计算，不能按图或 sector 写专用分支。单条 pure-massless full edge 的解析 chamber 积分对两个顶点能量求导后，式 \eqref{eq:massless-contact-phase} 使完整 DE residual 逐分量精确为零。
...
\end{align}
这就是“删去被缩并端点 bit，再在另一侧补入其互补 bit”的精确含义；所有未删 bits 都由 Kronecker delta 保持。本文的 $(-\tau)^A$ normalized-$J$ convention 把式 \eqref{eq:2401-binary-top-to-sub} 的整体 $-\ii$ 按式 \eqref{eq:package-raising-contact-sign} 改成式 \eqref{eq:global-offdiagonal-omega} 的 $+\ii$；不能只改二进制 selector 而漏掉这个 convention 变换。
...
\texttt{masslessEndpointH} slots，按有序端点保留 $(00,01,10,11)$ 四态。后一种输入只接受
$\nu=1/2$，并等价接受 \texttt{functionSystem -> "h"} 作为初始化别名。context 建立后，递推、
contact、dlog 与数值层都不能逐调用切换表示。
...
原 dSIBP package 的 public line representation 仍按 root line 顺序保存三槽 massless state；共享二维空间是公式层和 canonical source 层的 quotient，不允许据此改写旧 package 的公开积分 Head。MadStree 使用独立的原生 time-only 表示；其缺省 massless 输入是 $\nu=1/2$ 和正 prefactor，组装 2401 公式时统一代入 $\nu\to-\nu=-1/2$。这一解析等价不替代 line metadata、theta 或 contact normalization。
...
\end{verbatim}
其中 \texttt{k0} 与 \texttt{A0} 是纯顶点指数能量和 $(-\tau)$ 基准幂；其余位置一一给出 h block 的动量、$\nu=|\nu|$ 和 Hankel branch。显式形式可以改给 \texttt{k0/timePower/hBlocks/exponentialBlocks}。每个纯指数 block 是一维因子，只平移 effective energy；每个 h block 是二维 massive endpoint slot。因此该 context 可以原样调用 \texttt{MSFormulaMatrices}、\texttt{MSMasterIntegrals}、\texttt{MSDLogDE}、\texttt{MSBoundaryData} 和 \texttt{MSEvaluateVertexFamily}。\texttt{NuConvention} 只在初始化时选择，之后所有这些接口都固定读取 context。
...
上述定理也给出了自动化范围。完全不含任何 $K_v$ 的奇点已经显式列在 DE letters 中，不是这个边界 producer 的待解决障碍，也不需要 package 去计算奇点上的发散值。缺省自动路径只接受普通点并绕开它们；用户若主动把路径终点放在这些奇点上，直接使用 package 给出的 DE 自行选择 blow-up。若额外 gauge transformation 引入了不属于式 \eqref{eq:general-affine-K-divisor} 的新分母，则该变换本身不能自动继承上述证明。
...
因此推荐流程是：先按式 \eqref{eq:preferred-rank-from-point} 选择首选 chart，从式 \eqref{eq:nested-blowup-coordinates} 的边界得到同序向量，在 chart 内取一个小而有限的普通点，再自动绕开所有 letter hypersurfaces 到用户 preferred 普通点和最终目标点。辅助顶点能量若要移除，应逐个取零，并在每一步对所有 sectors 重做普通点证书。只有证书通过时，“先把这个辅助能量跑到零再跑其它参数”才是合法的简化；一旦零点是奇点，自动流程停止并返回该点的 DE letters，由用户自行决定后续 blow-up。

\subsection{当前 2411 无穷远边界与 FlintNDE 输运}

生产入口不在有限 Euclidean 普通点直接计算定义积分。\texttt{MSBoundaryData} 以文献 \cite{Chen:2024jms} 的 $K_v\to\infty$ 数据为唯一边界来源。单顶点 massive-external family 保留式 (3.44)--(3.46) 的显式多变量级数优化；其它已闭合 tree/time-only context 不再按顶点数、master 数或图名分派，而是统一消费 sector DAG、component、slot registry、normalization、state order 与 strict rank。
...
\end{equation}
于是 $W_{s,\bm a}v_{s,\bm a}$ 自动包含 child solution 注入 parent 的 leading 分量。重根或整数差共振不另造图专用参数；完整 exact residue 与这些 $\{\lambda,0,v\}$ 交给 FlintNDE 的 power-log 递推。有限定义积分只允许存在于独立验证目录，不是生产 fallback。
...
\texttt{MSEvaluateTree} 先调用 FlintNDE 的 regular-singular Frobenius 模块把每个 exact branch 从 $t=0$ 输运到有限匹配点，乘回 Gamma 等物理权重并求和，再沿匹配点到用户普通点的仿射参数拉回 dlog connection。\texttt{MSEvaluateVertexFamily} 对 \texttt{MSInitVertexFamily} context 复用同一链路。后端路径只由相对项目根目录的单一配置变量控制；运行文件与 Python cache 写入调用脚本目录的 \texttt{results\_temp/}，不写 package 源码目录。

路径保存点由 Wolfram 选项 \texttt{FlintNDESavePoints} 给出。普通路线的完整输入为
\begin{center}
 \texttt{FlintNDESavePoints ->}\\
 \texttt{\{\{0,"save"\},\{1/2,"save"\},\{1,"save"\}\}}.
\end{center}
奇点与普通段同时存在时可用键 \texttt{"singular"}、\texttt{"ordinary"} 分开列出。接口不接受点名或第三个字段。后端每到一个标记点便立即在调用目录的 \texttt{results/flintnde\_save\_points/run-UUID/} 写独立 JSON，全链成功后再生成总表 \texttt{madstree\_flintnde\_save\_points.json}。普通点记录保存坐标和完整主积分向量；正则奇点起点保存可复用的 Frobenius $\{a,b,C\}$，不能伪造一个“奇点处函数值”。若路径中的保存坐标属于 FlintNDE 已认证、可双向桥接的指数型高阶 pole，后端改为保存
$\{\phi,a,b,C\}$，表示 $\exp(\phi(z))z^a(\log z)^bC$；\texttt{phi} 的全部负幂系数由微分方程推导并 exact 校验。缺少 Stokes connection 的 formal 分支只能保存起点，不能作为中间点或终点。若后续段失败，已经完成的逐点文件保留，但不写完整汇总。
...
当前已经端到端检查单顶点三 massive 的 2411 边界、两顶点 pure-massless 的解析 chamber 边界、三顶点 massive/massless 的通用 15 维边界与输运，以及两顶点 $G^{++}$ 五维系统。pure-massless 定义积分同时检查式 \eqref{eq:massless-contact-phase} 的两张 DE residual 和完整目标向量；massive 五维系统的 connection、奇点拉回、五个 leading vectors、五个系数和目标向量均与独立论文路线一致。结构检查还覆盖共同-theta simultaneous events、massless/massive coincident quotient，以及 triangle time-only 圈图的通用 boundary/residue。FlintNDE 适配器另检查普通/正则奇点保存点和超能力 pole 的 fail-closed。代表性运行证据不等于对所有 topology 与参数区域逐点认证；通用性来自同一 metadata 数据流与结构不同拓扑的共同回归，而不是列举专用生产分支。
