...
(***
文件：Topology.wl
用途：规范化并验证 MadStree 所需的有序树或纯 time-only incidence graph 输入。
边界：不读取 loop momentum、ISP 或 scalar-product closure；数值边界所需的顶点相位与
      Hankel branch 只在初始化时规范化，此后由 context 固定持有。
***)
...
(* branch 编号遵循 HankelH[1|2,nu,z]。full propagator 的 branch 由时间排序动态决定。 *)
...
msNormalizeLine[line_Association, position_Integer, nuConvention_String] := Module[
  ...
  pinchNormalization = Lookup[
    ...
    Switch[
      type,
      (* 2411.03088 Eq. (4.2) 已把物理动量定义为正模长；不要把形式上的
         (-k)^power 交给 Mathematica 主支，否则一般实 nu 会产生伪复相位。 *)
