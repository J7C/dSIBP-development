...
`MSBoundaryData[context,targetRules]` 只使用 2411.03088 的 $k_0\to\infty$ 数据，不计算有限点定义积分。单顶点 massive-external family 保留 Eqs. (3.44)--(3.46) 的多变量级数优化；其它已闭合 tree/time-only context 统一使用 metadata producer：按 strict rank 构造 $K_{\sigma(j)}=B^{V-j+1}t^{-(V-j+1)}$，拉回完整 block-triangular dlog connection，求 $t=0$ residue，并对每个 sector master 使用自身 component base powers、normalization、slot/state order 与 theta fixing 计算物理 leading weight。ancestor-sector 线性系统给出与全局 master 顺序一致的 leading vector。
...
- `MSBoundaryData[context,targetRules]` 生成单顶点显式有限匹配向量，或任意已闭合 multi-sector context 的 exact 奇点分支数据；
...
FlintNDE 的位置只由 `$MSFlintNDERelativePath` 定义，v0.5 缺省为版本目录内的 `Vendor/FlintNDE`；`MSSetFlintNDERelativePath` 可显式覆盖。普通与奇点 JSON schema 分别为 `madstree_flintnde_transport_v1`、`madstree_flintnde_singular_transport_v1`。临时 JSON 写入调用脚本目录的 `results_temp/`；成功后删除。Python 使用默认 `__pycache__/` 并由 Git 忽略，版本源码目录不接收用户结果。`MSRuntimeDirectory` 的绝对路径直接展开，相对路径相对调用脚本目录解析；实现只使用单参数 `ExpandFileName`，兼容当前 Mathematica 内核。

`FlintNDESavePoints` 的 Wolfram 合同为 `{{coordinate,"save"},...}`，或分阶段 Association `<|"singular"->...,"ordinary"->...|>`；不接受点名或第三字段。每个点到达后立即写入 `results/flintnde_save_points/run-UUID/<stage>/`，全链成功后再生成 `madstree_flintnde_save_points.json`。普通点保存数值向量；正则奇点起点保存 Frobenius `{a,b,C}`；后端路径若遇到已认证、可桥接的指数型中间奇点，则保存 `exp(phi(z)) z^a log(z)^b C` 的 `{phi,a,b,C}`。formal start-only 与需要 Stokes connection 的中间/终点不允许继续。后续失败保留已经完成的逐点文件，但不生成伪完整汇总。
...
```text
package-MadStree/versions/MadStree-v0.5/
  VERSION.md
  README.md
...
- [x] 集中配置 `package-FlintNDE/versions/FlintNDE-v0.1.0.dev0` 相对路径，并完成 exact $\mathbb Q(i)(s)$ 序列化、普通点检查、adaptive path 和 refinement 输运。
...
- [x] 实现 2411.03088 单顶点边界系数、state order 与 normalization；实现由完整 sector DAG、component/slot metadata 和 strict rank 驱动的通用 multi-sector producer，并直接接入 FlintNDE 奇点模块。两顶点 massive `G++`、pure massless、mixed 三顶点和 triangle time-only context 共用该 producer；单顶点显式级数只作为优化保留。
...
`NuConvention -> "Negative"`。v0.5 T1--T6 已 fresh 通过，计数依次为 `24/24`、`12/12`、`18/18`、`15/15`、`17/17`、`16/16`。
