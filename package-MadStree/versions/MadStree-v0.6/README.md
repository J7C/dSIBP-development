...
当前版本：`v0.6`。本目录由冻结 v0.5 建立；接口变化见 `UPDATE_NOTES.md`。
...
```wl
packageRoot = ".../package-MadStree/versions/MadStree-v0.6";
...
FlintNDE 源码随 v0.6 放在版本目录内（同步自 FlintNDE `0.1.0`），缺省位置只在一个相对路径变量中定义：
...
三个 examples 已从 v0.5 迁入 v0.6，并已从 v0.6 源码 fresh 运行且退出 `0`。
...
v0.6 全部 11 个开发 tests 与跨包 DE 检验均已串行 fresh 通过（含 Vendor 同步为 FlintNDE 0.1.0 后的重跑）；三个 examples 已用 v0.6 源码验证。独立验证计数沿用 v0.5：T1--T6 依次为 `24/24`、`12/12`、`18/18`、`15/15`、`17/17`、`16/16`。
