...
MSExportEvaluationData::usage = "MSExportEvaluationData[evaluation] exports the saved ordinary-point records from one MSEvaluatePlannedPath result to CSV and JSON. Transient, removed-singular and leading-order records are not exported. Options: MSOutputDirectory, ExportFormats and SignificantDigits.";
MSGeneratePath::usage = "MSGeneratePath[context,pointSequence] performs phase-one planning and returns a complete executable multipoint jump path. Any transport through intermediate nodes is a jump; only a jump that explicitly crosses a singularity through a local basis is a singularity jump. Bare coordinates are saved by default, {coordinate,\"tmp\"} marks transient waypoints, and {coordinate,\"lo\"} requests a direction-dependent leading-order record at a singular point. SingularityMode defaults to \"Avoid\"; \"SingularityJump\" permits singularity jumps, whose multivalued branch must be checked by the user. MessageLanguage defaults to \"EN\" and also accepts \"CN\".";
MSEvaluatePlannedPath::usage = "MSEvaluatePlannedPath[context,path] performs phase-two execution by consuming the complete plans stored by MSGeneratePath; it never replans. Execution WorkingPrecision must not exceed the stored planning precision, otherwise the path must be regenerated. The result contains coordinate-paired point records, singularity diagnostics and executionAction. MessageLanguage defaults to \"EN\" and also accepts \"CN\".";
MSPlannedPathQ::usage = "MSPlannedPathQ[expression] tests whether an object is a planned path association returned by MSGeneratePath.";
...
MSEvaluatePlannedPath::backendLaunchFailed = "FlintNDE backend produced no output under Python command `1`. Captured backend output tail: `2`. Install python-flint for that interpreter, or point PythonExecutable or MADSTREE_PYTHON at an interpreter that has python-flint.";
MSEvaluatePlannedPath::backendRunFailed = "FlintNDE backend returned a non-success status under Python command `1`. Backend error: `2`.";
