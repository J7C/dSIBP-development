...
\begin{abstract}
本手册对应 \texttt{package-MadStree} v0.10；time-only sector 继续由 root propagator 顺序的定长字符串标识。
...
\texttt{MSBoundaryData}，并通过 \texttt{MSGeneratePath} 与
\texttt{MSEvaluatePlannedPath} 完成两阶段数值计算。\texttt{NuConvention} 只在初始化时选择，
之后所有接口都固定读取 context。
...
\texttt{MSGeneratePath} 先为 exact 正则奇点边界的每个分支建立从 $t=0$ 到有限匹配点的
FlintNDE 计划，再在 MadStree 侧按输入顺序把用户点划分为最大连续复仿射单变量平面组
$x(s)=x_0+s v$，每组只拉回一次单变量矩阵 DE，并把全部 exact 复参数点交给 FlintNDE。
组内落在同一节点收敛圆盘的点由该节点缓存的解系数生成；不同组只在公共点继承数值向量，
不共享局部系数，因而没有构造多变量高维 Taylor 球。\texttt{MSEvaluatePlannedPath} 直接执行
这些已保存计划：它在有限匹配点乘回 Gamma 等物理
权重并求和，然后继续输运到用户普通点，不重新规划路径。后端位置由相对项目根目录的单一配置
...
FlintNDE 工作位数为 $\lceil p_{\rm dec}\log_2 10\rceil+32$：70、100 位分别为 265、365 bit，
更高精度继续按同一公式增长；奇点局部基门禁也按当前工作精度收紧。线段投影、匹配比例、旋转
因子和 winding/monodromy 均保持当前 Arb 精度；计划保存规划精度，执行要求更高精度时必须
重新规划。

用户点只接受三种形式：裸坐标缺省保存，\texttt{\{coordinate,}\allowbreak\texttt{"tmp"\}} 是临时途经点，
\texttt{\{coordinate,}\allowbreak\texttt{"lo"\}} 请求沿到达方向的奇点领头阶。普通保存点在
\texttt{MSEvaluatePlannedPath} 返回值中携带坐标、完整主积分向量、状态和用户序号；使用
\texttt{MSExportEvaluationData[result,}\allowbreak\texttt{ MSOutputDirectory -> outputDirectory]} 导出 CSV/JSON。
奇点领头阶、被移除奇点和临时点不进入普通点导出。
