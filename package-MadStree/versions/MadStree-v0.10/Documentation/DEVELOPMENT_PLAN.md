...
- `MSGeneratePath[context,pointSequence]`：在 MadStree 侧规划多变量折线、逐段仿射拉回，并保存每段完整 FlintNDE 计划；
- `MSEvaluatePlannedPath[context,path]`：只执行上述计划，不重新调用路径规划器；
- `MSPlannedPathQ[path]`：严格验证当前计划 Association；
...
用户点只允许三种形式：裸坐标缺省保存，`{coordinate,"tmp"}` 是临时途经点，`{coordinate,"lo"}` 请求沿到达方向的奇点领头阶。执行结果的普通点记录携带坐标、数值、状态和用户序号；普通保存点由 `MSExportEvaluationData` 导出。MadStree 不提供另一套路径保存选项。
...
```text
package-MadStree/versions/MadStree-v0.10/
  VERSION.md
  README.md
...
- [x] 集中配置 版本内 `Vendor/FlintNDE` 0.3.0 相对路径，并完成 exact $\mathbb Q(i)(s)$ 序列化、普通点检查、adaptive path 和 refinement 输运。
