...
\title{MadStree v0.10: tree formula}
...
\begin{abstract}
This handbook corresponds to \texttt{package-MadStree} v0.10; time-only sectors continue to be identified by fixed-length strings in the order of the root propagators.
...
\end{verbatim}
Here \texttt{First[ki]} and \texttt{First[nui]} are respectively the pure vertex phase
energy and the base power of $(-\tau)$; the remaining entries give the momentum,
$\nu=|\nu|$, and Hankel branch of each h block. The explicit data model uses
\texttt{energy/timePower/hBlocks/exponentialBlocks}. Each pure exponential block is a
one-dimensional factor that shifts only the effective energy, while each h block is a
...
\texttt{MSBoundaryData}, then use \texttt{MSGeneratePath} and
\texttt{MSEvaluatePlannedPath} for the two-phase numerical workflow.
...
The theorem above also fixes the scope of automation. Singularities that contain no $K_v$ at all are already listed explicitly in the DE letters; they are not obstacles to be solved by this boundary producer, and the package does not need to compute divergent values on the singularities. Default planning accepts only ordinary-point polylines that do not cross these singularities; if a direct segment hits one, the program returns the offending point pair and the user supplies explicit detour points. If the user deliberately places the end of a path on such a singularity, one should use the DE given by the package and choose the blow-up oneself. If an extra gauge transformation introduces new denominators not of the form of Eq.~\eqref{eq:general-affine-K-divisor}, that transformation itself cannot automatically inherit the proof above.
...
\texttt{MSGeneratePath} first builds a FlintNDE plan from $t=0$ to a finite matching point
for every exact regular-singular boundary branch. It then plans the user-supplied
multivariable polyline on the MadStree side, partitions the input order into maximal
consecutive complex-affine one-variable plane groups $x(s)=x_0+s v$, and pulls each group
back only once. All exact complex parameter points are passed to FlintNDE. Points in one
node convergence disk are evaluated from that node's cached solution coefficients; distinct
groups inherit only the value at their common point and never share local coefficients, so
no multivariable high-dimensional Taylor ball is constructed.
\texttt{MSEvaluatePlannedPath} executes these stored plans directly:
at the finite matching point it restores physical weights such as Gamma factors, sums the
branches, and continues to the user's ordinary points without replanning. A single
project-relative configuration variable controls the backend location; run files and the
Python cache are written under \texttt{results\_temp/} in the calling script's directory,
not in the package source tree.

Every multipoint transport through intermediate nodes is a jump; only an explicit crossing
of a singularity through a local-basis connection is a singularity jump.
\texttt{SingularityMode -> "Avoid"} is the default: when a segment between adjacent user
points hits a singularity, the result contains \texttt{"Singular Path Pair"} and the
offending pair. Explicit \texttt{SingularityMode -> "SingularityJump"} permits a
singularity jump and warns the user to confirm the multivalued branch of the equivalent
detour class. \texttt{MessageLanguage -> "EN"} gives English notices by default, while
\texttt{"CN"} selects Chinese; the values are case-sensitive. The backend discovers finite
singularities from each pulled-back one-variable system, so the user does not extract or
supply a singularity list.

If every dlog letter is constant on a segment, every pulled-back derivative vanishes, so the
segment is a zero connection with no finite poles and is transported normally. FlintNDE
uses $\lceil p_{\rm dec}\log_2 10\rceil+32$ working bits: 70 and 100 digits use 265 and
...
Plans store their planning precision, and execution at a higher precision requires
replanning.

User points have three forms: a bare coordinate is saved, \texttt{\{coordinate,"tmp"\}} is
a transient waypoint, and \texttt{\{coordinate,"lo"\}} requests singular leading data along
the arrival direction. Ordinary saved points in the \texttt{MSEvaluatePlannedPath} result
...
them as CSV/JSON. Singular leading data, removed singular points, and transient waypoints
are excluded from the ordinary-point export.
