...
For a multipoint polyline, replace the second argument with `{pointP1, pointP2, pointP3}`. `MSGeneratePath` only plans and returns the complete path; `MSEvaluatePlannedPath` only executes that plan and never replans. `MSPlannedPathQ` tests whether an object is a valid plan.
...
MadStree pulls every affine segment back to a one-variable connection. If all dlog letters are constant on a segment, every pulled-back derivative is zero, so the backend constructs a zero connection with no finite poles and transports it normally; the absence of poles is not a rejection condition.
