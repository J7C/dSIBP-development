...
Current version: `v0.10`. This directory is built by modifying v0.9 (version lineage v0.5 -> v0.6 -> v0.7 -> v0.8 -> v0.9 -> v0.10); interface changes are listed in `UPDATE_NOTES_en.md` (Chinese: `UPDATE_NOTES.md`).
...
The massless graph above can generate a complete multi-sector Frobenius boundary and transport to one or more user points through the same two-phase API:
...
For a multipoint polyline, replace the second argument with `{pointP1, pointP2, pointP3}`. `MSGeneratePath` partitions the input order into maximal consecutive complex-affine one-variable plane groups `x(s)=x0+s v`, pulls each group back once, and sends all exact complex parameters to FlintNDE. `MSEvaluatePlannedPath` only executes the stored plans and never replans. `MSPlannedPathQ` tests whether an object is a valid plan.

Bare coordinates are saved by default; `{coord, "tmp"}` marks a transient waypoint excluded from saved results, while `{coord, "lo"}` requests a singular leading-order record resolved along the arrival direction. Every other string tag is rejected. Point records pair `coordinate`, `value`, `status`, and `userIndex`; points on a singular hyperplane are removed and reported under `"removedPoints"` and `"reconnections"`.
...
The plan stores `WorkingPrecision` and every backend `serializedPlan`. The internal bit count is `ceil(WorkingPrecision*log2(10))+32`: 70 and 100 digits use 265 and 365 bits, and higher requests continue to grow by the same formula. Segment projection, match ratios, rotation factors, and winding/monodromy retain the current Arb precision. Execution above the planning precision is rejected with a request to rerun `MSGeneratePath`, because serialized low-precision nodes cannot recover information.
MadStree pulls every complex-affine group back to a one-variable connection. User points in one group may occupy the complex parameter plane; every point inside a node convergence disk is evaluated from that node's cached solution coefficients and need not be real-collinear. Different groups inherit only the value at their common point and never share local coefficients, so no multivariable high-dimensional Taylor ball is constructed. If all dlog letters are constant in a group, every pulled-back derivative is zero and the backend transports the resulting zero connection normally.

`de["masters"]` is the unique authority for the order of the DE rows and boundary vectors. `MSBoundaryData` generates for closed tree/time-only contexts a nested curve from the sector DAG, component base powers, slot registry, normalization and strict time rank, pulls back the complete dlog connection, computes the residue, and solves the indicial leading vector of every master over the ancestor sectors; the single-vertex massiveExternal case follows the same general route (the explicit 2411.03088 Sec.3.3 series is kept only as a test reference baseline and is not in the production path). `MSGeneratePath` stores the singular-boundary plans associated with the exact `{a,b,C}` branches; `MSEvaluatePlannedPath` executes those plans directly, transports from the regular singular point to the finite matching point, and continues to the user's ordinary target point. Production code never evaluates defining integrals or calls `NIntegrate`; these are allowed only as independent-validation oracles.
...
`masslessFull` defaults to `"Quotient"`: the whole edge contributes one shared two-state slot. Setting `"masslessRepresentation" -> "RedundantH"` at initialization keeps two h endpoints and four states; `M1/M0`, contacts, recurrence, `MSReduce`, dlog and the H/h transforms are all generated directly from these two slots. The choice is written into the context and cannot be switched per call. `RedundantH` accepts only massless `nu=1/2`.
...
The FlintNDE numerical backend (a synchronized copy of standalone FlintNDE 0.3.0: chained polyline transport, singularity jumps, local Frobenius bases, default avoid-singularity mode; the standalone FlintNDE package itself remains a general first-order matrix differential equation solver) ships inside this version directory; its default location is defined in a single relative-path variable:
...
Bare ordinary user points are returned under the `"saved"` records of `MSEvaluatePlannedPath`; `"tmp"` points participate only in the path, and `"lo"` points return only direction-dependent singular leading data. Export the saved ordinary points with
...
The five examples are inherited from v0.9 (examples 01--03 originate from v0.5); under v0.9 validation they all passed fresh runs with `Example PASSED` and exit code `0`; the v0.10 regression criterion is the development test suite and the validation record in `UPDATE_NOTES.md`.
...
- FlintNDE implements exact Lee--Moser projector balances, strictly decoupled exponential times power-log, and start-only formal asymptotics for second-order poles with simple distinct leading roots. Version 0.3.0 uses separate plan and execute schemas for ordinary polylines, direction-dependent leading data, and singular boundaries. Singularity-node construction merges residues of letters vanishing at the same point and performs singularity jumps through local Frobenius bases; resonances whose local bases cannot be constructed return structured refusals. `build_adaptive_path_plan` reports `continuation_ready` first; uncertified high poles and internal/final points requiring ramification, algebraic extensions, or general Stokes connections fail closed.

v0.10 is built by modifying v0.9 (v0.9 corresponds to the remote tag `MadStree-v0.9` and is frozen); all eleven development tests, the three original examples and the cross-package DE checks of v0.5 passed fresh serial runs (historical evidence), and the T1--T6 independent validations passed with counts `24/24`, `12/12`, `18/18`, `15/15`, `17/17`, `16/16` (reports and machine summaries live under `../../independent-validation/MadStree-v0.5-validation-*/`); the fresh v0.10 regression evidence (path-focused checks 53/53, Python adapter 10/10, all twelve Wolfram development files 221/221, Examples 01--05 at 5/5, and single-vertex cross-checks 8/8 and 10/10) is in `UPDATE_NOTES.md`.
