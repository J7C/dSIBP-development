...
用途：验证原生 MSIntegral，执行 H/h 状态变换，并适配 dSIBP 020 原生 time-only J。
边界：只在 sectorKey、timeShifts 与 stateBits 三个槽完全匹配当前 context 时允许反向转换。
***)
...
(* 输入必须先通过当前 MadStree context 验证；输出保持惰性，避免未加载 dSIBP 时求值。 *)
...
(* 反向入口逐槽验证 sector、时间幂维数和离散态；schema 不一致时返回原生 Failure。 *)
