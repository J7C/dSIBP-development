...
] := Module[{slotKeys, endpointIndex, positions, endpointBits, sharedPosition, complementCoefficient},
  slotKeys = Lookup[sourceSector["slots"], "key"];
  endpointIndex = msEventEndpointIndex[line, sourceSector, componentPosition];
  Switch[line["type"],
    "massiveFull",
      positions = (First@FirstPosition[slotKeys, {line["id"], #}] &) /@ {1, 2};
    ...
    "masslessFull",
      If[
        ...
        sharedPosition = First@FirstPosition[slotKeys, {line["id"], "shared"}];
