...
def _single_path_name(value: str, field_name: str) -> str:
...
class OutputLayout:
    ...
    def file(self, category: str, filename: str) -> Path:
        ...
        safe_filename = _single_path_name(filename, "filename")
...
def initialize_output_layout(
    ...
    *,
    ...
    script_path = Path(caller_script).resolve()
    if not script_path.is_file():
        raise ValueError(f"caller_script must identify an existing file: {script_path}")
    resolved_run_name = _single_path_name(run_name or script_path.stem, "run_name")
