...
FlintNDE 的位置只由 `$MSFlintNDERelativePath` 定义，缺省为仓库根目录相对路径 `package-FlintNDE/versions/FlintNDE-v0.1.0.dev0`。普通与奇点 JSON schema 分别为 `madstree_flintnde_transport_v1`、`madstree_flintnde_singular_transport_v1`。临时 JSON 与 Python cache 写入调用脚本目录的 `results_temp/`；成功后 JSON 删除，cache 保留复用，版本源码目录保持只读。`MSRuntimeDirectory` 的绝对路径直接展开，相对路径相对调用脚本目录解析；实现只使用单参数 `ExpandFileName`，兼容当前 Mathematica 内核。
...
```text
package-MadStree/versions/MadStree-v0.4/
  VERSION.md
  README.md
...
`../../independent-validation-task/MadStree-v0.4-independent-validation-task.md`。其中 2411.03088 两项必须显式使用
`NuConvention -> "Negative"`。v0.4 因 sector key 变化重跑的 T1--T5 均已通过；T6 与 key 无关，本轮未重跑。
