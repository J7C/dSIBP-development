...
当前版本：`v0.4`。本目录是由 v0.3 建立的新版本源码目录；接口变化见 `UPDATE_NOTES.md`。
...
```wl
packageRoot = ".../package-MadStree/versions/MadStree-v0.4";
...
results_temp/python_cache/MadStree-v0.4/
```
...
后两项由 v0.3 T4/T5 配置迁移为 v0.4 用户入口，并已改用 context 提供的位串 top key；三个 examples 本轮均已 fresh 运行且退出 `0`，独立证据仍以本版本报告为准。
...
v0.4 的 sector-key 开发检查已通过 core `49/49`、simultaneous/time-only chart `22/22`、vertex/reduction `18/18`。受 key 影响的独立验证 T1--T5 fresh 通过 `24/24`、`12/12`、`18/18`、`15/15`、`17/17`；T6 不依赖 sector key，本轮不重跑，其 v0.3 报告只作为冻结能力边界。
