...
MSReconstructEpSeries::usage = "MSReconstructEpSeries[context,ep,pointTemplate,MaximumEpPower->0] certifies the lowest integer ep power from the symbolic boundary condition and dlog DE before any numerical solve, chooses exact production and independent validation ep points, and reconstructs all Laurent coefficients through MaximumEpPower. The internal fit includes at least EpFitExtraOrder->2 extra powers and expands incrementally after failed validation while reusing solved points. EpGoalDigits->20, EpFitOrderIncrement->2, EpFitMaximumRounds->3 and ParallelTaskCount->12 are the defaults.";
