...
(* 当前拓扑输入只接受这一组字段；未知字段在归一化前统一拒绝。 *)
$msTreeSpecKeys = {
  "vertices", "lines", "name", "normalization", "graphMode", "thetaBundles"
};
$msVertexInputKeys = {"id", "energy", "timePower", "phaseSign"};
$msLineInputKeys = {
  "id", "type", "endpoints", "momentum", "nu", "skType", "sigma",
  "phaseSigns", "hankelBranches", "masslessRepresentation",
  "contactRawPower", "pinchNormalization"
};


msInputSchemaIssues[spec_Association, vertices_List, lines_List] := Module[
  {rootFields, vertexFields, lineFields, issues = {}},
  rootFields = Complement[Keys[spec], $msTreeSpecKeys];
  If[rootFields =!= {},
    AppendTo[issues, <|"code" -> "unknownTreeFields", "fields" -> rootFields|>]
  ];
  Do[
    vertexFields = Complement[Keys[vertex], $msVertexInputKeys];
    If[vertexFields =!= {},
      AppendTo[issues, <|
        "code" -> "unknownVertexFields", "position" -> position,
        "fields" -> vertexFields
      |>]
    ],
  ...
  Do[
    lineFields = Complement[Keys[line], $msLineInputKeys];
    If[lineFields =!= {},
      AppendTo[issues, <|
        "code" -> "unknownLineFields", "position" -> position,
        "fields" -> lineFields
      |>]
    ],
...
msNormalizeVertex[vertex_Association, position_Integer] := <|
  ...
  "position" -> position,
  "energy" -> Lookup[vertex, "energy", 0],
  "timePower" -> Lookup[vertex, "timePower", 0],
  "phaseSign" -> Lookup[vertex, "phaseSign", 1]
|>;
...
msNormalizeLine[line_Association, position_Integer, nuConvention_String] := Module[
  {type, endpoints, momentum, nuMagnitude, hankelOrder, formulaNu, skType, sigma, contactRawPower, pinchNormalization,
   phaseSigns, masslessRepresentation, functionSystem, hankelBranches},
  type = Lookup[line, "type", Missing["LineType", position]];
