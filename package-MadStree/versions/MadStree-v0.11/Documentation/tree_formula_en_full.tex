...
\begin{abstract}
This handbook corresponds to \texttt{package-MadStree} v0.11; time-only sectors continue to be identified by fixed-length strings in the order of the root propagators.
...
point is a transport node. Distinct segments never share local coefficients. A single
...
User points are bare coordinates or \texttt{\{coordinate,"tmp"\}}; the former are saved and
the latter are transient waypoints. Ordinary saved points in the \texttt{MSEvaluatePath} result
