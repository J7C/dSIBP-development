...
def _assign_dense_sample_points(
    ...
    不产生新的递推起点——这是 dense output 与旧 detour 路线的本质区别。
    """
        ...
        for segment_index, segment_start in enumerate(path[:-1]):
            ...
            if transition["method"] == "regular_singular_bridge":
                ...
                segment_radii.append(
                    analytic_system.nearest_singularity_distance(segment_start)
                    * radius_fraction
                )
...
def _embedded_prefix_snapshots(
    ...
    for node_index in range(1, len(snapshots)):
        ...
        if patch.get("method") == "singular_bridge":
            primary_snapshots.append(acb_mat(snapshots[node_index]))
            continue
...
def transport_path_refined(
    ...
    *,
    ...
    ``certification_mode="certified"``（缺省，与 0.1.0 行为一致）：独立双链完整
    重算。``"embedded"``：只跑 reference_order 一条链，主链结果由解系数前缀重新
    求值得到（递推前缀性质保证逐位一致），参考链开销归零；逐段截断差作为局部
    误差估计随报告返回。``sample_points`` 为段内 dense output 求值点，不改变路径。
    """
