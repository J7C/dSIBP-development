...
- ``AnalyticMatrixSystem``：黑盒矩阵函数的 Cauchy--DFT 圆周采样重建，只要求采样圆
  ...
  恒零矩阵元。
- ``PartialFractionSystem``：``A(z)=C+Σ R_j/(z-p_j)`` 型 dlog/单极点系统的极点状态
  递推，复杂度 O(PN) 次 dim×dim 矩阵--向量乘，替代 O(count) 次全矩阵求值的采样
  重建。递推全程 acb 球算术，不丢弃误差球。
"""
...
class PartialFractionSystem:
    """``A(z)=C+Σ_j R_j/(z-p_j)`` 型系统的极点--留数解系数递推。

    dlog 系统经部分分式后正是这一形式；所有极点必须是有限简单极点且互不相同。
    ``C``、``R_j``、``p_j`` 在构造时即转为 acb（保留球半径），递推中不再触碰精确
    有理对象。需要稠密 ``A_n`` 的调用方可以走 ``taylor_matrix_coefficients``，其结果
    由同一状态递推导出，与 Cauchy--DFT 路线在截断误差内一致。
    """
    ...
    def evaluate(self, point: acb) -> acb_mat:
        """直接求值 ``C+Σ R_j/(z-p_j)``，供对照与调试使用。"""

        matrix = acb_mat(self.constant)
    ...
    def solution_taylor_coefficients(
        ...
        """用极点状态递推直接生成 ``Y'=A Y`` 的解系数 ``c_0..c_order``。

        每步状态更新 ``u_j[n]=(c[n]-u_j[n-1])/(center-p_j)``、
        ``c[n+1]=(C·c[n]+Σ_j R_j·u_j[n])/(n+1)``；只含 O(极点个数) 次
        矩阵--向量乘，不做全矩阵系数卷积。递推前缀性质保证高阶运行给出的低阶
        系数与低阶运行逐位相同，是嵌入式截断认证的基础。
        """
    ...
    def taylor_matrix_coefficients(
        ...
        *,
        ...
        """生成 ``A_0..A_(order-1)``；单极点结构下按几何级数闭式展开，无需采样。
        ...
        coefficients = [acb_mat(dimension, dimension) for _ in range(order)]
        if order > 0:
