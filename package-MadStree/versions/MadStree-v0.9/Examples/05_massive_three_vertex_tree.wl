...
(* ::Chapter:: *)
(* Batch multi-point evaluation and CSV/JSON export *)

(* Fixed parameters shared by all points *)
batchFixedRules = {
  k3 -> -5 I,
  q12 -> 1, q23 -> 2,
  nu12 -> 3/4, nu23 -> 1/3,
...
(* Varying values: one row per point, in the order of batchPointSymbols *)
...
(* Reuse the already computed target point as the finite anchor, so the singular launch is shared.
   AnchorValues carries an empty placeholder if the target transport failed; the failure gate
   then reports the real reason instead of letting downstream code cascade on bad anchor values. *)
batchResults = MSBatchEvaluateTree[
  context,
  batchFixedRules,
  batchPointTable,
  {BoundaryScale -> 4, WorkingPrecision -> 32},
  {WorkingPrecision -> 32, TransportOrder -> 72,
   ReferenceTransportOrder -> 96, TargetRelativeError -> "1e-14"},
  PointSymbols -> batchPointSymbols,
  AnchorPoint -> targetRules,
  AnchorValues -> If[Head[targetValue] === Failure, {}, targetValue["values"]]
];
...
exampleGateFailures = Select[
  Join[{targetValue, batchExport}, If[ListQ[batchResults], batchResults, {batchResults}]],
  Head[#] === Failure &
];
