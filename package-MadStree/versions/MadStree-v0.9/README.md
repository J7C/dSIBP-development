...
当前版本：`v0.7`。本目录是在 v0.6 基础上修改建立（版本沿革 v0.5 -> v0.6 -> v0.7）；接口变化见 `UPDATE_NOTES.md`。
...
FlintNDE 源码随 v0.7 放在版本目录内，缺省位置只在一个相对路径变量中定义：
...
可用 `MSRuntimeDirectory -> path` 显式指定其它调用者目录：绝对路径直接使用，相对路径相对当前调用脚本目录解析，不依赖进程工作目录。成功后临时 JSON 自动删除；失败时保留输入、输出路径供诊断。Python 按缺省规则在相应 package 旁建立 `__pycache__/`，该目录及 `*.pyc` 已由 Git 忽略，可在同一 Python 版本和源码状态下复用。除该解释器缓存外，程序包源码目录不接收运行产物。
...
五个 examples 中 01--03 已从 v0.5 迁入 v0.7（v0.5 源码 fresh 运行均退出 `0`）；04、05 端到端退出 `0`，批量节导出 `results/madstree_evaluation*` 下的 `evaluation_data.csv/json`。
...
v0.7 在 v0.6 基础上修改建立（v0.6 由冻结 v0.5 复制）；v0.5 的全部 11 个开发 tests、三个 examples 与跨包 DE 检验均串行 fresh 通过（历史证据），T1--T6 独立验证计数依次为 `24/24`、`12/12`、`18/18`、`15/15`、`17/17`、`16/16`（报告和机器 summary 位于 `../../independent-validation/MadStree-v0.5-validation-*/`）。
