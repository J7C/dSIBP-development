...
v0.9: Backend failures capture the subprocess stdout/stderr into the Failure and emit a diagnostic message; PythonExecutable defaults to Automatic (MADSTREE_PYTHON override, otherwise "python"); no interpreter probing is performed anywhere.
***)
...
msExecuteFlintNDEAdapter[inputData_Association, pythonExecutable_, runtimeDirectory_String] := Module[
  {cacheDirectory, cacheFile, cached, transportDirectory, identifier, inputFile,
   adapterFile, logFile, command, commandText, process, imported, finalInput, logTail},
  ...
  If[! FileExistsQ[cacheFile],
    Message[MSFlintNDETransport::backendLaunchFailed, pythonExecutable, logTail];
  ...
  If[process["ExitCode"] =!= 0 || Lookup[imported, "status", None] =!= "success",
    Message[MSFlintNDETransport::backendRunFailed, pythonExecutable, Lookup[imported, "error", logTail]];
...
MSFlintNDETransport[
  ...
  If[boundaryKind === "singularFrobenius",
    ...
    singularImported = msExecuteFlintNDEAdapter[
      inputData, msResolvePythonExecutable[OptionValue[PythonExecutable]], runtimeDirectory
    ];
