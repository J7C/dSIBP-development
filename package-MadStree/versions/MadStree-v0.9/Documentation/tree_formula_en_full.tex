...
\title{MadStree v0.7: tree formula}
...
\begin{abstract}
This handbook corresponds to \texttt{package-MadStree} v0.7; in this version the time-only sectors are identified by fixed-length strings in the order of the root propagators.
...
\end{equation}
where $q_b$ keeps the user-supplied name and definition. A pure vertex energy and a massless cross/external branch can both be such a block; the same vertex can be associated with several $b$. Viewing $E_b$ as a vector in the one-dimensional space $\cE_b\simeq\mathbb C$, its time generator is
\begin{equation}
 Q_b=[\ii\chi_bq_b],
 \qquad I_{\cE_b}=[1].
...
For each vertex choose its pure vertex-energy exponential
\begin{equation}
...
Fix a sector $s$, let $N_s$ be the number of two-dimensional slots in the sector and $c_s$ the number of time components after contact. Following the binary representation of 2401, write a state as
\begin{equation}
...
MadStree fixes the representation once at initialization for each \texttt{masslessFull}. The default \texttt{masslessRepresentation -> "Quotient"} uses one shared two-state \texttt{masslessShared} slot; explicitly choosing \texttt{"RedundantH"} uses two \texttt{masslessEndpointH} slots, keeping the four states $(00,01,10,11)$ in the ordered-endpoint sense. The latter input accepts only $\nu=1/2$ and equivalently accepts \texttt{functionSystem -> "h"} as an initialization alias. Once the context is built, the recurrence, contact, dlog and numerical layers cannot switch representations call by call.
...
The previous text used the $\tau^{\nu_0}$ convention of the paper. The only public integral object of the package is the normalized object
\begin{equation}
 J_s=\cN_sI_s,
\end{equation}
with the time power written as $(-\tau_v)^{A_{v,s}}$, so
\begin{equation}
...
\end{verbatim}
where \texttt{k0} and \texttt{A0} are the pure vertex-exponential energy and the $(-\tau)$ base power; the remaining positions give, one by one, the momentum, $\nu=|\nu|$ and Hankel branch of each h block. The explicit form may instead be given by \texttt{k0/timePower/hBlocks/exponentialBlocks}. Each pure exponential block is a one-dimensional factor that only shifts the effective energy; each h block is a two-dimensional massive endpoint slot. The context can therefore call \texttt{MSFormulaMatrices}, \texttt{MSMasterIntegrals}, \texttt{MSDLogDE}, \texttt{MSBoundaryData} and \texttt{MSEvaluateVertexFamily} unchanged. \texttt{NuConvention} is chosen only at initialization; afterwards all these interfaces read the context fixedly.
...
The theorem above also fixes the scope of automation. Singularities that contain no $K_v$ at all are already listed explicitly in the DE letters; they are not obstacles to be solved by this boundary producer, and the package does not need to compute divergent values on the singularities. The default automatic path accepts only ordinary points and bypasses them; if the user deliberately places the end of a path on such a singularity, one should use the DE given by the package and choose the blow-up oneself. If an extra gauge transformation introduces new denominators not of the form of Eq.~\eqref{eq:general-affine-K-divisor}, that transformation itself cannot automatically inherit the proof above.
...
\subsection{Why auxiliary vertex energies cannot be run to zero by default}
...
The recommended flow is therefore: first choose the preferred chart according to Eq.~\eqref{eq:preferred-rank-from-point}, obtain the same-ordered vector from the boundary of Eq.~\eqref{eq:nested-blowup-coordinates}, take a small but finite ordinary point inside the chart, and then automatically bypass all letter hypersurfaces to reach the user-preferred ordinary point and the final target point. If auxiliary vertex energies are to be removed, set them to zero one at a time and redo the ordinary-point certificate for all sectors at every step. Only when the certificate passes is ``run this auxiliary energy to zero first, then run the other parameters'' a legal simplification; once the zero point is singular, the automatic flow stops and returns the DE letters at that point, and the user decides the subsequent blow-up.

\subsection{The current 2411 infinity boundary and FlintNDE transport}

The production entry point does not compute the defining integral directly at a finite Euclidean ordinary point. \texttt{MSBoundaryData} uses the $K_v\to\infty$ data of \cite{Chen:2024jms} as the only boundary source. The single-vertex massive-external family keeps the explicit multi-variable series optimization of Eqs.~(3.44)--(3.46); the other closed tree/time-only contexts are no longer dispatched by vertex count, master count or graph name, but uniformly consume the sector DAG, components, slot registry, normalization, state order and strict rank.
...
\end{align}
$N_s$ is the sector normalization, $\Phi_s$ the initialized massive-contact phase, $\Xi_{s,\bm a}$ collects the shared-massless chamber sign and the RedundantH normalization, and $E_{sC,\bm a}$ is the product of all Hankel endpoint coefficients of the component. All quantities are taken from the current sector metadata; one must not guess the child powers or slots from the root sector.
...
\texttt{MSEvaluateTree} first calls the regular-singular Frobenius module of FlintNDE to transport each exact branch from $t=0$ to a finite matching point, multiplies back physical weights such as the Gamma functions and sums, and then pulls back the dlog connection along an affine parameter from the matching point to the user's ordinary point. \texttt{MSEvaluateVertexFamily} reuses the same chain for \texttt{MSInitVertexFamily} contexts. The backend path is controlled only by a single configuration variable relative to the project root; run files and the Python cache are written to \texttt{results\_temp/} in the calling script's directory, not to the package source directory.

The path save points are given by the Wolfram option \texttt{FlintNDESavePoints}. The complete input for an ordinary route is
\begin{center}
 \texttt{FlintNDESavePoints ->}\\
 \texttt{\{\{0,"save"\},\{1/2,"save"\},\{1,"save"\}\}}.
\end{center}
When singular and ordinary segments coexist, they can be listed separately with the keys \texttt{"singular"} and \texttt{"ordinary"}. The interface does not accept point names or a third field. The backend writes an independent JSON immediately at every marked point in the calling directory under \texttt{results/flintnde\_save\_points/run-UUID/}, and only after the whole chain succeeds does it produce the summary table \texttt{madstree\_flintnde\_save\_points.json}. Ordinary-point records save the coordinates and the complete master-integral vector; a regular-singular starting point saves the reusable Frobenius $\{a,b,C\}$ and cannot fabricate a ``function value at the singularity.'' If the saved coordinates on the path belong to an exponential-type higher pole that FlintNDE has certified and can bridge in both directions, the backend instead saves $\{\phi,a,b,C\}$, meaning $\exp(\phi(z))z^a(\log z)^bC$; all negative-power coefficients of \texttt{phi} are derived from the differential equation and verified exactly. A formal branch lacking a Stokes connection can only save its starting point, and cannot be used as an intermediate or final point. If a later segment fails, the already completed point-by-point files are kept, but no complete summary is written.
