...
约束：只调用 v0.13 现行 MSEvaluatePath；A/B 各自一次 cold Python 进程。
***)
...
madStreeRoot = ExpandFileName@FileNameJoin[{validationRoot, "..", "..", "versions", "MadStree-v0.13"}];
...
reportFile = FileNameJoin[{validationRoot, "000_MadStree-v0.13-validation-01-flintnde-planned-vs-user-nodes-report.md"}];
...
checks = <|
  "version013" -> MadStree`Private`$MadStreeVersion === "0.13",
...
evidence = <|
  "schema" -> "madstree_v0.13_planned_vs_user_nodes_validation_v1",
  "status" -> status,
...
report = StringRiffle[{
  ...
  "",
  If[status === "passed", "本任务通过。", "本任务失败；失败键见 `results/summary.wl`，不得据此认证 v0.13。"]
}, "\n"];
...
Print["MadStree v0.13 independent validation: ", passedCount, "/", Length[checks], " ", status];
