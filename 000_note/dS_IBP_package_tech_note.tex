...
当前 017 以既有 loop 物理核心为只读基线，已实现 momentum seed 的传播子项、shrunk-line $bS$ 项、z/ISP 吸收、massive/massless building-block 导数与 EOM/quotient canonical 门禁；time seed 已包含顶点幂次、外部相位、双端点 regular 项、massive/massless theta-boundary shrink、目标 sector coincident 抵消与 sector prefactor normalization，并通过标准 \texttt{dSIBP`} context 提供原子与高层接口。massive 导数与 shrink 分别只读取由 $A_T/W_T$ 编译的数据。Kira serializer 只转换经过这些门禁的后端中立线性系统，importer 只验证并读取外部完整 reduction；二者都不替代任何前端关系或运行 reduction。
...
\end{equation}
式~\eqref{bundleMergedAShift}--\eqref{bundleFinalJIndexForm} 中，所有选中线只触发一次顶点合并，但其整数与 zero-point shift 分别求和；各 shrunk line 的 $bS/bS0$ 仍逐线记录。未选中的 full lines 在合并后成为 coincident：massless odd endpoint state 立即置零，massiveFull 应用 $\{b,1,0\}\to\{b,0,1\}$；它们不再产生 theta boundary。sector 由这些非零 contact 事件做 BFS 得到，不能用 full-line 幂集代替。
...
\texttt{DSSeeds} 完整遍历离散 $n_i=0,1$ 并先执行 EOM、symmetry 与 canonical，连续指标保持 general。它同时返回：
\begin{itemize}
...
\end{equation}
该区间相对用户包络缩小；对其中全部 seed 作用该组的全部 shift 后，所得关系的指标外包络刚好回到 $[L_x,U_x]$。不得误用并集外包络 $[L_x-\max\Delta_x,U_x-\min\Delta_x]$，后者会无故扩大撒点域。若用户包络宽度小于某组 shift 跨度，式~\eqref{seedEnvelopeInverse017} 为空；程序 warning 并保留结构化 \texttt{emptySeedRangeGroups}，但该结果的 complete IBP capability 为 false，不能作为完整后端约化系统。

实际生成顺序固定为：按式~\eqref{seedEnvelopeInverse017} 构造候选整数点，\emph{在生成元作用前}按当前 sector 的 parity 筛点，再把数值点代入 general seed，随后执行 EOM/symmetry/canonical 与用户坐标投影。生成后的 parity certificate 只检查关系，不删除关系中的积分，也不把不满足 parity 的积分替换为零。\texttt{candidatePointCount}、\texttt{parityAcceptedPointCount}、逐组 shift/range 和空域状态均写入返回数据。用户包络唯一决定规模；package 不设置 seed 数、离散态数或方程数上限。
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item 对每个输入组使用 \texttt{DSSeeds} 已完整枚举的全部离散 $n=0,1$ 模板；生产路径不允许离散抽样，也不允许残留符号 $n$
...
本 package 默认不安装、配置或运行 Kira/Rational Tracer，也不保存本机 Kira/Fermat 路径。017 的 importer 可读取并验证用户在 package 外生成的完整 reduction 与 master-integral 结果；缺失完成标记、映射或来源 manifest 时必须拒绝继续。
...
其中 $P_e$ 为外动量组合，$P_e^2$ 和 $q_l \cdot P_e$ 中的外动量点积均以当前动力学坐标替代；017 缺省写成 $ss_{ij}^{,2}$，也可由用户重定义。因此 $z_e$ 是标量积变量 $\vec{s}$ 的线性函数加上常数项。
...
\end{equation}
而不是单个指标积分。公开接口 \verb|ds[expr,var]| 使用 family 初始化时的外部变量名；017 缺省为根号坐标的 API 短名，并严格实现
\begin{equation}
...
017 继续保持 ``package 不运行 reduction'' 的边界，但允许读回用户已经运行完成的 Kira 全套输出。导出端的 \texttt{linearData}、积分编号映射和 convention manifest 必须与读回结果同源；否则 reduction rule 即使语法可读，也不能用于微分方程。
...
树图缺省、圈图可选的 \texttt{ibpMode->"timeOnly"} 不构造残缺的 loop family。所有 active lines 统一标为 \texttt{fixedCoefficient}，公开对象仍只有
\begin{equation}
 J[\{a_v\}_v,\{P_e^{\rm fixed}\}_e,\{\}],
 \qquad
 P_e^{\rm fixed}\in
 \bigl\{\{\texttt{"F"},n_{e1},n_{e2}\},
 \{\texttt{"F"},0,0\},\{\texttt{"F"}\}\bigr\}.
 \label{directPureTimeJ}
\end{equation}
full massive/massless 线使用第一种三槽，masslessCross 使用固定零态，shrunk line 使用单槽；root line 位置始终保留。全部 active line 模长纳入 \texttt{independentExternalMomenta} 审计，且没有 momentum generator、$z$/ISP 闭合或 $b/b_S$ 指标。\texttt{DSSeeds} 完整枚举允许的端点态并立即应用 EOM/quotient/canonical。论文 vertex basis 只作为 Private massive-only 适配器服务于 \texttt{DSTreeSeeds}、\texttt{repIterative}、naive time-IBP/DE 与直接 dlog DE；结果必须映回同一公开表示并使用同序 master 列表比较。含 massless full line 的公式型 tree 接口在 quotient basis 重新推导前返回 \texttt{PendingRederivation}。
...
fixed line 的模长幂不进入式~\eqref{v017FixedThreeSlotSchema} 的第一槽，而属于每个 sector 的 normalization。初始化按稳定 fixed-line 顺序保存
\begin{equation}
 \{e_i\},\qquad \{\kappa_i\},\qquad \{p_i\},\qquad
 \{u_i\},\qquad \{(u_i^{\rm int},u_i^{0})\},\qquad c_s,
 \label{v017SectorPrefactorLists}
\end{equation}
分别对应 line indices、稳定内部 parameter keys、当前输出坐标的 parameter list、总 power list、整数/zero-point 分解及常数因子。metadata 不保存已经乘开的 $p_i^{u_i}$；唯一 materializer 在需要时构造
\begin{equation}
 N_s=c_s\prod_i p_i^{u_i},\qquad J_s=N_s I_s.
 \label{v017SectorNormalization}
\end{equation}
变量重定义只更新式~\eqref{v017SectorPrefactorLists} 的当前 parameter list，并保留 $e_i\leftrightarrow\kappa_i$ 映射。由此
\begin{equation}
 \partial_xJ_s=(\partial_x\log N_s)J_s
 +\sum_t c_{st}{N_s\over N_t}J_t,
 \qquad
 \partial_xI_s=\sum_t c_{st}I_t.
 \label{v017NormalizedDerivative}
\end{equation}
所以 prefactor 虽然逻辑上属于 $J_s$，其导数和跨 sector 比值仍必须由 \texttt{ds/DSDE} 调取。atomic massless lower sector 若 $N_t$ 已含 $s_E^{-\beta}$，contact 项不得再次外乘同一因子。
...
\end{equation}
且 source/target sector 中该线的 prefactor 分母指数分别为 $B_{s,e}$、$B_{t,e}$，则 normalized target 的模长系数唯一为
\begin{equation}
 c^{\rm norm}_{s\to t}
...
017 沿用的缺省 sector zero-point 传播为 $B_{t,e}-B_{s,e}=z_e$，所以式~\eqref{v017FixedShrinkCoefficient} 化为 $\mathcal C_e r_e^{-s_e}$：只通过 zero point 本身吸收 $z_e$，不额外吸收整数 $s_e$。用户若合法改写 source 或 target zero point，式~\eqref{v017FixedShrinkCoefficient} 自动改变 reduction/DE coefficient。package 不提供 \texttt{fixedShrinkConvention}、\texttt{fullContactPower} 或其它平行吸收状态；若需进一步吸收剩余显式模长幂，由用户后续选择 master/basis 处理。
...
\end{tabular}
\end{center}
h/H 另有 $\mathcal C_e=(4i/\pi)e^{\pi\operatorname{Im}\nu_e}$；massless 没有 Wronskian prefactor，端点 contact 的 $-2/+2$ 仍单独保留。初始化 metadata 对每个 fixed transition 保存 $B_s$、$B_t$、zero-point override 来源、compiled $(s,z)$ 与剩余 exponent $s+z-(B_t-B_s)$，从而可由 $N_s/N_t$ 唯一重建约化系数。
