...
参考 FIRE7 和 LiteRed 的方法，对 $L$ 圈积分，016 要求用户显式给出有序的 \texttt{loopExternalMomenta} 基 $\{k_1,\ldots,k_K\}$；程序不按名字或 line 首次出现自动选择角色。该基的平方 Gram 原子为 $x_{ij}=\operatorname{sp}(k_i,k_j)$，公开变量为 $\sqrt{x_{ij}}$。另一个显式列表 \texttt{independentExternalMomenta} 只声明实际无圈 line/phase 的独立模长；这些对象不计入 $K$、loop IBP 生成元或 ISP 空间，也不自动产生彼此交叉点积。
...
\texttt{sp[p,r]} 是用户口 scalar-product convention，并具有 \texttt{Orderless} 属性。所有含圈 line momentum 与 ISP 参数必须由 \texttt{loopMomenta} 和 \texttt{loopExternalMomenta} 线性张成；实际无圈 line/phase 模长必须由 \texttt{independentExternalMomenta} 完备声明。016 对 loop Gram 使用短名 \texttt{ssij}，对独立无圈模长使用 \texttt{sE1,sE2,...}。类别总数超过 9 时按总数位宽补零。与两类动量声明都无关的相位标量仍可写成 \texttt{ke[i]}。
...
\begin{enumerate}
  ...
  \item \texttt{numericRules} 应覆盖全部外部不变量与顶点能量符号；016 缺省给根号坐标赋值，例如 \verb|ss11 -> value| 或 \verb|sE1 -> value|，程序在内部映射为平方值。显式旧规则或自定义名保持其声明语义；独立顶点能量写 \verb|ke[3] -> value|。
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{外--外}：$k_i \cdot k_j$ 定义为独立符号，不保持矢量点积结构；016 未指定 \texttt{KinematicRules} 时，按 \texttt{loopExternalMomenta} 的位置生成 $\operatorname{sp}(k_i,k_j)\to ss_{ij}^{,2}$。在 IBP 生成中这些量作为常数参数处理。
\end{itemize}
...
其中 $P_e$ 为外动量组合，$P_e^2$ 和 $q_l \cdot P_e$ 中的外动量点积均以当前动力学坐标替代；016 缺省写成 $ss_{ij}^{,2}$，也可由用户重定义。因此 $z_e$ 是标量积变量 $\vec{s}$ 的线性函数加上常数项。
...
微分方程 seed 需要把 $\partial_x J$ 写回同一积分族。016 将独立变量分为三类：由 \texttt{loopExternalMomenta} 完整 Gram 坐标得到的模长、\texttt{independentExternalMomenta} 声明且在 topology 中实际出现的无圈动量模长，以及与任何动量向量都无关的独立标量能量。第一类沿用标准外动量矢量导数；第二类对绑定的 line momentum 作标量径向导数并同时微分相位；第三类只按显式系数与顶点相位依赖求导。构造 $D_{ij}$ 时，$i,j$ 只遍历 \texttt{loopExternalMomenta}。
...
\end{equation}
完整 $K^2$ 个 $D_{ij}$ 通常有零空间，所以 $c^{(a)}_{ij}$ 不唯一。原子实现选择上三角 external-vector basis 给出一组 canonical 解。016 不重写该分解；对直接模长
\begin{equation}
...
\end{equation}
而不是单个指标积分。公开接口 \verb|ds[expr,var]| 使用 family 初始化时的外部变量名；016 缺省为根号坐标的 API 短名，并严格实现
\begin{equation}
...
\end{verbatim}
把每个顶点的 $a_e$ 迭代到指定终点；\texttt{Automatic} 表示全零终点。显式终点的长度必须等于顶点数，且每段步数必须是可判定整数。多 sector 情形按 contact-reachable sector DAG 从低到高处理，避免把含 source 的非齐次关系当作齐次 replacement repeated。016 中凡 sector 含 theta/contact source，raw 与 sector-tagged 迭代均从同一个 direct pure-time seed 生成单步关系；旧三槽 loop 投影只作交叉验证。这个要求保证用户显式给出的 \texttt{treeEnergy} 不会被 fixed-line notation \texttt{sEi} 替换。
...
\section{016 图论、fixed line 与 pure-time 表示}
...
\begin{equation}
 P_e=
 \begin{cases}
  \{b_e,n_{e1},\ldots\},&e\text{ 为 full cycle line},\\
  \{b_{S,e},\ldots\},&e\text{ 为 shrunk cycle line},\\
  \{n_{e1},\ldots\},&e\text{ 为 fixed massive/massless full line},\\
  \{\},&e\text{ 为不带离散端点态的 fixed 或 cross line}.
 \end{cases}
 \label{cycleFixedPackSchema}
\end{equation}
因此不能由 pack 第一槽或长度猜测 $b$；必须读取 sector metadata。
...
树图缺省、圈图可选的 \texttt{ibpMode->"timeOnly"} 不构造残缺的 loop family。所有 active lines 统一标为 \texttt{fixedCoefficient}，但表示必须保留实际离散态。若不存在未缩并的 \texttt{masslessFull} line，可使用
\begin{equation}
 J[\{\{a_v,n_{e_1v},\ldots,n_{e_pv}\}\}_v]
 \label{directPureTimeJ}
\end{equation}
的 vertex-pack 表示。若存在未缩并的 \texttt{masslessFull} line，则有向状态 $n_e\in\{0,1\}$ 属于整条 line，不能无歧义地分配给某个顶点；此时必须使用
\begin{equation}
 J[\{a_v\}_v,\{\{n_e\}\}_{e\,{\rm full}}\cup\{\{\}\}_{e\,{\rm shrunk/cross}},\{\}]
 \label{directPureTimeMasslessJ}
\end{equation}
的三槽 fixed-line 表示。两条路线都把全部 active line 模长纳入 \texttt{independentExternalMomenta} 审计，且都没有 momentum generator、$z$/ISP 闭合和 $b/b_S$。\texttt{DSSeeds} 根据 line state 自动选择表示；\texttt{DiscreteMode->"all"} 枚举完整 $n=0,1$，不引入私有状态参数。只有式~\eqref{directPureTimeJ} 的 direct vertex-pack 路线进入 \texttt{DSTreeSeeds}、\texttt{repIterative} 和 tree DE；式~\eqref{directPureTimeMasslessJ} 由 canonical time seed 与 \texttt{DSLinear} 直接消费。\texttt{repIterative}、naive time-IBP/DE 与直接 dlog DE 必须使用同序 master 列表比较。
