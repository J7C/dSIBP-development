...
树图缺省、圈图可选的 \texttt{ibpMode->"timeOnly"} 不构造残缺的 loop family。所有 active lines 统一标为 \texttt{fixedCoefficient}，使用
\begin{equation}
 J[\{\{a_v,n_{e_1v},\ldots,n_{e_pv}\}\}_v]
 \label{directPureTimeJ}
\end{equation}
的 vertex-pack 表示，并把全部 active line 模长纳入 \texttt{independentExternalMomenta} 审计。此模式没有 momentum generator、$z$/ISP 闭合和 $b/b_S$；\texttt{DSTreeSeeds} 直接生成 pure-time sector rules，\texttt{repIterative} 的 source-aware 首步也消费同一 direct relation，旧 loop 投影只作为独立交叉验证。\texttt{repIterative}、naive time-IBP/DE 与直接 dlog DE 必须使用同序 master 列表比较。
