...
\texttt{sp[p,r]} 是 006 起的用户口 scalar-product convention，并具有 \texttt{Orderless} 属性；内部编号变量 \texttt{qq/qk/kk} 只用于程序线性代数，不要求用户输入。顶点相位能量若不由 \texttt{externalMomenta} 的标量积关系决定，应作为独立 \texttt{ke[i]} 参数；例如 $|ke_1+ke_2|$ 应另记为 \texttt{ke[3]}，不能自动等同于 \texttt{ke[1]+ke[2]}。这些 \texttt{ke[i]} 不进入 \texttt{externalMomenta} 或 ISP 完备性坐标；外腿能量参数之间不生成点积关系。若某个顶点能量属于 \texttt{externalMomenta} 张成的空间并应复用变量，应在 \texttt{vertexEnergies} 中显式写成外部不变量表达式，例如 \verb|Sqrt[s11]| 或 \verb|Sqrt[sigW]|。
...
\begin{enumerate}
  ...
  \item \texttt{numericRules} 应覆盖全部外部不变量与顶点能量符号；外部不变量推荐写输出变量名，例如默认 \verb|s11 -> value| 或用户自定义 \verb|sigW -> value|，\verb|sp[k_i,k_j] -> value| 仅作为输入兼容形式；独立顶点能量写 \verb|ke[3] -> value|；若顶点能量写成 \verb|Sqrt[s11]| 这类外部不变量表达式，则只需给对应外部不变量数值
\end{enumerate}
