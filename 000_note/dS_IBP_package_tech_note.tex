...
\begin{enumerate}
  ...
  \item ISP 之间线性无关
  \item ISP 数目 $= N_{\text{sp}} - E_{\text{prop}}$（$E_{\text{prop}}$ 为独立传播子数）
\end{enumerate}
...
\begin{equation}
  ...
  \label{zLinear}
\end{equation}
其中 $M$ 是 $E_{\text{prop}} \times N_{\text{sp}}$ 矩阵（$E_{\text{prop}}$ 为内线数），$\vec{c}$ 的分量为各 $P_e^2$（用 $k_{ij}$ 表示）。

当传播子数等于独立标量积数（$E_{\text{prop}} = N_{\text{sp}}$，即无 ISP）时，$M$ 为方阵且可逆，逆变换为
\begin{equation}
  \vec{s} = M^{-1}(\vec{z} - \vec{c}).
  \label{zInverse}
\end{equation}

当存在 ISP 时，$M$ 不是方阵，逆变换需补充 ISP 的定义关系（ISP 直接用 $\vec{s}$ 中的分量表示）。
