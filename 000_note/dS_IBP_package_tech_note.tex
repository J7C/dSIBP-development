...
\begin{enumerate}
  ...
  \item \texttt{numericRules} 应覆盖全部外部不变量与顶点能量符号；外部不变量推荐写输出变量名，例如默认 \verb|s11 -> value| 或用户自定义 \verb|sigW -> value|，\verb|sp[k_i,k_j] -> value| 仅作为输入兼容形式；顶点能量写 \verb|k15 -> value|
\end{enumerate}
