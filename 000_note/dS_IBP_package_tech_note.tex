...
当前 012 已实现 momentum seed 的传播子项、shrunk-line $bS$ 项、z/ISP 吸收、massive/massless building-block 导数与 EOM/canonical 门禁；time seed 已包含顶点幂次、外部相位、massive 端点导数、massless 有向 $0\leftrightarrow1$ regular 项、massive/massless theta-boundary shrink 以及目标 sector 的 coincident 抵消，并提供公开原子 IBP/表示转换接口。massive 导数与 shrink 分别只读取由 $A_T/W_T$ 编译的数据。Kira serializer 只转换经过这些门禁的后端中立线性系统，不替代任何前端关系。
...
本 package 默认不安装、配置或运行 Kira/Rational Tracer，不保存本机 Kira/Fermat 路径，也不导入 reduction 或 master-integral 结果。
...
微分方程 seed 需要把 $\partial_x J$ 写回同一积分族。012 将独立变量分为两类：一类是只进入顶点相位的外腿能量参数，另一类是由外动量 Gram 坐标给出的外不变量。后一类沿用标准 Feynman 积分微分方程中的做法：先用外动量矢量导数表示不变量求导，再把矢量导数作用到被积函数并约化为指标移位。这里的 $i,j$ 都只遍历 \texttt{externalMomenta} 中声明的独立外动量向量。
...
\subsection{外不变量的矢量导数分解}

令外不变量坐标为
\begin{equation}
  x_a\in\{k_i\cdot k_j\}_{1\le i\le j\le K},
\end{equation}
或其用户命名版本。定义外动量矢量导数算符
\begin{equation}
...
因此对指定坐标 $x_a$ 求导时，先在约束坐标 $\{x_b\}$ 上解线性方程
\begin{equation}
...
\end{equation}
完整 $K^2$ 个 $D_{ij}$ 通常有零空间，所以 $c^{(a)}_{ij}$ 不唯一。012 的默认实现选择上三角 external-vector basis 给出一组 canonical 解。实际返回接口是
\begin{center}
...
每个 $D_{ij}$ 作用到被积函数时，传播子、massive/massless building block 与 ISP 的求导都按同一链式法则处理。若某个顶点能量在 \verb|vertexEnergies| 中写成外不变量函数，例如 \verb|Sqrt[s11]|，则 $D_{ij}$ 也会按式~\eqref{externalVectorGramAction} 作用到这部分相位能量。
...
\end{equation}
因此 $F=\sqrt{s_{11}}$ 给出普通的 $1/(2\sqrt{s_{11}})$ 链式因子。把每个坐标直接替换成 $D_{ij}x_a$ 只对线性 $F$ 成立，不能用于顶点能量或其它非线性动力学量系数。
...
\end{equation}
而不是单个指标积分。公开接口 \verb|ds[expr,sij]| 使用 family 初始化时的外部变量名，并严格实现
\begin{equation}
...
\end{equation}
实现时先把各个 $J_r$ 替换为惰性 token，让 Mathematica 的 \texttt{D} 只作用于显式系数；随后逐项调用单积分导数核心，合并后再统一执行 EOM、symmetry 与 canonical。接口拒绝内部 \texttt{kk[i,j]} 名称及 $J_iJ_j$ 等非线性输入，输出恢复到 \texttt{sij} 或用户自定义的外部表示。
