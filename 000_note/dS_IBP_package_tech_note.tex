...
参考 FIRE7 和 LiteRed 的方法，对 $L$ 圈积分，独立外动量 $E-1$ 个（动量守恒 $\sum p_i = 0$），完备的 IBP 生成元为：

\begin{equation}
  \mathcal{O}_{l,v} = \frac{\partial}{\partial q_l^\mu} \cdot v^\mu, \quad l \in \{1,\ldots,L\}, \quad v^\mu \in \{q_1^\mu, \ldots, q_L^\mu, k_1^\mu, \ldots, k_{E-1}^\mu\}
\end{equation}
...
\begin{itemize}
  ...
  \item \textbf{外动量（special）}：$v = k_j$，共 $L(E-1)$ 个。$\partial \cdot k_j = 0$。
\end{itemize}

\textbf{交叉 IBP 的必要性}：$L \geq 2$ 时，仅对角 IBP 不足以将所有积分约化到 master integrals。交叉 IBP $\partial_{q_l} \cdot q_m$ 提供不同圈动量之间的关系，是完备约化系统所必需的。

\textbf{验证}：独立标量积数目 $N_{\text{sp}} = L(L+1)/2 + L(E-1)$ 恰好等于 $N_{\text{IBP}}$，确认生成元集合与标量积空间维度匹配。
...
\begin{enumerate}
  ...
  \item ISP 之间线性无关，且当前主线要求 ISP 直接是某个 \texttt{qq[i,j]} 或 \texttt{qk[i,j]} 标量积变量
  \item \texttt{zExprs} 数量等于非 ISP 标量积数量，即 $\# z_e = N_{\text{sp}}-\#\text{ISP}_{\text{direct}}$
  \item 数量闭合后必须能实际反解出 \texttt{repSP2Z}；重复或退化传播子动量会触发 \texttt{scalarProductCoordinateSolveFailed}
  \item \texttt{numericRules} 应覆盖全部外部不变量 \texttt{kk[i,j]}；缺失只触发 \texttt{numericRulesMissingExternal\allowbreak Invariants} warning，不阻止解析 seed 生成
\end{enumerate}

这一步验证用户初始化给出的 \texttt{z/ISP} 坐标系是否闭合；程序不自动从冗余传播子中选择独立子集，也不替用户重定义 family。
验证通过后，才进入 IBP 生成步骤。
...
\begin{itemize}
  ...
  \item \textbf{圈--外}：$q_l \cdot k_j$，共 $L(E-1)$ 个。
  \item \textbf{外--外}：$k_i \cdot k_j \equiv k_{ij}$（定义为独立符号，不保持矢量点积结构），共 $(E-1)(E-2)/2$ 个。在 IBP 生成中作为常数参数处理。
\end{itemize}

总独立标量积数目为
\begin{equation}
  N_{\text{sp}} = \frac{L(L+1)}{2} + L(E-1).
  \label{Nsp}
\end{equation}

将所有独立标量积排成向量 $\vec{s}$，其分量依次为 $\{q_1^2,\, q_1 \cdot q_2,\, \ldots,\, q_L^2,\, q_1 \cdot k_1,\, \ldots\}$。
...
其中 $P_e$ 为外动量组合，$P_e^2$ 和 $q_l \cdot P_e$ 中的外动量点积均以 $k_{ij}$ 符号替代。因此 $z_e$ 是标量积变量 $\vec{s}$ 的线性函数加上常数项。
...
\begin{equation}
  ...
  \label{zLinear}
\end{equation}
其中 $M$ 是 $N_z \times N_{\text{sp}}$ 矩阵（$N_z$ 为用户输入的传播子平方方程数），$\vec{c}$ 的分量为各 $P_e^2$（用 $k_{ij}$ 表示）。
...
\begin{align}
  ...
  z_2 &= Q_2^2 = q_1^2 - 2\, q_1 \cdot k + k_{11}, \label{z2bubble}
\end{align}
其中 $k_{11} \equiv k \cdot k$。矩阵形式为
\begin{equation}
  \begin{pmatrix} z_1 \\ z_2 \end{pmatrix}
  = \begin{pmatrix} 1 & 0 \\ 1 & -2 \end{pmatrix}
  \begin{pmatrix} q_1^2 \\ q_1 \cdot k \end{pmatrix}
  + \begin{pmatrix} 0 \\ k_{11} \end{pmatrix}.
  \label{bubbleMatrix}
\end{equation}
...
\begin{align}
  ...
  q_1 \cdot k &= \frac{1}{2}(z_1 + k_{11} - z_2). \label{q1kInv}
\end{align}

\textbf{标量积 $q_1 \cdot Q_e$}（在 IBP 链式法则中直接使用）：
\begin{align}
  q_1 \cdot Q_1 &= q_1^2 = z_1, \label{q1Q1} \\
  q_1 \cdot Q_2 &= q_1 \cdot (q_1 - k) = q_1^2 - q_1 \cdot k = z_1 - \frac{1}{2}(z_1 + k_{11} - z_2) = \frac{1}{2}(z_1 + z_2 - k_{11}). \label{q1Q2}
\end{align}

这些关系表明 $q_l \cdot Q_e$ 可以表示为 $z$ 变量和外动量不变量 $k_{ij}$ 的线性组合。
...
\textbf{指标移位规则}：$q_l \cdot Q_e$ 用式~\eqref{zInverse} 或显式计算表示为 $z$ 变量的线性组合后，每一项形如 $\alpha\, z_e$（$\alpha$ 为常数或 $k_{ij}$ 的函数）。$z_e$ 作用在传播子 $z_e^{-(b_e+b0_e)/2}$ 上的效果为
\begin{equation}
...
\textbf{实现步骤}：
\begin{enumerate}
  \item 用替换规则将每个 $q_l \cdot Q_e$ 表示为 $\{z_e\}$ 和 $\{k_{ij}\}$ 的线性组合。
