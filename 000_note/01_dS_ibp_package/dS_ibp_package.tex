...
\abstract{本手册说明 dS IBP package 的费曼规则、统一积分表示、massive/massless 指标约定、time 与 loop-momentum IBP、独立变量微分方程 seed、EOM/缩并 canonical、拓扑输入、公开 API、Kira 接口和 tree vertex family。当前主线为 018：所有公开积分统一写成 \texttt{J[aList,linePacks,ispList]}；用户显式分别声明任意命名的 \texttt{loopExternal\allowbreak Momenta} 与 \texttt{independentExternal\allowbreak Momenta}，程序验证结构圈数、routing、声明完备性和动力学坐标，不自动猜角色。前者使用完整 \texttt{ssij} 根号 Gram 坐标，后者只为用户声明的实际无圈模长生成 \texttt{sE1,sE2,...}。package 只生成、序列化和验证关系，不运行 reduction。}
...
本 package 生成 dS 圈图的 canonical IBP seed，并把关系转换为后端中立的 \texttt{linearData}。当前稳定主线为模块化 018，支持 topology-driven 的任意圈数、任意拓扑以及 massive/massless 混合内线；标准入口和冻结单文件兼容入口分别为
\begin{verbatim}
000_code/018_dSIBP/
independent-benchmark/package/package_018.wl
\end{verbatim}
010--017 是只读历史/基线版本。018 继承标准 context、初始化 metadata、完整 time/loop-momentum 生成元、massive EOM、共同-theta odd-subset contact、contact 可达 sector、跨 sector coincident canonical、Kira serializer/importer 以及 DE/scaling 检查，并把 massive/massless 统一为双端点三槽 line pack，加入完整结构化 sector normalization、compact subsector 求导、h/H parity transport 及唯一公开 tree 表示。

仓库内标准加载与独立 benchmark 单文件加载是两个等价入口，应在新 kernel 中任选其一：
\begin{verbatim}
packageDir = FileNameJoin[{Directory[],"000_code","018_dSIBP"}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];

(* 独立 benchmark 的自包含单文件入口： *)
Get["independent-benchmark/package/package_018.wl"];
\end{verbatim}
...
\end{verbatim}
建议先运行 \texttt{06\_mix\_bubble\_tree/main.wl}；它展示 018 的显式动量角色、\texttt{ssij/sEe} 初始化、参数重定义、总导数和 cycle/fixed line 数据。\texttt{03\_single\_massive\_sunrise/main.wl} 使用共同外腿能量 \texttt{kE}、独立圈外模长 \texttt{kL}、两个在 massless-line 交换下互换的 ISP，以及顶点/平行线 \texttt{symmetryRules}；\texttt{DSSeeds} 正常构造可达 sector 的 general 模板，随后只选一个 top-sector \texttt{qIBP} 模板展开一个 seed 点的最小关系。\texttt{04\_pure\_massive\_bubble\_closed\_loop/main.wl} 展示外部 Kira 回读、DE 与 scaling；其余例子展示 mixed bubble、一般 \texttt{P,Q,T,W} 函数系统和两顶点 tree pure-time/dlog。examples 不含独立 expected；Kira example 只写输入并取回已有结果，不启动 reduction。
...
坐标名只由两个用户列表各自的稳定顺序决定，与符号名无关。每套列表的总数为 1--9 时使用 \texttt{ss11/sE1}；10--99 时使用两位补零，例如 \texttt{ss0101,ss0110,sE01}；100 起按该列表总数位数自然扩展。若显式给出旧平方坐标规则，仍以单位 Jacobian 工作，但它不再是 018 缺省：
\begin{verbatim}
...
018 的 \texttt{linePacks} 始终按 root \texttt{lineOrder} 保留每条传播子的位置。cycle/fixed full pack 分别为 \texttt{\{b,n1,n2\}} 与 \texttt{\{"F",n1,n2\}}；收缩后分别为 \texttt{\{bS\}} 与 \texttt{\{"F"\}}。因此 fixed line 没有 \texttt{b/bS}，但其端点离散态不会被删除；模长幂属于该 sector 的结构化 prefactor。\texttt{timeOnly} 也使用同一三参数公开表示，只令 \texttt{ispList=\{\}} 并把所有线按 fixed schema 保存。完整 pack pattern 唯一编码 shrink set；不得删除收缩槽位或用 compact \texttt{aList} 猜 sector。
...
\texttt{metaSeedRange} 是 \texttt{DSMetaSeedRange} 的公开兼容别名；两者返回同一 metadata，
后一次合法调用覆盖最近一次逐组范围状态。
...
\end{verbatim}
普通 batch 与 \texttt{allSeeds} 都完整遍历离散态。\texttt{DSSeeds} 的每条模板保存独立
...
\section{018 标准 package 与初始化}

一个 018 example 的开头分为四个独立单元：package 加载、详细输入、缺省 option、初始化。所有路径相对于 example 的 \texttt{main.wl}：
\begin{verbatim}
exampleDir = DirectoryName[$InputFileName];
packageDir = FileNameJoin[{
  exampleDir,"..","..","..","000_code","018_dSIBP"
}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];
...
同目录 \texttt{active\_basis\_19.wl} 提供可直接交给 formal \texttt{DSKiraPlan} 的 21 个有序候选、
\texttt{activeIndices=Range[19]}、physical \texttt{ks=ss11}、导数变量和 scaling degrees；
