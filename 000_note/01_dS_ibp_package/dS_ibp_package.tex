...
\abstract{本手册说明 dS IBP package 的费曼规则、统一积分表示、massive/massless 指标约定、time 与 loop-momentum IBP、独立变量微分方程 seed、EOM/缩并 canonical、拓扑输入、公开 API、Kira 接口和 tree vertex family。当前主线为 017：所有公开积分统一写成 \texttt{J[aList,linePacks,ispList]}；用户显式分别声明任意命名的 \texttt{loopExternal\allowbreak Momenta} 与 \texttt{independentExternal\allowbreak Momenta}，程序验证结构圈数、routing、声明完备性和动力学坐标，不自动猜角色。前者使用完整 \texttt{ssij} 根号 Gram 坐标，后者只为用户声明的实际无圈模长生成 \texttt{sE1,sE2,...}。package 只生成、序列化和验证关系，不运行 reduction。}
...
本 package 生成 dS 圈图的 canonical IBP seed，并把关系转换为后端中立的 \texttt{linearData}。当前稳定主线为模块化 017，支持 topology-driven 的任意圈数、任意拓扑以及 massive/massless 混合内线；标准入口和冻结单文件兼容入口分别为
\begin{verbatim}
000_code/017_dSIBP/
independent-benchmark/package/package_017.wl
\end{verbatim}
010--016 是只读历史/基线版本。017 继承标准 context、初始化 metadata、完整 time/loop-momentum 生成元、massive EOM、共同-theta odd-subset contact、contact 可达 sector、跨 sector coincident canonical、Kira serializer/importer 以及 DE/scaling 检查，并把 massive/massless 统一为双端点三槽 line pack，加入 sector-aware prefactor、compact subsector 求导、h/H parity transport 及唯一公开 tree 表示。

仓库内标准加载与独立 benchmark 单文件加载是两个等价入口，应在新 kernel 中任选其一：
\begin{verbatim}
packageDir = FileNameJoin[{Directory[],"000_code","017_dSIBP"}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];

(* 独立 benchmark 的自包含单文件入口： *)
Get["independent-benchmark/package/package_017.wl"];
\end{verbatim}
...
\begin{enumerate}
  ...
  \item 用 \texttt{DSSeeds} 生成普通 canonical seed batch，并从稳定字段 \texttt{"allSeeds"} 或 \texttt{DSAllSeeds} 取得全部 sector/generator 的一维 general 模板；模板始终完整遍历离散 $n_i=0,1$；
  \item 模板层立即执行 EOM/symmetry/canonical；再用 \texttt{DSGenerateIBP} 或同义入口 \texttt{generateIBP} 显式展开连续指标；
...
\end{verbatim}
建议先运行第一行的 bubble+tree 经典例子；它展示 017 的显式动量角色、\texttt{ssij/sEe} 初始化、参数重定义、总导数与错误输入门禁。其余例子依次展示 mixed massive/massless 工作流、一般 \texttt{P,Q,T,W} 函数系统、两圈 ISP、pure massive bubble 的外部 Kira 闭环，以及两顶点 tree pure-time/dlog 路线。examples 不含独立 expected；Kira example 只写输入并取回已有结果，不启动 reduction。

\section{第一个完整例子：bubble+tree}
...
\begin{equation}
 ...
 \label{bareHEOM}
\end{equation}
另一端点指标不变。最后一项就是 $\nu_e^2/x^2$ 二次 pole；loop-momentum 导数若产生 $n=2$，同样在 seed 层立即使用对应 EOM。当前 017 由统一函数系统编译这些递推；旧版内置公式只作为历史基线。

\subsection{通用 $2\times2$ 函数系统输入}

当前 017 对每条 massive line 输入
\begin{equation}
 f''+P(x)f'+Q(x)f=0,
...
\begin{equation}
 ...
 \qquad W_h=x^{-2\nu}W_H.
\end{equation}
该编译接口已在当前 017 实现；显式 $W_T$ 只作 $W_T=\det(T)W$ 的一致性校验。缺省 h、裸 H、一般 $T$ 和非法输入由函数系统专项检查覆盖，h/H 另有独立物理 benchmark。Wronskian 的标准推导见技术笔记专门 subsection。
...
\begin{align}
 ...
 \label{masslessM01}
\end{align}
第一端点定义 $M_1$ 的方向。交换端点时 $M_0$ 不变、$M_1$ 变号。017 的公开双端点态记为 $F_{n_1n_2}$，并用
\begin{equation}
...
这里缺省 compiled shift 是 $(s,z)=(0,0)$。“没有 shift”的准确含义是 shrink 不新增任何整数幂或 zero-point 幂吸收。对 cycle line，式~\eqref{masslessShrink} 直接保留 $bS_e=b_e$、$bS0_e=b0_e$；对 fixed/non-loop line，source/target 的结构化 prefactor exponent 相同，即 $B_t-B_s=0$、$N_s/N_t=1$。原有 fixed-line 模长因子仍属于 target sector 的 normalization，不能删除，也不能在 contact coefficient 外再次乘一份；normalized contact coefficient 的模长部分为 1，只保留式~\eqref{masslessDeltaSigns} 的 $-2/+2$ 与共同-theta系数。

massless 的 subsector parity 不保持原 offset。017 双端点表示中 contact 支持满足 $n_1+n_2=1$，而 $bS=b$，所以若 parent 使用 $b+n_1+n_2$ 分级，
\begin{equation}
 b+n_1+n_2=b+1=bS+1\pmod 2.
 \label{manualMasslessContactParity}
\end{equation}
因此 cycle massless contact 使 child-sector parity offset 相对 parent 翻转 1；fixed/non-loop massless line 不参与 parity，没有这次翻转。多线 simultaneous contact 按被 shrink 的 massless cycle line 数模 2 累加。017 不自动给 pure massless family 启用 parity；mixed h/H 系统一旦使用已认证 root parity，massless cycle transition 必须携带上述 affine offset，不能套用 h/H 的“缺省不变”。
...
用户可给圈动量和外动量任意符号名，例如 \texttt{sah,bob,alice}。017 不从符号字符串猜测角色，也不再从一个统一列表自动分类；输入必须显式声明
\begin{verbatim}
loopMomenta
loopExternalMomenta
independentExternalMomenta
\end{verbatim}
\texttt{loopExternal\allowbreak Momenta} 是圈积分外动量基，决定完整 Gram 坐标、$dqk$ 生成元和 ISP 闭合维数；\texttt{independentExternal\allowbreak Momenta} 是 topology 中实际无圈 line/phase 的独立模长列表，只定义 $|P_e|$，不定义彼此点积。旧字段 \texttt{externalMomenta}/\texttt{externalLegMomenta} 仅作为分别对应这两个字段的兼容别名；同时给新旧字段时以新字段为准。程序会从 line routing 独立计算实际需要的空间并与用户声明比较，而不会替用户选择基。
...
017 内部保留用户给定 loop-external 基的平方 Gram 原子，公开缺省为根号坐标：
\begin{verbatim}
kk[i,j] = sp[ki,kj]
ssij = Sqrt[sp[ki,kj]]
\end{verbatim}
...
坐标名只由两个用户列表各自的稳定顺序决定，与符号名无关。每套列表的总数为 1--9 时使用 \texttt{ss11/sE1}；10--99 时使用两位补零，例如 \texttt{ss0101,ss0110,sE01}；100 起按该列表总数位数自然扩展。若显式给出旧平方坐标规则，仍以单位 Jacobian 工作，但它不再是 017 缺省：
\begin{verbatim}
...
017 的 \texttt{linePacks} 始终按 root \texttt{lineOrder} 保留每条传播子的位置。cycle/fixed full pack 分别为 \texttt{\{b,n1,n2\}} 与 \texttt{\{"F",n1,n2\}}；收缩后分别为 \texttt{\{bS\}} 与 \texttt{\{"F"\}}。因此 fixed line 没有 \texttt{b/bS}，但其端点离散态不会被删除；模长幂属于该 sector 的结构化 prefactor。\texttt{timeOnly} 也使用同一三参数公开表示，只令 \texttt{ispList=\{\}} 并把所有线按 fixed schema 保存。完整 pack pattern 唯一编码 shrink set；不得删除收缩槽位或用 compact \texttt{aList} 猜 sector。
...
当前 017 提供 seed 层和表达式层的独立变量求导接口。变量分三类：
\begin{itemize}
...
令 $x_{ij}=\operatorname{sp}(k_i,k_j)=\texttt{ssij}^2$。017 不重写平方 Gram 导数，而是使用 $\partial_{\texttt{ssij}}=2\,\texttt{ssij}\,\partial_{x_{ij}}$ 调用原子层。显式系数与 sector prefactor 仍执行完整乘积与链式法则；程序不使用 \texttt{PowerExpand}。
...
017 保留以下原子接口。推荐显式传入 \texttt{DSInit} context 或 parsed topology；若先调用 \texttt{setIBPTopologyContext[topo]}，也可使用短签名：
\begin{verbatim}
...
sector 的全部 $n_i$ 遍历 $0,1$ 并完成 EOM/canonical；连续 $a_i,b_i,bS_i$ 和 ISP 指标仍保持
general。\texttt{DSSeeds} 不允许离散态抽样，连续指标保持 general，交给
...
反向区间以及把 \texttt{n[...]} 写进范围都会给红色双语 Error，并以相应结构化字段返回失败：
\begin{verbatim}
...
J[aList,linePacks,{}]
\end{verbatim}
并记录 \texttt{representation -> "J[aList,linePacks,ispList]"}。每条 full line 都使用
\texttt{\{"F",n1,n2\}}，shrunk line 使用 \texttt{\{"F"\}}；root line 位置始终保留。
这条路线没有 momentum generator、$b/bS$ 或 ISP，但 fixed-line 模长幂仍在
\texttt{sectorPrefactorData} 中参与 \texttt{ds/DSDE} 的乘积法则。用户不输入也不能调用
Private vertex basis 或 state selector。
...
\end{verbatim}
其中 top sector 的状态集合为 $(n_1,n_2)\in\{0,1\}^2$，先在 relation 层应用 massless 额外关系和 canonical 化；同分支可达 sector 为 \texttt{\{"top","e1"\}}，异分支只有 \texttt{\{"top"\}}。两个顶点各自的 time generator 作用于 top 状态；同分支 shrink sector 另有合并顶点的 time seed。成功时 \texttt{DSSeeds} 与 \texttt{DSLinear} 都返回 \texttt{status -> "generated"}。含 massless full line 的 \texttt{DSTreeSeeds/repIterative/DSTreeNaiveIBP/DSTreeNaiveDE/DSTreeDLogDE} 当前返回 \texttt{PendingRederivation}；它们不会退回旧单 $n$ 或单参数 $J$ 公式。
...
输入中的 \texttt{seedRanges/generatorSeedRanges} 仍用于普通 \texttt{DSSeeds} batch 的缺省或
reference 非矩形范围；它们不替代 \texttt{DSGenerateIBP} 对 \texttt{allSeeds} 的显式范围。
新公开闭环只使用以下接口：
\begin{verbatim}
context = DSInit[case];
...
\end{verbatim}
普通 batch 与 \texttt{allSeeds} 都完整遍历离散态。seed 或裸模板不能直接
交给 serializer；必须先得到成功的 backend-neutral \texttt{linearData}。准备构造 DE 时必须
保留全部微分变量的符号系数域；numeric 模式只适用于不再对被固定变量求导的独立任务。
...
预约化阶段只冻结 reference-style 全局排序和用户指定的候选 targets：
\begin{verbatim}
...
用户取得并检查预约化 master 后，再建立有序 active basis：
\begin{verbatim}
activeBasis = <|
...
\end{verbatim}
formal 阶段先对每个 active expression 解析构造一阶导数和全部 derivative target closure；
只有闭合成功才返回
\begin{verbatim}
...
formal export manifest 会保留该证书和数值规则审计。两个阶段必须使用不同输出目录，不能让
...
\section{017 标准 package 与初始化}

一个 017 example 的开头分为四个独立单元：package 加载、详细输入、缺省 option、初始化。所有路径相对于 example 的 \texttt{main.wl}：
\begin{verbatim}
exampleDir = DirectoryName[$InputFileName];
packageDir = FileNameJoin[{
  exampleDir,"..","..","..","000_code","017_dSIBP"
}];
If[!MemberQ[$Path,packageDir],AppendTo[$Path,packageDir]];
Needs["dSIBP`"];
...
\texttt{DSSeeds} 始终完整枚举所有适用 $n_i=0,1$ 状态。\texttt{quickCheck}、\texttt{fullDiscrete} 与 \texttt{bounded} 只提供不同的缺省连续 \texttt{seedRanges}，不改变离散覆盖，也不触发抽样。\texttt{timeOnly} 与 full mode 使用同一个三参数 \texttt{J} schema；不存在需要用户填写的 \texttt{TreeState} 或其它私有分派参数。
...
017 使用一个集中消息层，所有耗时模块只上报结构化事件，不在内部循环散落 \texttt{Print}：
\begin{verbatim}
DSMessagesOn[]
DSMessagesOff[]
DSMessagesQ[]
...
Kira 数值化只用于固定不参与微分的系数参数。若本次 DE variables 是 $\{x_a\}$，则从
\texttt{DSSeeds}、\texttt{DSLinear}、\texttt{DSKiraExport}、外部 reduction、\texttt{DSKiraImport}
直到 \texttt{DSDE} 完成，所有 $x_a$、对应内部平方原子及含它们的表达式都必须保持符号。
export 前应显式验证 numeric-rule 左端与 DE variables 交集为空，右端也不含 DE variables。
例如 \texttt{kk[1,1]->ss11\string^2} 是符号坐标映射，不是数值规则。最终有理 probe 只能在
DE 已构造后代入，并须先检查全部分母非零。
交付的 pure-massive-bubble closed-loop example 为了与已有 reference 资产对接，仍演示旧平方坐标
\texttt{sp[k,k]->s11}。若要验证 017 缺省 root-coordinate DE，应复用该 example 的 topology、basis 与 seed range，
但删去这条兼容规则，重新初始化为 \texttt{sp[k,k]->ss11\string^2}，并用
\texttt{derivativeVariables->\{ss11,P0\}}重新导出、外部约化和取回。不能将旧 \texttt{s11} reduction
作字符串改名后充当新坐标的 fresh 结果。
...
Tree 与 loop 的公开对象完全相同：
\begin{verbatim}
J[aList,linePacks,{}]
\end{verbatim}
\texttt{aList} 按当前 sector 的 active vertex 顺序保存时间幂次；\texttt{linePacks} 按 root line 顺序保存
\texttt{\{"F",n1,n2\}} 或 \texttt{\{"F"\}}。massive-only 论文公式需要的 vertex basis 由 package
在 Private context 内按 \texttt{sectorKey} 构造，完成迭代或 dlog 计算后立即映回上述公开表示；用户结果、诊断、master list 和 \texttt{linearData} 均不得出现一参数 \texttt{J}。
...
$\{\text{剩余可 contact 的 theta 线数},\sum_e|a_e-a_{e,\mathrm{end}}|\}$ 严格下降，并检测重复 canonical 状态；不下降、递推奇异或循环才是数学失败。含 contact source 时，raw 与 sector-tagged 迭代都从 \texttt{DSTreeSeeds} 的 direct pure-time relation 生成同一首步，因而保留显式 \texttt{treeEnergy}；旧三槽 loop 投影只作交叉验证。
...
若不同 lower sectors 具有相同 compact \texttt{aList}，root-ordered line-pack pattern 仍唯一编码 shrink set；内部 tree linear data 和约化 token 另外保存由该 pattern 派生的 \texttt{sectorKey} 作交叉核验，不另建并行积分表示。公式型 tree 路线当前只认证 massive-only family；含 massless full line 时五个 tree 公式接口均 fail closed 为 \texttt{PendingRederivation}，直到三槽 quotient basis 的递推和 dlog 公式重新推导完成。
...
当前未实现：一般图 automorphism/参数对称性的自动发现、scaleless 全自动前端、自动选择 propagator basis、streaming/chunked 高圈 seed、massless 三槽 quotient basis 上的公式型 tree 递推/dlog，以及实际运行 reduction。017 已实现标准 context、动力学变量审计、h/H parity、交互消息、Kira 结果取回和 loop/tree DE/scaling；package 仍只验证并导入用户在外部生成的完整 reduction。
...
\texttt{DSSeeds} & 完整枚举全部 $n=0,1$ 并生成连续指标保持 general 的一维模板 \texttt{allSeeds} 与推荐分组；所有模式返回统一三参数 \texttt{J} & 生成 shrink sector=True；进度=Automatic \\
...
\texttt{DSMetaSeedRange}, \texttt{metaSeedRange} & 按用户 seeds 外层结构初始化逐组 shift metadata；flat 为一组，nested 按顶层分组；再次调用覆盖旧状态 & 声明指标多报或少报只 warning，按实际变量继续 \\
\texttt{DSGenerateIBP}, \texttt{generateIBP} & 从统一或逐指标最终关系包络反推逐组 seed 点域，先筛 parity 再撒点；返回目标包络、逐组范围、空组、方程记录和 canonical 证书 & 无数量上限；进度=Automatic \\
\texttt{DSLinear} & seed 转后端中立 \texttt{linearData} & 系数规则=Automatic；Kira ordering=Automatic \\
\texttt{DSKiraPlan} & 生成 reference-style 顺序、预约化 candidates，或 formal active-basis derivative closure/minimal targets & stage 必填；numericStage=symbolic；进度=Automatic \\
...
表中的原子、tree 与 \texttt{DS*} 高层接口均由 017 当前主线提供。各函数的精确可用 options 以 \texttt{Options[函数]} 为准；massless 公式型 tree 接口以结构化 \texttt{PendingRederivation} 明确失败，不得静默回退到历史表示或不完整结果。
