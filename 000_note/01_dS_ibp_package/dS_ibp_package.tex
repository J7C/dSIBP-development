...
坐标名只由两个用户列表的稳定顺序决定，与符号名无关。类别总数为 1--9 时使用 \texttt{ss11/sE1}；10--99 时使用两位补零，例如 \texttt{ss0101,ss0110,sE01}；100 起按总数位数自然扩展。若显式给出旧平方坐标规则，仍以单位 Jacobian 工作，但它不再是 016 缺省：
\begin{verbatim}
...
在 loop 表示中，只有 cycle line 的 pack 含 \texttt{b/bS}；bridge/non-cycle line pack 只含 endpoint \texttt{n}。fixed line 的零点幂和导数产生的幂变化全部进入显式模长系数。\texttt{timeOnly} 下所有 active lines 都使用 fixed-coefficient schema，并转到直接 \texttt{J[vertexPacks]} pure-time 表示；即使输入图含结构圈，也不为 line 伪造 \texttt{b/bS}。
...
\texttt{DSKinematics} & 审计结构/routing、双动量列表与动力学规则完备性 & rules=Automatic \\
