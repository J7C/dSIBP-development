...
\usepackage{tikz}
\usetikzlibrary{calc}
\graphicspath{{./}{000_note/}{../reference/ref_note/note_dS_massless/}{../../reference/ref_note/note_dS_massless/}}
...
\title{dS IBP Package: 指标约定与 massive/massless 统一框架}
\author[a]{Su Yingze}
\affiliation[a]{Institute not specified}
\emailAdd{yingze.su@outlook.com}
\abstract{本笔记记录 dS IBP package 的第一版指标 convention 和初始化约定。目标是统一 massive Hankel 传播子与 massless 指数传播子的 IBP 生成方式：先固定图、时间变量、圈动量与传播子 building block，再在给定幂次范围内生成时间 IBP、圈动量 IBP、边界收缩关系和前端简化关系。}
...
% ============================================================
\section{Package 目标与总约定}
% ============================================================

本 package 的目标是建立一个同时覆盖 massive 与 massless 内线的 dS IBP 生成框架。核心原则是：

\begin{itemize}
    \item 时间顶点幂次 $a_v$ 单独记录，写作 $(-\tau_v)^{a_v}$，不和传播子或圈动量指标混在一起。
    \item 每条圈内线的动量幂次与该线自身的离散 building-block 指标打包记录；包的长度由场类型决定：
    \begin{equation}
        \ell_i^{\rm massive}:\qquad \{b_i,n_{i,1},n_{i,2}\},
        \qquad
        \ell_i^{\rm massless}:\qquad \{b_i,n_i\}.
        \label{linepackconv}
    \end{equation}
    这里 $b_i$ 是内线模长 $q_i$ 的分母幂次。Massive 传播子在两端各有一个 Hankel building-block 指标 $n_{i,1},n_{i,2}$；massless 指数核利用端点导数关系后只剩两个独立状态，记为 $n_i=0,1$。这样圈动量 IBP 产生的奇偶性筛选自然绑定到对应传播子上。
    \item 不属于可移位指标的数据写入 family 初始化配置，而不是写进积分标签；例如图连接关系、圈动量基、外部能量和、场类型、SK 分支、正规化参数、边界处方和后端排序。
\end{itemize}

对一个有 $V$ 个时间顶点和 $E$ 条圈内线的 tq family，建议记作
\begin{equation}
    \mathcal{I}_{\Theta}^{F}\!\left[
        \{a_v\}_{v=1}^{V};
        \{\mathfrak{p}_i\}_{i=1}^{E}
    \right],
    \label{pkgfamilygeneric}
\end{equation}
其中 $\Theta$ 表示该 family 的 time-ordering/theta 选择，$F$ 表示图或拓扑名；$\mathfrak{p}_i$ 是第 $i$ 条圈内线的指标包，massive 时取 $\{b_i,n_{i,1},n_{i,2}\}$，massless 时取 $\{b_i,n_i\}$。机器内部可以把指标展平成列表，但人读文档和调试输出应保留式 (\ref{pkgfamilygeneric}) 的分组结构。

\subsection{初始化配置中保存的内容}

以下信息不进入指标：
\begin{itemize}
    \item 图的顶点、内线、外线、圈动量基，以及每条内线动量
    \begin{equation}
        \boldsymbol{Q}_i=\sum_{\alpha=1}^{L}c_{i\alpha}\boldsymbol{\ell}_\alpha+\boldsymbol{K}_i,
        \qquad q_i=|\boldsymbol{Q}_i|.
    \end{equation}
    \item 每条内线的场类型：massive Hankel、conformal massless、minimally coupled massless，或后续其它变体。
    \item SK 分支、theta 处理方案、早时 $i0$ 处方、晚时边界收敛调节、维度 $d$ 与解析正规化参数。
    \item overall normalization、耦合常数、对称因子；这些通常不参与 IBP 约化。
    \item 前端关系：Hankel EOM、massless endpoint 简化、对称性、scaleless sector、apart 关系，以及是否把这些关系和 IBP 一并导出给后端。
\end{itemize}

% ============================================================
\section{Massive 两 theta 框架}
% ============================================================

Massive 情形作为统一框架的基准先写。对一条连接顶点 $u_i,v_i$ 的 massive 内线，building block 记为
\begin{equation}
    h_{\nu_i}\!\left(n_{i,u};q_i\tau_{u_i}\right)\,
    h_{\nu_i}\!\left(n_{i,v};q_i\tau_{v_i}\right),
    \label{massiveblock}
\end{equation}
并在积分指标中打包成
\begin{equation}
    \{b_i,n_{i,u},n_{i,v}\}.
\end{equation}
对 bubble，若两条内线分别为 $q_1=|\boldsymbol q|$ 与 $q_2=|\boldsymbol q+\boldsymbol k_3|$，则 top sector 写成
\begin{equation}
    G\!\left[
        \{a_1,a_2\};
        \{\{b_1,n_{1,1},n_{1,2}\},\{b_2,n_{2,1},n_{2,2}\}\}
    \right].
    \label{massivebubblepack}
\end{equation}
这与旧代码中的
\begin{equation}
    G[\{n_1,n_2,n_3,n_4\},\{a_1,a_2\},\{b_1,b_2\}]
\end{equation}
等价，但新的写法把同一条圈内线的 $b_i$ 与两端 $n$ 指标放在一起：
\begin{equation}
    \{n_1,n_2,n_3,n_4\},\{b_1,b_2\}
    \quad\longrightarrow\quad
    \{\{b_1,n_1,n_2\},\{b_2,n_3,n_4\}\}.
\end{equation}

这个打包方式对圈动量 IBP 更自然。参考 massive bubble 代码中的奇偶筛选，top sector 满足
\begin{equation}
    n_{1,1}+n_{1,2}+b_1\in2\mathbb{Z},
    \qquad
    n_{2,1}+n_{2,2}+b_2\in2\mathbb{Z}.
    \label{massiveparity}
\end{equation}
也就是说，奇偶性并不是全局地作用在所有 $n$ 上，而是逐条传播子作用在 $\{b_i,n_{i,1},n_{i,2}\}$ 这个局部包上。

\subsection{Massive 的 IBP 移位结构}

时间 IBP 对顶点 $\tau_v$ 求导时，会对所有 incident line 的对应端点指标产生移位。例如 bubble 中对 $\tau_1$ 求导，schematically 有
\begin{equation}
    \partial_{\tau_1}:
    \quad
    a_1\to a_1-1,\qquad
    n_{1,1}\to n_{1,1}+1,\qquad
    n_{2,1}\to n_{2,1}+1,
\end{equation}
并可能产生 $\theta$ 导数给出的收缩子拓扑。对 $\tau_2$ 求导则作用在 $n_{1,2},n_{2,2}$。

Hankel EOM 用于把超出基准范围的 building block 约回 $n=0,1$。bubble 代码中的典型关系可抽象写为
\begin{equation}
    h_\nu(n+2;q\tau)
    \;\longrightarrow\;
    \text{linear combination of }h_\nu(n;q\tau),\ h_\nu(n+1;q\tau),
\end{equation}
实际指标层面表现为 $n_{i,r}=2$ 的项被重写为 $n_{i,r}=0,1$ 的项，同时伴随 $a_v$ 与 $b_i$ 的移位。

圈动量 IBP 对某条 $q_i$ 求导时，不仅移动该条线的 $b_i$，还会通过链式法则移动相邻线的 $b_j$，并在指数或 Hankel block 上产生 $n_{i,r}$ 的移位。因此所有 seed 生成后都应立刻应用：
\begin{equation}
    \text{Hankel EOM}\quad\to\quad
    \text{逐线 parity selection}\quad\to\quad
    \text{对称 canonical 化}.
\end{equation}

% ============================================================
\section{Massless 选定方案}
% ============================================================

Massless note 中讨论了两类处理：

\begin{itemize}
    \item A 方案：两项 Heaviside 合并处理，结构上最接近 massive 的两 theta/Hankel building block 框架。
    \item B 方案：固定单个 theta 分支处理，方程更小，适合专门的纯 massless 项目。
\end{itemize}

本 package 的目标是任意 massive/massless case 的统一生成器，因此主线只采用 A 方案：
\begin{equation}
    \boxed{
    \begin{array}{ll}
    \text{纯 massless case} & \text{使用 A 方案，两 theta 合并；}\\[2mm]
    \text{massive 或 massive/massless 混合 case} & \text{使用 A 方案，两 theta 合并。}
    \end{array}}
    \label{masslesschoice}
\end{equation}
只有在这条路线中，massless 传播子的端点导数关系才会自然压缩成逐线指标包 $\{b_i,n_i\}$。B 方案只作为参考公式和交叉检查背景，不作为本 package 的主线 convention。

\subsection{A 方案中的 massless 简化}

在本 package 的 A 方案中，massless 线不再保留两个端点指标，而是使用逐线打包指标
\begin{equation}
    \{b_i,n_i\},\qquad n_i=0,1.
\end{equation}
但 massless 指数核满足
\begin{equation}
    \partial_{\tau_1}e^{\mp iq_i(\tau_1-\tau_2)}
    =
    -\partial_{\tau_2}e^{\mp iq_i(\tau_1-\tau_2)},\qquad
    \partial_{\tau_1}\partial_{\tau_2}e^{\mp iq_i(\tau_1-\tau_2)}
    =
    q_i^2 e^{\mp iq_i(\tau_1-\tau_2)}.
    \label{masslessendpointrels}
\end{equation}
因此若从 naive 两端点标签出发，同一条 massless 线满足
\begin{equation}
    \{b_i,1,0\}=-\{b_i,0,1\},\qquad
    \{b_i,1,1\}=\{b_i-2,0,0\}.
    \label{masslesspackedrels}
\end{equation}
这里第二个关系中的 $b_i-2$ 来自 $q_i^2/q_i^{b_i}=1/q_i^{b_i-2}$。这两个关系说明 naive 的 $\{n_{i,1},n_{i,2}\}$ 只有两个独立状态，所以正式指标中直接压缩为 $n_i=0,1$：$n_i=0$ 表示基准/对称状态，$n_i=1$ 表示相对的反对称或一次端点导数状态。

\subsection{同一顶点对的多条 massless 线}

若两个顶点之间连接多条 massless 传播子，还存在一个额外的合并简化。设同一对顶点 $(u,v)$ 之间有 $m$ 条 massless 线，模长为 $q_1,\dots,q_m$。每条传播子本身有两个 time-ordering 项，但这些 Heaviside 因子并不是 $2^m$ 个独立区域。因为同一对时间只有
\begin{equation}
    \tau_u>\tau_v,\qquad \tau_v>\tau_u
\end{equation}
两个互斥区域，任何混合了 $\theta(\tau_u-\tau_v)$ 与 $\theta(\tau_v-\tau_u)$ 的项在非 pinched 区域上支撑为空。因此乘积只剩两类分支：
\begin{equation}
    \prod_{i=1}^{m}e^{-iq_i(\tau_u-\tau_v)}\theta(\tau_u-\tau_v),
    \qquad
    \prod_{i=1}^{m}e^{+iq_i(\tau_u-\tau_v)}\theta(\tau_v-\tau_u),
\end{equation}
也就是只依赖
\begin{equation}
    Q_{uv}\equiv\sum_{i=1}^{m}q_i
\end{equation}
的两项组合。

这说明：在 A 方案中，除了逐线关系 (\ref{masslesspackedrels})，同一顶点对上的多条 massless 线还可以在 theta 结构上进一步按 bundle 合并。参考 note 中把这种合并后的状态写成顶点对的两个状态 $n_{uv}=0,1$，分别对应两项 Heaviside 组合的对称/反对称部分。

但第一版 package 不把这个 bundle 指标作为强制代码接口。正式积分指标仍按传播子逐线记录
\begin{equation}
    \{\{b_i,n_i\}\}_{i\in E_{\rm massless}},
\end{equation}
并把同一顶点对多 massless 线的合并视为可选前端简化或后续优化：生成/检查 IBP 时可以记录该关系，必要时用来减少方程；若实现成本过高，也可以先只使用逐线二元包，不做 bundle canonical 化，但仍保持两 theta 合并路线。

\subsection{参考：B 方案的单 theta 表示}

在另一个专门的纯 massless 项目中，可以不引入端点 $n$ 指标，而固定每条 massless 传播子的 time-ordering 符号 $\sigma_i=\pm$。这不是本 package 的主线 convention；这里只保留公式用于对照。bubble 的单分支相位可写为
\begin{multline}
    ...
    \right]\\
    \times\theta\!\bigl(\sigma_1(\tau_1-\tau_2)\bigr),
    \qquad \sigma_1=\sigma_2.
    \label{pkgmasslesssinglephase}
\end{multline}
对应积分族保留时间幂次和 massless 线包：
\begin{equation}
    \mathcal{I}_{\sigma_1,\sigma_2}
    \left[\{a_1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}\right]
    =
    \int d\tau_1d\tau_2\,(-\tau_1)^{a_1}(-\tau_2)^{a_2}
    \int\frac{d^d\boldsymbol q}{(2\pi)^d}
    \frac{\Phi_{\sigma_1,\sigma_2}}{q_1^{b_1}q_2^{b_2}}.
    \label{pkgmasslessBfamily}
\end{equation}
对最简单的纯指数单分支，通常从 $n_1=n_2=0$ 的 sector 开始；若 IBP 或前端展开产生一次端点导数状态，则用 $n_i=1$ 表示，而不是重新引入 $n_{i,1},n_{i,2}$ 两个端点槽。

\subsection{参考：Pure massless B 方案的时间 IBP}

单分支的 $\tau_1$ 与 $\tau_2$ IBP 为
\begin{align}
    0={}&-a_1\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1-1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}]
    \notag\\
    &+ik_1\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}]
    -\sigma_1 i\,\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2\};\{\{b_1-1,n_1\},\{b_2,n_2\}\}]
    \notag\\
    &-\sigma_2 i\,\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2\};\{\{b_1,n_1\},\{b_2-1,n_2\}\}]
    +\sigma_1\mathcal{I}_{\delta}[\{a_1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}],
    \label{pkgmasslesstau1}
\\
    0={}&-a_2\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2-1\};\{\{b_1,n_1\},\{b_2,n_2\}\}]
    \notag\\
    &+ik_2\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}]
    +\sigma_1 i\,\mathcal{I}_{\sigma_1,\sigma_2}
    [\{a_1,a_2\};\{\{b_1-1,n_1\},\{b_2,n_2\}\}]
    \notag\\
    ...
    [\{a_1,a_2\};\{\{b_1,n_1\},\{b_2-1,n_2\}\}]
    -\sigma_1\mathcal{I}_{\delta}[\{a_1,a_2\};\{\{b_1,n_1\},\{b_2,n_2\}\}].
    \label{pkgmasslesstau2}
\end{align}
这里 $\mathcal{I}_{\delta}$ 是 $\theta$ 导数产生的塌缩子拓扑。它不应简单视为 top sector 的某个 $b_i=0$ 边界点，而应作为独立 family 初始化。

\subsection{幂次范围的第一版约定}

每个 case 配置独立给出指标盒子：
\begin{equation}
    a_v\in[a_v^{\min},a_v^{\max}],\qquad
    \mathfrak{p}_i\in\mathcal{P}_i.
\end{equation}
Massive 传播子的
\begin{equation}
    \mathcal{P}_i=[b_i^{\min},b_i^{\max}]\times\{0,1\}\times\{0,1\},
\end{equation}
生成 seed 后用 Hankel EOM 把 $n=2$ 递回该集合。Massless 传播子的
\begin{equation}
    \mathcal{P}_i=[b_i^{\min},b_i^{\max}]\times\{0,1\},
\end{equation}
其中 $n_i=0,1$ 是式 (\ref{masslesspackedrels}) 压缩后的两个独立状态。

实际生成 IBP 时，盒子需要覆盖常见移位：
\begin{equation}
    a_v\to a_v\pm1,\qquad
    b_i\to b_i\pm1,\ b_i\pm2,\qquad
    n_{i,r}\to n_{i,r}+1.
\end{equation}
第一版可采用目标积分外加 padding 的方式，之后由残留报告决定是否扩盒。
...
\subsection{t→x 变换：将 \texorpdfstring{$\tau$}{tau} 积分转为 \texorpdfstring{$x$}{x} 积分}
...
\subsubsection{示例：\texorpdfstring{$(++)$}{(++)} 构型}
...
\subsection{xq 表示中的 \texorpdfstring{$\mathcal{I}_1$}{I1} 积分族}
...
\subsection{\texorpdfstring{$(++)$}{(++)} 情形：tq 积分族}
...
因此参考 note 中的 A 类可以不用 naive 的多端点标签，而对每一对顶点记一个 bundle 指标 $n_i$，其中 $i$ 标记这对顶点。当前 bubble 只有这一对顶点，所以函数族第一槽就是这个 $n_i$；$n_i=0,1$ 分别表示两项 Heaviside 组合的对称或反对称部分：
\begin{equation}
    \begin{aligned}
    ...
    \end{aligned}
\end{equation}
下面这些 A 类 bubble 公式沿用参考 note 的 bundle 记法；第一槽的 $\{0\}$ 或 $\{1\}$ 就是这一对顶点的 $\{n_i\}$。这组公式说明了多条 massless 线共享同一对顶点时的额外合并结构。Package 第一版的正式接口仍以前文的逐线包 $\{b_i,n_i\}$ 为主，bundle 记法先作为可选前端简化或后续优化记录。若图中有多对顶点，则参考 note 的 bundle 视角会给每一对顶点各引入自己的 $n_i$。在 massive Hankel 情形中，同一端点导数会把 $h_{\nu}(n;q_i\tau_a)$ 提升到新的 Hankel building block；两端导数通常仍是独立对象。Massless 指数核把这些对象压缩成上面的 $n_i=0,1$ 两类，这是两 theta 合并路线相对 massive 情形的额外简化，也是本 package 能用同一套 `J` 表示覆盖 massive、massless 与混合图的原因。

时间 IBP 现在按这个参考 bundle 记法书写。由于
\begin{equation}
...
\end{equation}
B 路线在纯 massless 专题中方程较小、可复用：$\sigma_i=+$ 与 $\sigma_i=-$ 的差别只在内线移位项和塌缩项的符号。先约化 $\mathcal{I}_{+,+}$，再通过 $\sigma_i\mapsto-\sigma_i$ 得到 $\mathcal{I}_{-,-}$，通常比合并路线更轻。但这条路线不提供本 package 所需的统一 $\{b_i,n_i\}$ 打包 convention，所以下式只作为参考。单分支的 $\tau_1$-IBP 为
\begin{align}
...
\subsection{正文的 \texorpdfstring{$\mathcal{I}_1$}{I1} 族}
