...
\abstract{本手册说明 dS IBP package 的费曼规则、统一积分表示、massive/massless 指标约定、time 与 loop-momentum IBP、EOM/缩并 canonical、拓扑输入、线性系统和验证方式。当前开发主线为 009。纯 massless 与 mixed case 都使用逐传播子的双 theta 合并路线；单 theta 路线不属于本 package。}
...
\end{verbatim}
它已经实现 topology 解析、完整 time/loop-momentum 生成元、massive EOM、massless 有向单 $n$、massive/massless theta shrink、跨 sector coincident canonical、后端中立线性系统和基础 Kira serializer。
...
009 当前内部使用 \texttt{applyTimeGeneratorSeed} 与 \texttt{applyMomentumGeneratorSeed}。自动 batch 必须在给定离散 $n$ 后调用，再立即 EOM/canonical。

\subsection{后续原子 API}

以下接口已经写入 plan/todolist，但尚未在 009 实现，不能当作当前功能：
\begin{verbatim}
dtau[i,expr_]
dqq[i,j,expr_]
dqk[i,j,expr_]
...
\texttt{009\_symmetry\_check.wl} & $11/11$ &
\texttt{sp} Orderless、用户 symmetry、Vpm\\
\bottomrule
\end{tabular}
\end{center}
...
尚待重推的主要函数族是 mixed bubble、mixed triangle、mixed sunrise、pure massive bubble reference 和 two-loop ISP toy。完成这些之前，不能宣称 package 对任意拓扑已被数学穷尽验证。
