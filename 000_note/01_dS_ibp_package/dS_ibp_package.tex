...
外动量矢量导数对 \texttt{Sqrt[s11]} 等非线性表达式显式执行 $D F=\sum_a(\partial F/\partial x_a)D x_a$；不能把 $x_a$ 直接替换为 $D x_a$。
...
一般积分族对称性依赖质量和外参条件，012 不自动猜测一般图 automorphism。程序会自动加入受门禁保护的原始 self-loop tadpole 规则，并与用户 \texttt{symmetryRules} 取去重并集：
\begin{verbatim}
repSymmetry0[topo_]
...
ds[expr,sij,topo]       or ds[expr,sij]
