...
- `package/` 保留冻结的 `package_014.wl/pdf` 与当前 `package_015.wl/pdf`，并只含 `examples/` 下少量典型应用文件；不含无版本名副本、expected、验证脚本、plan、design、技术笔记或 reduction 输出。
- 任务书第 2--15 节冻结 014 的平方不变量 benchmark；第 16 节独立验证 015 根号坐标 adapter。015 对 `externalMomenta` 使用完整 `ssij=Sqrt[sp[k_i,k_j]]` Gram 基；其它无圈动量按实际出现模长平方的增量秩选择生成 `sE1,sE2,...`，从属项保留 binding，不生成外腿交叉点积。有序方向导数仍固定为 `Dij=k_i.partial_{k_j}`，并必须验证简单根号、一般混合坐标、从属 binding 和实际无圈线径向导数的链式法则。
...
本项目内 014 全面独立手推和 015 根号坐标增量手推、冻结 expected 与单向比较位于 `codex-independent-benchmark/`，正式报告统一归档到 `000-report/`。新的外部推导结果只有在完成来源隔离、逐式人工审查和 package 对照后，才可按当前版本规则导入；不得写回 `independent-benchmark/` 交付目录。
