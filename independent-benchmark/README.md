...
- 任务书第 2--15 节保存 014 物理 benchmark，第 16 节保存 015 根号坐标 adapter，第 17 节独立验证 016 的显式双动量列表、多重图/routing、cycle/bridge pack、pure-time、参数重定义和 exact/over/under 门禁。第 17.4 节固定 bubble+tree 六变量专项，第 17.5 节统一审计全部 family 的 general 参数闭合、条件性 EOM/symmetry/canonical 和物理 zero-point。全部 expected 冻结后只加载当前 016 交付做比较。
...
已有报告只证明其报告中列出的范围。新增第 17.4--17.5 节在形成新的冻结手推、单向比较和正式报告之前，状态只能记为“任务书已要求”或“package 已有实现/自检”，不能沿用旧报告的总通过数宣称 source-isolated 完成。
