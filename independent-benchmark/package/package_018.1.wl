...
J::usage = "J[aList,linePacks,ispList] 是 018 唯一积分对象；timeOnly 使用空 ispList，sector 由 root-ordered full/shrunk line-pack pattern 唯一编码。";
...
(* 新 canonical record 直接携带唯一 sectorKey；旧 source 编码只作为缺字段兼容，
   不能覆盖显式 sector 身份，否则 direct tree/contact records 会被误归到 top。 *)
seedEntrySourceSectorKey[entry_Association] := Module[
   {explicit = Lookup[entry, "sectorKey", Missing["NoSectorKey"]], source = Lookup[entry, "source", "top"]},
   Which[
    StringQ[explicit], explicit,
...
DSLinear[seedData_Association, context_: Automatic, opts : OptionsPattern[]] := Module[
   {resolved, linearData, mode = OptionValue[LinearSystemMode],
...
vertexEnergyNamingReport[topo_Association] := Module[
   ...
   <|
    ...
    "dependencyData" -> dependencies,
    "externalLegInvariantNamingReport" -> externalLegInvariantNamingReport[topo],
     "message" -> "vertexEnergies 可使用 loop-external Gram 根号或 independentExternalMomenta 声明的无圈模长；018 不自动生成无圈动量之间的交叉点积。无圈动量变量不进入 loop IBP/ISP。"
     |>
    ];
...
sectorKeyForIntegral018[
   ...
   sectorKeyFromPattern018[sectorPattern018[rootTopo, linePacks]]
   ];
...
(* ::Package:: *)
(* 本模块把论文 vertex-basis 公式实现限制在 Private 内部，并为 018 的 tree 公开接口提供
...
dsPureTimeDirectTemplateRecord018[
   ...
   {sectorKey, publicIntegral, publicRecord, baseIntegral, discreteVariables,
   ...
   discreteRules = Thread[Extract[baseIntegral, discretePositions] -> Extract[publicIntegral, discretePositions]];
   metadata = SelectFirst[
     context["sectors"],
...
DSTreeSeeds[vertex_, int : J[_, _, _], context_Association] /; dsContextQ[context] := Module[
   ...
   metadata = integralSectorMetadata018[context["topology"], int];
...
dsTreePublicResult018[result_Association, context_Association, auditLevel_: "standard"] := Module[
   {public, masters, legacyIntegrals, privateTokens, representationAudit},
   ...
   representationAudit = If[
     ...
     <|"status" -> "audited", "passQ" -> TrueQ[legacyIntegrals === {} && privateTokens === {}],
       "legacyVertexIntegrals" -> legacyIntegrals, "privateTreeTokens" -> privateTokens|>,
     <|"status" -> "producerGuaranteed", "passQ" -> True,
       "legacyVertexIntegrals" -> {}, "privateTreeTokens" -> {},
