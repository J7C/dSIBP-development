...
numerical vector.  Only $m$ is required; the leading power, sample grid, working precision,
and surplus fitted orders are automatic unless the user overrides them.  Every regulator
...
\node[box, text width=.82\textwidth] (input) at (0,0)
  {Input \texttt{DEmatrix}, \texttt{boundary}, \texttt{path}, requested
   \texttt{maximum\_power} $m$, target digits, and optional overrides};
\node[decision, text width=.29\textwidth] (leading) at (0,-1.55)
  {Leading power supplied?};
\node[box, text width=.36\textwidth] (pilot) at (-3.20,-3.25)
  {No: run nested fixed-regulator pilot solves, infer a common stable integer
   $n_{\min}$ from component ratios, and fail closed if ambiguous};
\node[box, text width=.31\textwidth] (override) at (3.45,-3.25)
  {Yes: accept the user integer $n_{\min}$ and record that the pilot was bypassed};
\node[box, text width=.82\textwidth] (plan) at (0,-5.25)
...
\node[box, text width=.82\textwidth] (solve) at (0,-7.05)
...
\node[box, text width=.82\textwidth] (fit) at (0,-8.75)
...
\node[box, text width=.82\textwidth] (validate) at (0,-10.45)
...
\node[box, text width=.82\textwidth] (output) at (0,-12.15)
...
\draw[flow] (leading) -| node[above left]{no} (pilot);
\draw[flow] (leading) -| node[above right]{yes} (override);
\draw[flow] (pilot) -- (plan);
\draw[flow] (override) -- (plan);
...
automatic $\eps$ scale, surplus orders, precision rules, pilot, square interpolation, and
...
\begin{verbatim}
python -m pip install path/to/000_FlintNDE/code/package
\end{verbatim}
...
\subsection{de Sitter example}

This subsection is reserved for a future de Sitter IBP/DE example.  No placeholder equation
or untested numerical result is included in the present release.
...
$\mathbb Q(i)[s]$ arithmetic performs the polynomial gcd and square-free decomposition, and
the roots of those factors are isolated as Acb balls.  The pole order recorded at a root is
...
$|\Delta z|/R\leq f_{\mathrm{step}}$.  The executable path builder reports the
classifications of both endpoints and all singularities on every straight segment.  A
regular singular endpoint is replaced in the ordinary transport list by a nearby matching
point inside its Frobenius disk.  If a singularity lies strictly inside a segment, the
builder warns but does not stop.  The user may provide explicit complex
\texttt{detour\_points}; otherwise the executable path stores a unified local-basis bridge
between ordinary matching points on the two sides.  The bridge may use the original
Frobenius system, a Moser-reduced Fuchsian system, or a certified exponential-times-power--log
sector.  The incoming step is controlled by
$R_{\mathrm{sing}}(p)$ of the target singularity, not by the radius of the preceding
ordinary point.  The path reports the ordered list of all $|\Delta z|/R$ values and its
maximum.  Bridge evaluation uses the principal Acb logarithm/power branch, corresponding
to the upper-half-plane continuation from a negative real local coordinate to a positive
 one.  A high-pole checkpoint remains fail closed when neither the exact Moser reduction
 nor the certified exponential-sector gate supplies a local basis.
...
Singular start & The input is the exact $\{a,b,C\}$ power--log boundary record, not a finite
value at the singularity.  The package validates the indicial and log branch, evaluates it
at the first ordinary matching point, and then starts Taylor transport.  The matching
distance is limited by $R_{\rm sing}$, which excludes the singularity itself.  For a
certified exponential solution, the exponential sector is inferred from $C$ rather than
entered as a separate user parameter. \\
...
\end{verbatim}
The same pair may be used for adaptive-path start, target, or detour coordinates.  Internal
\texttt{NamedPoint} labels remain routing diagnostics and are not part of the save contract.
...
a direct regular singular start or target it contains the coordinate and reusable
$\{a,b,C\}$ terms.  The latter are boundary data, not $Y(z_*)$.  Saving a Lee--Moser
transformed original-basis singularity or an irregular/Stokes target remains fail closed
until a certified inverse local-jet or Stokes connection is available.
...
for this exact decision; the explicitly numerical fallback described below records its full
...
differential equation.  The implemented default instead determines the actual leading integer
power from pilot NDE evaluations.  For each nonzero output component it forms stable ratio
estimates
\begin{equation}
  \nu_i^{(k)}=
  \frac{\log|F_i(q\eps_k)/F_i(\eps_k)|}{\log q},
\end{equation}
and accepts a rounded integer only when several adjacent estimates agree within the
precision-dependent tolerance.  The candidate is then required to pass a complete
reconstruction and independent-sample residual check.  Ambiguous cancellation, a
noninteger exponent, or unstable ratios cause a fail-closed request for a user override;
they are never silently rounded.

The automatic pilot starts from the no-pole provisional depth
$K_{\rm pilot}=\max(0,m)$ and scale
\begin{equation}
 \eps_{\rm pilot}=10^{-g/(K_{\rm pilot}+1)}.
\end{equation}
It evaluates a nested grid with ratio $q$; after an unsuccessful round, the next round starts
at the smallest scale reached by the preceding one.  The automatic integer-consistency
tolerance is $10^{-\min(8,g/4)}$.  The starting scale, ratio, number of points, round limit,
and tolerance are all user-overridable.  These pilot rules are FlintNDE safeguards, not
formulas quoted from AMFlow.

Let the detected global leading power be $n_{\min}$, let the user request only the absolute
...
retained coefficients.  The pilot records an integer estimate for every nonzero component and
uses their minimum as the common Laurent start; it does not forcibly zero coefficients of the
other components.  All component coefficients are determined by the same square Acb
interpolation and are tested at the independent points.  The reported result includes the
pilot estimates, accepted leading powers, all effective parameters, the internal and returned
power ranges, coefficient vectors, and independent residuals.

For example, if the pilot stage finds $n_{\min}=-4$, while the user requests
...
19 production solves and two independent validation solves, in addition to any pilot solves.
...
\texttt{"automatic"} selects the documented pilot or production rule; a user value replaces
...
configure_working_precision(80)
...
it does not transform only the isolated leading vector.  For an exponential start, the exact constant-basis transform of $C_i$ identifies
the sector and the exponential of Eq.~\eqref{eq:implemented-exponential-factor} is supplied
automatically; the user does not enter $k$ or $\Phi$.  One term must not mix distinct
...
logarithm/power branch.  The same protocol is used independently at every pilot, production,
...
There is no public \texttt{series\_range}: the lowest power is detected from the solved
quantity or supplied through \texttt{leading\_power}. \\
...
\texttt{leading\_power="automatic"} & Automatic runs the stable pilot-ratio test and then
requires successful reconstruction and validation.  An integer override skips the pilot
classification. \\
...
\texttt{pilot\_sample\_count=4} & Number of nested points used to estimate each component's
leading power when \texttt{leading\_power="automatic"}. \\
\texttt{pilot\_base\_sample=}\newline\texttt{"automatic"} & Starting pilot scale;
automatic means $10^{-g/(\max(0,m)+1)}$. \\
\texttt{pilot\_ratio=0.5} & Scale ratio $q$ between adjacent pilot points. \\
\texttt{pilot\_max\_rounds=3} & Maximum number of successively smaller pilot grids before an
ambiguous leading power fails closed. \\
\texttt{leading\_power\_}\newline\texttt{tolerance="automatic"} & Maximum spread and distance from a common
integer accepted in the pilot estimates.  Automatic means $10^{-\min(8,g/4)}$ and the
effective value is reported. \\
...
cap.  Pilot and validation evaluations are reported separately. \\
...
Failure classes & \texttt{LeadingPowerDetectionError} rejects unstable, noninteger, or
ambiguous pilot behavior.  \texttt{SeriesValidationError} rejects an unresolved NDE
refinement, interpolation, or independent-sample residual. \\
\bottomrule
\end{longtable}
\normalsize
