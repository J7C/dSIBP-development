...
- 高阶 pole 的 exact Lee--Moser projector-balance 降阶、可严格解耦 sector 的
  指数乘 power-log 局部基，以及单重不同主导根二阶 pole 的起点形式渐近基；
- 正规化参数的自动采样、工作精度规划和 Laurent 幂级数解重构。

目录分工：

- `code/package/`：发布名为 `FlintNDE`、导入名为 `flintnde` 的可安装程序包与独立测试；
- `code/example/`：通过顶部路径变量加载程序包的示例；
- `note/`：论文源文件、论文结构和发布路线规划；
...
二阶常微分方程示例放在 `code/example/`，不进入程序包本体接口。
...
python -m pip install "path\to\000_FlintNDE\code\package"
python -m pip install -e path/to/000_FlintNDE/code/package
```
...
复根；因此同一矩阵元内相消的候选 pole 不会进入奇点清单。无穷远使用 `sinv=1/s`，完整变换为
...
列向量；正则奇点记录含坐标和可再次输入的 `{a,b,C}` 边界数据，不把奇点处通常不存在的
有限 `Y(z_*)` 冒充为结果。当前只有直接 regular-singular power-log 起点/终点可保存这种
边界；Lee--Moser 后原基和需要 Stokes 数据的不规则奇点 fail closed。
...
本征/广义本征子空间或把普通有限值冒充奇点边界时，均在计算前拒绝。指数型局部解的
指数 sector 由高阶 Laurent 矩阵自动推断；用户不另填 `k`，但跨多个指数 sector 的 `C`
必须拆成多项。该协议也由
