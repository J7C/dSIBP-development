...
from .boundary import FrobeniusBoundary
from .core import acb_midpoint_matrix, relative_difference_inf
from .local_solutions import build_local_solution_basis
from .savepoints import SavePointWriter, frobenius_terms_from_constants, vector_record
...
def _resolve_transport_boundary(
    ...
    if start is None or start.kind == "ordinary":
        if isinstance(boundary, FrobeniusBoundary):
            raise TypeError("Frobenius {a,b,C} boundary requires a regular-singular path start")
...
def _write_singular_start_request(
    ...
    """奇点起点直接保存已经过局部系统验证的用户 ``{a,b,C}``。"""

    if writer is None:
        return
    for request in requests:
        if request["classification"] != "ordinary" and request["role"] == "start":
            if not isinstance(boundary, FrobeniusBoundary):
                raise TypeError("saved singular start requires a Frobenius {a,b,C} boundary")
            writer.write(
                request,
                {
                    "resultType": "frobenius_boundary",
                    "result": {"terms": boundary.to_json()},
                },
            )
...
def _write_singular_target_request(
    ...
    for request in target_requests:
        ...
        terms = frobenius_terms_from_constants(basis, constants, writer.digits)
        writer.write(
            request,
            {
                "resultType": "frobenius_boundary",
                "result": {
                    "terms": terms,
                    "canonicalBasisConstants": vector_record(constants, writer.digits),
...
def transport_path(
    ...
    *,
    ...
    _write_singular_start_request(writer, save_requests, initial_vector)
