...
def _preflight_local_basis(
    ...
    record = next(
        (
            ...
            if item.identifier == classification.singularity_identifier
        ),
        None,
    )
...
def build_adaptive_path(
    ...
    *,
    ...
    if any(item.kind != "ordinary" for item in route_classifications[1:-1]):
        bad_names = [
            item.name for item in route_classifications[1:-1] if item.kind != "ordinary"
        ]
        raise ValueError(
            "detour points must be ordinary; singular detour points: " + ", ".join(bad_names)
        )
    ...
    for role, classification in (
        ...
        *(("internal", item) for item in internal_classifications),
    ...
    if internal_by_leg:
        ...
        method_label = (
            "Frobenius bridges"
            if all(item.kind == "regular_singular" for item in internal_classifications)
    ...
    return AdaptivePath(
        ...
        internal_singularities=internal_classifications,
