...
用户环境只要求 Python 3.10 或更高版本和 `python-flint>=0.6`。三种安装方式为：
...
# 开发者可编辑安装
python -m pip install -e "path\to\000_FlintNDE\code\package"
```

`pip` 会自动安装满足版本要求的 `python-flint`。安装一次后，可在任意目录 `A` 的脚本中
...
`maximum_power=m` 是唯一必填的展开阶数。自动 pilot 从多个嵌套小 `ep` 点的分量模比判定
最低整数幂 `n_min`；不稳定、非整数或含混的零分量会抛出
`LeadingPowerDetectionError`。随后定义
...
独立验证点位于生产网格的 `validation_scale` 倍尺度，完全不参与插值；NDE 主/参考链以及
...
局部边界数据，不是奇点处的 `Y(z_*)`。直接 regular-singular power-log 起点/终点已支持；
Lee--Moser 后原基和需要 Stokes connection 的奇点保存目前拒绝执行。
...
`../../note/FlintNDE.pdf` 的 “Installation and public interface” 一节。
