...
ke::usage = "ke[i] 表示只进入顶点时间相位、与圈动量空间独立的外腿能量参数。";
...
ds::usage = "ds[expr,sij] 对外部独立变量 sij 求总导数，同时作用于积分指标和显式动力学系数。";
rep2innerform::usage = "rep2innerform[expr] 把用户 sp/sij 表示转换为当前 topology 的内部坐标表示。";
rep2outform::usage = "rep2outform[expr] 把内部标量积坐标转换为用户 sp/sij 表示。";
...
ScalingWeights::usage = "ScalingWeights 指定 Euler 算符中各变量的系数；例如 s11=ks^2 时 s11 的权重为 2。";
