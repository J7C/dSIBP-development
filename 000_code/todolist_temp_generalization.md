...
- [x] seed preset 初始化：`quickCheck/fullDiscrete/bounded` 统一配置 `seedRanges`、默认离散枚举模式和 seed/batch 上限，未知 preset 只给 warning，显式 `seedRanges/seedOptions` 可覆盖，batch 调用的显式 option 优先级最高。
