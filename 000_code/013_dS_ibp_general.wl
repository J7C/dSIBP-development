...
rep2innerform[expr_, topoSpec_Association] := Module[{topo},
   topo = resolvePublicTopologyContext[topoSpec];
   If[topo === $Failed, Return[$Failed]];
...
repIterative[expr_, end_, data_Association, OptionsPattern[]] /;
   KeyExistsQ[data, "vertices"] && ! KeyExistsQ[data, "families"] := repIterativeData[
   expr, end, data, MaxIterations -> OptionValue[MaxIterations]
   ]["result"];
...
repIterative[expr_, end_, context_Association, OptionsPattern[]] /; KeyExistsQ[context, "families"] := repIterativeSectorData[
   expr, end, context, MaxIterations -> OptionValue[MaxIterations]
   ]["result"];
