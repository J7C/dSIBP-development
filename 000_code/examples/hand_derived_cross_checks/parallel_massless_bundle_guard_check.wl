(* ::Package:: *)
(* parallel_massless_bundle_guard：读取独立 expected，并作为 009 程序包交叉验证 example。 *)
...
handDerivedDir = FileNameJoin[{codeDir, "check", "hand-derived-v2", "parallel_massless_bundle_guard"}];
...
getParallelMasslessTop[signKey_String] := If[
   KeyExistsQ[topologyCache, signKey],
   topologyCache[signKey],
   topologyCache[signKey] = parseTopology[makeParallelMasslessBundleGuardCase[signKey]]
   ];

getParallelMasslessSector[signKey_String, "top"] := getParallelMasslessTop[signKey];
getParallelMasslessSector[signKey_String, sector_String] := shrinkSectorTopology[
   getParallelMasslessTop[signKey],
   manualShrunkLines[sector]
   ];

parallelMasslessActual[relation_Association] := Module[
   {topo, int, label, generator, raw},
   topo = getParallelMasslessSector[relation["vertexSigns"], relation["sector"]];
...
relationResults = MapIndexed[
   Function[{relation, index},
    Module[{actual, difference},
     actual = Expand[parallelMasslessActual[relation] /. {kk[1, 1] -> s11, kk[1, 2] -> s12, kk[2, 2] -> s22}];
...
Print["parallel_massless_bundle_guard relations: ",
  Count[Lookup[relationResults, "passQ"], True], "/", Length[relationResults]];
Print["relation counts correct: ", SameQ[actualCounts, parallelMasslessExpectedCounts]];
Print["expected total: ", parallelMasslessExpectedTotal];

If[! SameQ[actualCounts, parallelMasslessExpectedCounts], Exit[1]];
