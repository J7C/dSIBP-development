(* ::Package:: *)
(* two_loop_isp_toy：读取独立 expected，并作为 011 程序包交叉验证 example。 *)
...
handDerivedDir = FileNameJoin[{codeDir, "check", "hand-derived-v2", "two_loop_isp_toy"}];
...
getTwoLoopISPTop[signKey_String] := If[
   KeyExistsQ[topologyCache, signKey],
   topologyCache[signKey],
   topologyCache[signKey] = parseTopology[makeTwoLoopISPToyCase[signKey]]
   ];

getTwoLoopISPSector[signKey_String, "top"] := getTwoLoopISPTop[signKey];
getTwoLoopISPSector[signKey_String, sector_String] := shrinkSectorTopology[
   getTwoLoopISPTop[signKey],
   manualShrunkLines[sector]
   ];

twoLoopISPActual[relation_Association] := Module[
   {topo, int, label, generator, raw},
   topo = getTwoLoopISPSector[relation["vertexSigns"], relation["sector"]];
...
relationResults = MapIndexed[
   Function[{relation, index},
    Module[{actual, difference},
     actual = Expand[twoLoopISPActual[relation] /. {kk[1, 1] -> s11}];
...
Print["two_loop_isp_toy relations: ",
  Count[Lookup[relationResults, "passQ"], True], "/", Length[relationResults]];
Print["relation counts correct: ", SameQ[actualCounts, twoLoopISPExpectedCounts]];
Print["expected total: ", twoLoopISPExpectedTotal];

If[! SameQ[actualCounts, twoLoopISPExpectedCounts], Exit[1]];
