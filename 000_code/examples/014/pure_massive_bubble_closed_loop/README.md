...
3. reduction 完成后再次运行 `wolframscript -file main.wl`，执行 `DSKiraImport -> DSDE[{s11,P0}] -> DSScaleCheck[{2,1}]`。
...
仓库不再保存本 014 历史例的 `init/`、`kira/` 或 `results/` 生成目录；需要时必须从 `main.wl` 重新生成。
