(* ::Package:: *)
(* 本文件生成 pure massless bubble 的最小 numeric Kira workspace，并检查 012 的基础文件导出门禁。
   它只构造小样本 canonical seed（含 contact 可达 massless shrink sectors），不运行 Kira。 *)
...
smokeOutputDirectory = FileNameJoin[{
    projectRoot, "000_code", "test", "results_test", "kira_export_smoke_massless_bubble_012"
    }];
...
smokeCheckResults = <|
   ...
   "smallSeedAndLinearSystem" -> TrueQ[
     Lookup[smokeSeedBatch, "equationCount", 0] > 0 &&
      Lookup[smokeLinearSystem, "equationCount", 0] === Lookup[smokeSeedBatch, "equationCount", -1] &&
      Lookup[smokeLinearSystem, "integralCount", 0] > 0 &&
