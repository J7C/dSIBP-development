(* ::Package:: *)
(* vertex_energy_signs：读取独立 expected，并作为 012 程序包交叉验证 test。 *)
...
handDerivedDir = FileNameJoin[{codeDir, "test", "012_hand-derived", "vertex_energy_signs"}];
...
getVertexEnergyTop[signKey_String] := If[
   KeyExistsQ[topologyCache, signKey],
   topologyCache[signKey],
   topologyCache[signKey] = parseTopology[makeVertexEnergySignsCase[signKey]]
   ];

getVertexEnergySector[signKey_String, "top"] := getVertexEnergyTop[signKey];
getVertexEnergySector[signKey_String, sector_String] := shrinkSectorTopology[
   getVertexEnergyTop[signKey],
   manualShrunkLines[sector]
   ];

vertexEnergyActual[relation_Association] := Module[
   {topo, int, label, generator, raw},
   topo = getVertexEnergySector[relation["vertexSigns"], relation["sector"]];
...
relationResults = MapIndexed[
   Function[{relation, index},
    Module[{actual, difference},
     actual = Expand[vertexEnergyActual[relation] /. {kk[1, 1] -> s11}];
...
Print["vertex_energy_signs relations: ",
  Count[Lookup[relationResults, "passQ"], True], "/", Length[relationResults]];
Print["relation counts correct: ", SameQ[actualCounts, vertexEnergyExpectedCounts]];
Print["expected total: ", vertexEnergyExpectedTotal];

If[! SameQ[actualCounts, vertexEnergyExpectedCounts], Exit[1]];
