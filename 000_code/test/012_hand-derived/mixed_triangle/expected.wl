...
manualTriangleSectors[signKey_String] := manualTriangleSectorName /@
  Select[Subsets[manualTriangleFullLines[signKey]], Length[#] <= 2 &];
...
manualTriangleCanonicalIntegral[int_J, sector_String] := Module[
   {repMap = manualTriangleRepMap[manualTriangleShrunkLines[sector]], newPacks = int[[2]], endpoints},
   Do[
    endpoints = Lookup[repMap, manualTriangleLineEndpoints[line]];
    If[Length[newPacks[[line]]] === 3 && SameQ @@ endpoints &&
      newPacks[[line, {2, 3}]] === {1, 0},
...
manualTriangleCanonical[expr_, sector_String] := Module[
   {repMap = manualTriangleRepMap[manualTriangleShrunkLines[sector]], endpoints, massiveCanonical},
   massiveCanonical = Expand[expr /. int_J :> manualTriangleCanonicalIntegral[int, sector]];
   endpoints = Lookup[repMap, manualTriangleLineEndpoints[3]];
   Expand[
    massiveCanonical /. (int_J /; Length[int[[2, 3]]] === 2 && int[[2, 3, 2]] === 1 && endpoints[[1]] === endpoints[[2]]) :> 0
    ]
   ];
   ...
   newA = Table[
     Module[{oldSlots = Flatten@Position[Lookup[repNew, oldActive], newActive[[i]]]},
      If[MemberQ[Lookup[repNew, manualTriangleLineEndpoints[line]], newActive[[i]]] &&
       ...
       Total[aList[[oldSlots]]] - manualTriangleLineShrinkShift[line],
