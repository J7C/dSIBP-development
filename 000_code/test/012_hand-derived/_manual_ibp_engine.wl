...
manualReachableShrinkLineSets[def_Association, signKey_String] := Module[
   {lines = manualFullLines[def, signKey], queue = {{}}, seen = <||>, result = {},
    state, repMap, eligible, bundles, events, newState, key},
   AssociateTo[seen, ToString[{}, InputForm] -> True];
   While[queue =!= {},
    state = First[queue];
    queue = Rest[queue];
    repMap = manualRepMap[def, state];
    eligible = Select[
      Complement[lines, state],
      ! SameQ @@ Lookup[repMap, manualLineEndpoints[def, #]] &
      ];
    bundles = GatherBy[
      eligible,
      Sort[manualVertexPosition[def, #] & /@ Lookup[repMap, manualLineEndpoints[def, #]]] &
      ];
   ...
   Prepend[SortBy[result, {Length[#], #} &], {}]
   ];

manualSectors[def_Association, signKey_String] := manualSectorName /@ manualReachableShrinkLineSets[def, signKey];
...
manualCanonicalIntegral[def_Association, int_J, sector_String] := Module[
   {repMap = manualRepMap[def, manualShrunkLines[sector]], newPacks = int[[2]], line, endpoints},
   Do[
    line = manualLinePosition[def, lineId];
    endpoints = Lookup[repMap, manualLineEndpoints[def, lineId]];
    If[
     manualLineMass[def, lineId] === "massive" && Length[newPacks[[line]]] === 3 &&
      SameQ @@ endpoints && newPacks[[line, {2, 3}]] === {1, 0},
     newPacks[[line, {2, 3}]] = newPacks[[line, {3, 2}]]
     ],
    {lineId, manualLineIds[def]}
    ];
   J[int[[1]], newPacks, int[[3]]]
   ];

manualCanonical[def_Association, expr_, sector_String] := Module[
   {repMap = manualRepMap[def, manualShrunkLines[sector]], lineIds = manualLineIds[def], massiveCanonical},
   massiveCanonical = Expand[expr /. int_J :> manualCanonicalIntegral[def, int, sector]];
   Expand[
    massiveCanonical /. (int_J /; AnyTrue[lineIds,
         Function[lineId,
...
manualShrinkLinesIntegral[def_Association, int_J, sector_String, specs_List] := Module[
   {aList, packs, isps, shrunk, selected, newSector, oldActive, newActive, repNew,
    newA, selectedShiftForRep, pairs, linePos},
   {aList, packs, isps} = List @@ int;
   shrunk = manualShrunkLines[sector];
   selected = Lookup[specs, "lineId"];
   newSector = manualSectorName[Union[shrunk, selected]];
   ...
   pairs = manualLineEndpoints[def, #] & /@ selected;
   newA = Table[
     Module[{oldSlots = Flatten@Position[Lookup[repNew, oldActive], newActive[[i]]]},
      selectedShiftForRep = Total[MapThread[
         If[SameQ @@ Lookup[repNew, #1] && First[Lookup[repNew, #1]] === newActive[[i]], #2, 0] &,
         {pairs, Lookup[specs, "aShift"]}
         ]];
      Total[aList[[oldSlots]]] - selectedShiftForRep
      ],
     {i, Length[newActive]}
     ];
   Scan[
    Function[spec,
     linePos = manualLinePosition[def, spec["lineId"]];
     packs[[linePos]] = {packs[[linePos, 1]] + spec["bShift"]}
     ],
    specs
    ];
...
manualShrinkIntegral[def_Association, int_J, sector_String, lineId_] :=
  manualShrinkLinesIntegral[
   def, int, sector,
   {<|
     "lineId" -> lineId,
     "bShift" -> manualLineShrinkBShift[def, lineId],
     "aShift" -> If[manualLineMass[def, lineId] === "massive", manualLineShrinkBShift[def, lineId], 0]
     |>}
   ];
...
manualBoundaryAtomicSpecs[def_Association, signKey_String, sector_String, int_J, vertex_, lineId_] := Module[
   {line = manualLinePosition[def, lineId], slots, endpointSlot, n1, n2, nValue,
    endpointOrientation, sk, coefficient},
   slots = manualEndpointSlots[def, sector, vertex, lineId];
   If[Length[slots] =!= 1, Return[{}]];
   endpointSlot = First[slots];
   endpointOrientation = If[endpointSlot === 1, 1, -1];
   If[manualLineMass[def, lineId] === "massive",
    {n1, n2} = int[[2, line, {2, 3}]];
    If[n1 + n2 =!= 1, Return[{}]];
    sk = manualLineSk[def, signKey, lineId];
    coefficient = manualMassiveShrinkPrefactor[def, lineId] *
      (-1)^(int[[2, line, endpointSlot + 1]] + manualVpm[sk]);
    {<|"lineId" -> lineId, "coefficient" -> coefficient, "bShift" -> 1, "aShift" -> 1|>},
    nValue = int[[2, line, 2]];
    If[nValue =!= 1, Return[{}]];
    {<|"lineId" -> lineId, "coefficient" -> -2 endpointOrientation, "bShift" -> 0, "aShift" -> 0|>}
    ]
   ];

manualThetaBundleKey[def_Association, sector_String, lineId_] := Module[
   {repMap = manualRepMap[def, manualShrunkLines[sector]]},
   Sort[manualVertexPosition[def, #] & /@ Lookup[repMap, manualLineEndpoints[def, lineId]]]
   ];

manualTimeBoundaryBundleTerms[def_Association, signKey_String, sector_String, int_J, vertex_] := Module[
   {shrunk = manualShrunkLines[sector], eligible, bundles, oddSubsets, atomicChoices},
   eligible = Select[
     Complement[manualFullLines[def, signKey], shrunk],
     Length[manualEndpointSlots[def, sector, vertex, #]] === 1 &
     ];
   bundles = GatherBy[eligible, manualThetaBundleKey[def, sector, #] &];
   Total[Table[
     oddSubsets = Select[Rest[Subsets[bundle]], OddQ[Length[#]] &];
     Total[Table[
       atomicChoices = manualBoundaryAtomicSpecs[def, signKey, sector, int, vertex, #] & /@ selected;
       If[AnyTrue[atomicChoices, # === {} &],
        0,
        Total[
         Function[choice,
            2^(1 - Length[selected]) Times @@ Lookup[choice, "coefficient"] *
             manualShrinkLinesIntegral[def, int, sector, KeyDrop[#, "coefficient"] & /@ choice]
...
manualTimeEquation[def_Association, signKey_String, sector_String, stateRules_List, ispRules_List, vertex_] := Module[
   {int, power, phase, lineTerms, boundaryTerms},
   ...
   boundaryTerms = manualTimeBoundaryBundleTerms[def, signKey, sector, int, vertex];
   manualCanonical[def, Expand[power + phase + lineTerms + boundaryTerms], sector]
   ];
