(* ::Package:: *)
(* 014 高层 seed/linear/Kira-export 与 loop-to-tree 零点显式系数专项。 *)
...
loopCase = <|
   "name" -> "014WorkflowPureMasslessBubble",
   "vertexData" -> {{v1, "+"}, {v2, "+"}},
   "lineData" -> {
...
outputDir = FileNameJoin[{codeDir, "test", "results_test", "014_workflow_smoke", "kira"}];
...
deDir = FileNameJoin[{codeDir, "test", "results_test", "014_workflow_smoke", "results", "dlogDE"}];
...
treeCase = <|
   "name" -> "014TreeZeroPointProjection",
   "vertexData" -> {{u1, "+"}, {u2, "+"}},
   "lineData" -> {
...
tripleCase = <|
   "name" -> "014TripleMassiveContactProjection",
   "vertexData" -> {{w1, "+"}, {w2, "+"}},
   "lineData" -> {
...
Print["014 workflow smoke check: ", Count[Values[checks], True], "/", Length[checks]];
