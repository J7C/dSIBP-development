...
shrinkLineIntegral[topo_Association, J[aList_, linePacks_, ispList_], e_Integer, bShift_: Automatic, aShift_: Automatic] := Module[
   ...
   effectiveAShift = If[
     aShift === Automatic,
     If[Lookup[line, "massType", "massive"] === "massless", 1, effectiveBShift],
     aShift
     ];
...
timeThetaBoundaryShrinkTerms[topo_Association, J[aList_, linePacks_, ispList_], vertexId_] := Module[
   {pos, connectedLines, lines = topo["lines"], endpointSlots, endpointSlot,
     endpointOrientation, pack, packType, coeff, shrinkTerms},
   pos = vertexPosition[topo, vertexId];
   If[Head[pos] === Missing, Return[0]];
   connectedLines = topo["vertexLines"][[pos]][[All, 1]];
   Total[
     ...
     If[Length[endpointSlots] =!= 1,
      ...
      Switch[packType,
       "massiveFull",
        ...
        shrinkTerms = lineCompiledShrinkTerms[lines[[e]]];
        Total[
         coeff (Lookup[#, "coefficient", 0] /. topo["shrinkPrefactorRules"]) *
            shrinkLineIntegral[topo, J[aList, linePacks, ispList], e, Lookup[#, "bShift", 1]] & /@ shrinkTerms
         ],
       "masslessFull",
