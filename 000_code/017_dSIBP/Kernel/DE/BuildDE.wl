...
(* 旧 loop 投影只保留为正式 check 的单向 oracle，不再被 017 生产 DE 调用。 *)
...
dsTreeNaiveDERaw017[context_Association, variables_: Automatic, masters_: Automatic, OptionsPattern[DSTreeNaiveDE]] /; dsContextQ[context] := Module[
   {ibpData},
   If[treeFormulaMasslessPendingQ017[context],
    Return[treeFormulaPendingRederivation017["DSTreeNaiveDE", context]]
    ];
   ibpData = dsTreeNaiveIBPRaw017[context, masters, ProgressReporting -> OptionValue[ProgressReporting]];
