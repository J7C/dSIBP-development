(* ::Package:: *)
(* 本模块统一 017 的 massless 双端点 relation、sector-aware canonical/求导和 parity seed 域。
   parity 只筛选待作用生成元的 seed 点；生成后证书只报告错误，绝不把积分替换为零。 *)

(* ::Chapter:: *)
(*Massless 双端点 quotient*)

masslessFullLineQ017[line_Association] := Lookup[line, "packType", ""] === "masslessFull" &&
   Lookup[line, "state", "full"] =!= "shrunk";


canonicalizeMasslessIntegral017[
   topo_Association,
...
applyMasslessEndpointCanonical[expr_, topo_Association] := Module[{quotient},
   quotient = Expand[expr /. int_J :> canonicalizeMasslessIntegral017[topo, int]];
   Expand[quotient /. (int_J /; masslessCoincidentAntisymmetricIntegralQ[topo, int]) :> 0]
   ];


masslessBuiltInRelationData017[topo_Association] := Map[
   Function[e,
   ...
   Select[Range[topo["nE"]], masslessFullLineQ017[topo["lines"][[#]]] &]
   ];
...
toggleMasslessEndpointState017[
...
toggleMasslessLineState[topo_Association, int_J, e_Integer] :=
  toggleMasslessEndpointState017[int, e, 1];
     ...
     Switch[actualLinePackType[topo, e, linePacks[[e]]],
      ...
      Total@Table[
        shiftedIntegral = shiftLinePower[
          topo, toggleMasslessEndpointState017[int, e, endpointSlot], e, -1
          ];
   ...
   Switch[packType,
    ...
    Total@Table[
      -I sigma shiftVertexA[
        toggleMasslessEndpointState017[int, e, endpointSlot],
...
fixedLineShrinkResidualPower017[
   topo_Association,
   e_Integer,
   integerShift_,
   zeroPointShift_
   ...
   id = topo["lines"][[e, "id"]];
   sourceZero = zeroPointRuleValue017[rootRules, b0[id], 0];
   targetZero = lineTargetShrinkZeroPoint017[topo, e];
...
shrinkLineIntegral[
   ...
   {line = topo["lines"][[e]], uSlot, vSlot, oldActive, newRepMap, newActive,
    ...
    effectiveBShift, effectiveAShift, powerCoefficient},
   ...
   powerCoefficient = If[
     ...
     1,
     fixedLineMomentumMagnitude[topo, e]^(-fixedLineShrinkResidualPower017[
        topo, e, effectiveBShift, lineShrinkZeroPointShift[line]
        ])
     ];
   newLinePacks[[e]] = If[
     lineIndexedPowerQ[line],
     {lineIntegerPowerIndex[topo, int, e] + effectiveBShift},
     {fixedLineSentinel017[]}
     ];
   ...
   Scan[
    Function[spec,
     If[lineIndexedPowerQ[topo["lines"][[spec["lineIndex"]]]],
      ...
      newLinePacks[[spec["lineIndex"]]] = {fixedLineSentinel017[]}
      ]
     ],
    specs
    ];
...
sectorAwareCanonicalTerm017[term_, rootTopo_Association] := Module[
   ...
   Which[
    ...
    sectorTopo = sectorTopologyForIntegral017[rootTopo, int];
    If[Head[sectorTopo] === Missing, $Failed, applySeedCanonical[coefficient int, sectorTopo]]
    ]
   ];


sectorAwareCanonical017[expr_, rootTopo_Association] := Module[{terms, result},
   terms = linearTerms[Expand[expr]];
   result = sectorAwareCanonicalTerm017[#, rootTopo] & /@ terms;
...
ds[expr_, userVariable_, topoSpec_Association] := Module[
   {rootTopo, variableData, userVariables, userExpr, linearData, internalVariable,
    coefficientDerivative, integralDerivativeTerms, sectorTopo, result},
   ...
   result = sectorAwareCanonical017[
...
rep2Integrand[expr_, topoSpec_Association] := Module[{rootTopo, result, sectorTopo},
   rootTopo = resolvePublicTopologyContext[topoSpec];
   If[rootTopo === $Failed, Return[$Failed]];
   If[! validatePublicExpression[expr, rootTopo], Return[$Failed]];
   result = Expand[expr /. int_J :> (
        sectorTopo = sectorTopologyForIntegral017[rootTopo, int];
        If[Head[sectorTopo] === Missing, $Failed, integralToInertIntegrand[sectorTopo, int]]
        )];
...
normalizeParityConstraints017[constraints_] := Map[
   Function[item,
    Which[
...
lineFunctionPreset017[line_Association] := Lookup[
...
parityFunctionSystemUsableQ017[topo_Association] := Module[{massiveLines, presets},
   massiveLines = Select[
     topo["lines"],
     Lookup[#, "massType", "massive"] === "massive" &
     ];
   presets = lineFunctionPreset017 /@ massiveLines;
   massiveLines =!= {} && And @@ (MemberQ[{"h", "H"}, #] & /@ presets)
   ];


parityLineVariables017[line_Association] := {
   b[line["id"]], n[line["id"], 1], n[line["id"], 2]
   };


transportParityConstraint017[topo_Association, constraint_Association] := Module[
   ...
   Do[
    line = topo["lines"][[e]];
    vars = parityLineVariables017[line];
    ...
    If[shrunkQ && lineIndexedPowerQ[line],
     If[! TrueQ[Expand[c1 - c2] === 0],
      ...
      sourceZero = zeroPointRuleValue017[rootRules, b0[line["id"]], 0];
...
parityMetadataForSector017[topo_Association] := Module[
   {raw, normalized, transported, invalid, usableFunctionQ},
   raw = Lookup[topo, "parityConstraints", {}];
   usableFunctionQ = parityFunctionSystemUsableQ017[topo];
   If[raw === {} || raw === None,
   ...
   normalized = normalizeParityConstraints017[raw];
   transported = transportParityConstraint017[topo, #] & /@ normalized;
...
parityIntegralIndexRules017[metadata_Association, int_J] := Module[{lineRules, ispRules},
...
parityIntegralSignature017[metadata_Association, int_J] := Module[{data, rules},
   data = Lookup[metadata, "parityData", <|"status" -> "disabled"|>];
   If[Lookup[data, "status", "disabled"] =!= "enabled", Return[{}]];
   rules = parityIntegralIndexRules017[metadata, int];
   Mod[Expand[(#expression - #remainder) /. rules], 2] & /@ Lookup[data, "constraints", {}]
   ];


parityIntegralAllowedQ017[metadata_Association, int_J] := Module[{signature},
   signature = parityIntegralSignature017[metadata, int];
   signature === {} || And @@ (TrueQ[# === 0] & /@ signature)
   ];


dsParityFilteredPointRules017[entry_Association, pointRules_List, context_Association] := Module[
   ...
   Select[pointRules, parityIntegralAllowedQ017[metadata, sourceIntegral /. #] &]
   ];


dsParityCertificate017[records_List, context_Association] := Module[
...
treeFormulaMasslessLines017[context_Association] := Lookup[
   Select[
...
treeFormulaMasslessPendingQ017[context_Association] := treeFormulaMasslessLines017[context] =!= {};


treeFormulaPendingRederivation017[operation_String, context_Association] := Module[{lines},
   lines = treeFormulaMasslessLines017[context];
   dsErrorPrint[
